...
/obj/attack_alien(mob/living/carbon/Xenomorph/user)
	if(attack_generic(user, rand(user.xeno_caste.melee_damage_lower, user.xeno_caste.melee_damage_upper), BRUTE, "melee", 0))
		playsound(loc, "alien_claw_metal", 25, 1)
		if(user.stealth_router(HANDLE_STEALTH_CHECK)) //Cancel stealth if we have it due to aggro.
			user.stealth_router(HANDLE_STEALTH_CODE_CANCEL)
