...
/obj/effect/landmark/corpsespawner/proc/equip_items_to_mob(mob/living/carbon/human/corpse)
	if(corpseuniform)
		corpse.equip_to_slot_or_del(new corpseuniform(corpse), ITEM_SLOT_ICLOTHING)
	if(corpsesuit)
		corpse.equip_to_slot_or_del(new corpsesuit(corpse), ITEM_SLOT_OCLOTHING)
	if(corpseshoes)
		corpse.equip_to_slot_or_del(new corpseshoes(corpse), ITEM_SLOT_FEET)
	if(corpsegloves)
		corpse.equip_to_slot_or_del(new corpsegloves(corpse), ITEM_SLOT_GLOVES)
	if(corpseradio)
		corpse.equip_to_slot_or_del(new corpseradio(corpse), ITEM_SLOT_EARS)
	if(corpseglasses)
		corpse.equip_to_slot_or_del(new corpseglasses(corpse), ITEM_SLOT_EYES)
	if(corpsemask)
		corpse.equip_to_slot_or_del(new corpsemask(corpse), ITEM_SLOT_MASK)
	if(corpsehelmet)
		corpse.equip_to_slot_or_del(new corpsehelmet(corpse), ITEM_SLOT_HEAD)
	if(corpsebelt)
		corpse.equip_to_slot_or_del(new corpsebelt(corpse), ITEM_SLOT_BELT)
	if(corpsepocket1)
		corpse.equip_to_slot_or_del(new corpsepocket1(corpse), ITEM_SLOT_R_POCKET)
	if(corpsepocket2)
		corpse.equip_to_slot_or_del(new corpsepocket2(corpse), ITEM_SLOT_L_POCKET)
	if(corpseback)
		corpse.equip_to_slot_or_del(new corpseback(corpse), ITEM_SLOT_BACK)
	if(corpseid)
		...
		corpse.equip_to_slot_or_del(newid, ITEM_SLOT_ID)
