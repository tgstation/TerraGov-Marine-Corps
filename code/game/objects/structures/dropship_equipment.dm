...
/datum/action/innate/rappel_designate/Activate()
	...
	if(origin.rappel_condition < RAPPEL_CONDITION_GOOD || target_turf.density || target_area.ceiling >= CEILING_UNDERGROUND)
...
/obj/structure/dropship_equipment/rappel_system
	...
	///Stores current condition of the rappel (i.e whether it's usable or not)
	var/rappel_condition = RAPPEL_CONDITION_GOOD
	///Stores current state of the rappel (deployed, retracting...)
...
/obj/structure/dropship_equipment/rappel_system/update_icon_state()
	. = ..()
...
/obj/structure/dropship_equipment/rappel_system/proc/self_repair()
...
/obj/structure/dropship_equipment/rappel_system/attack_ghost(mob/dead/observer/user)
	. = ..()
	if(rappel_state == RAPPEL_STATE_USABLE)
		user.forceMove(get_turf(rope))

///Human animation for dropping down
/obj/structure/dropship_equipment/rappel_system/proc/rappel_animation(mob/living/user)
...
/obj/structure/dropship_equipment/rappel_system/proc/buckle(mob/living/carbon/xenomorph/X)
	...
	X.balloon_alert_to_viewers("[X] rips the rappel cord out from above, disabling it!","You have disabled their sky-rope!")
