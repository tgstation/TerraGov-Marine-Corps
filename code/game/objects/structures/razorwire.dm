...
/obj/structure/razorwire/attackby(obj/item/W, mob/user)
	...
	if(iswirecutter(W))
		...
		if(do_after(user,delay_disassembly, TRUE, src, USER_ICON_BUILD))
	...
	if(istype(W, /obj/item/tool/pickaxe/plasmacutter))
		...
		if(P.start_cut(user, src.name, src, PLASMACUTTER_BASE_COST * PLASMACUTTER_LOW_MOD))
			if(do_after(user, P.calc_delay(user) * PLASMACUTTER_LOW_MOD, TRUE, src, USER_ICON_HOSTILE)) //Barbed wire requires half the normal time
