...
/obj/structure/orbital_cannon/proc/fire_ob_cannon(turf/T, mob/user)
	...
	for(var/mob/living/screentext_receiver AS in receivers)
		screentext_receiver.play_screen_text(HUD_ANNOUNCEMENT_FORMATTING("ORBITAL STRIKE IMMINENT", "TYPE: [uppertext(tray.warhead.warhead_kind)]", CENTER_ALIGN_TEXT), /atom/movable/screen/text/screen_text/command_order)
