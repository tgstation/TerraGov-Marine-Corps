...
/obj/item/tool/deconstructor
	name = "rapid field disassembler"
	desc = "The RFD is employed by engineers to rapidly deconstruct fortified positions, allowing for efficient repositioning of resources or rapid penetration of hostile defences."
...
/obj/item/tool/deconstructor/preattack(atom/target, mob/user, params)
	if(!isbarricade(target) && !istype(target, /obj/structure/razorwire))
		return

	var/obj/structure/target_object = target
	if(!COOLDOWN_FINISHED(target_object, tool_cooldown))
		return FALSE
	COOLDOWN_START(target_object, tool_cooldown, 1 SECONDS)
	...
	var/list/stack_list = target_object.deconstruct(!get_self_acid(), user)
	if(!ishuman(user))
		return TRUE
