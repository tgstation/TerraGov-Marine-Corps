...
/obj/structure/door_assembly/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/pen))
		...
		if(mineral)
			...
			user.visible_message("<span class='notice'>[user] welds the [mineral] plating off the airlock assembly.</span>", "<span class='notice'>I start to weld the [mineral] plating off the airlock assembly...</span>")
			if(W.use_tool(src, user, 40, volume=50))
				to_chat(user, "<span class='notice'>I weld the [mineral] plating off.</span>")
				new mineral_path(loc, 2)
				var/obj/structure/door_assembly/PA = new previous_assembly(loc)
				transfer_assembly_vars(src, PA)

		else if(glass)
			user.visible_message("<span class='notice'>[user] welds the glass panel out of the airlock assembly.</span>", "<span class='notice'>I start to weld the glass panel out of the airlock assembly...</span>")
			if(W.use_tool(src, user, 40, volume=50))
				to_chat(user, "<span class='notice'>I weld the glass panel out.</span>")
			...
			user.visible_message("<span class='warning'>[user] disassembles the airlock assembly.</span>", \
								"<span class='notice'>I start to disassemble the airlock assembly...</span>")
			if(W.use_tool(src, user, 40, volume=50))
				to_chat(user, "<span class='notice'>I disassemble the airlock assembly.</span>")
				deconstruct(TRUE)
		...
		if(!anchored )
			...
			if(door_check)
				user.visible_message("<span class='notice'>[user] secures the airlock assembly to the floor.</span>", \
									 "<span class='notice'>I start to secure the airlock assembly to the floor...</span>", \
									 "<span class='hear'>I hear wrenching.</span>")

				if(W.use_tool(src, user, 40, volume=100))
					if(anchored)
						return
					to_chat(user, "<span class='notice'>I secure the airlock assembly.</span>")
			...
			user.visible_message("<span class='notice'>[user] unsecures the airlock assembly from the floor.</span>", \
								 "<span class='notice'>I start to unsecure the airlock assembly from the floor...</span>", \
								 "<span class='hear'>I hear wrenching.</span>")
			if(W.use_tool(src, user, 40, volume=100))
				if(!anchored)
					return
				to_chat(user, "<span class='notice'>I unsecure the airlock assembly.</span>")
				name = "airlock assembly"
				setAnchored(FALSE)
		...
		user.visible_message("<span class='notice'>[user] wires the airlock assembly.</span>", \
							"<span class='notice'>I start to wire the airlock assembly...</span>")
		if(W.use_tool(src, user, 40, amount=1))
			if(state != AIRLOCK_ASSEMBLY_NEEDS_WIRES)
				return
			state = AIRLOCK_ASSEMBLY_NEEDS_ELECTRONICS
			to_chat(user, "<span class='notice'>I wire the airlock assembly.</span>")
		...
		user.visible_message("<span class='notice'>[user] cuts the wires from the airlock assembly.</span>", \
							"<span class='notice'>I start to cut the wires from the airlock assembly...</span>")

		if(W.use_tool(src, user, 40, volume=100))
			if(state != AIRLOCK_ASSEMBLY_NEEDS_ELECTRONICS)
				return
			to_chat(user, "<span class='notice'>I cut the wires from the airlock assembly.</span>")
		...
		user.visible_message("<span class='notice'>[user] installs the electronics into the airlock assembly.</span>", \
							"<span class='notice'>I start to install electronics into the airlock assembly...</span>")
					...
					if(!glass)
						if(istype(G, /obj/item/stack/sheet/rglass) || istype(G, /obj/item/stack/sheet/glass))
							...
							user.visible_message("<span class='notice'>[user] adds [G.name] to the airlock assembly.</span>", \
												"<span class='notice'>I start to install [G.name] into the airlock assembly...</span>")
							if(do_after(user, 40, target = src))
								if(G.get_amount() < 1 || glass)
									return
								if(G.type == /obj/item/stack/sheet/rglass)
									to_chat(user, "<span class='notice'>I install [G.name] windows into the airlock assembly.</span>")
									...
									to_chat(user, "<span class='notice'>I install regular glass windows into the airlock assembly.</span>")
							...
							if(G.get_amount() >= 2)
								...
								if(do_after(user, 40, target = src))
									if(G.get_amount() < 2 || mineral)
										return
									to_chat(user, "<span class='notice'>I install [M] plating into the airlock assembly.</span>")
									G.use(2)
									var/mineralassembly = text2path("/obj/structure/door_assembly/door_assembly_[M]")
									var/obj/structure/door_assembly/MA = new mineralassembly(loc)
									transfer_assembly_vars(src, MA, TRUE)
							else
								to_chat(user, "<span class='warning'>I need at least two sheets add a mineral cover!</span>")
					else
						to_chat(user, "<span class='warning'>I cannot add [G] to [src]!</span>")
				else
					to_chat(user, "<span class='warning'>I cannot add [G] to [src]!</span>")
		...
		user.visible_message("<span class='notice'>[user] finishes the airlock.</span>", \
							 "<span class='notice'>I start finishing the airlock...</span>")
