...
/obj/machinery/self_destruct/rod/attack_hand(mob/living/user)
	...
	switch(active_state)
		if(SELF_DESTRUCT_MACHINE_ACTIVE)
			...
			user.balloon_alert(user, "You twist and release the control rod, arming it.")
		...
		if(SELF_DESTRUCT_MACHINE_ARMED)
			...
			user.balloon_alert(user, "You twist and release the control rod, disarming it.")
			...
			user.balloon_alert(user, "The control rod is not ready.")
