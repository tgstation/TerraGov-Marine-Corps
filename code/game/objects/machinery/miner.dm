...
/obj/machinery/miner/welder_act(mob/living/user, obj/item/I)
	...
	if(user.skills.getRating(SKILL_ENGINEER) < SKILL_ENGINEER_ENGI)
		...
		if(!do_after(user, fumbling_time, NONE, src, BUSY_ICON_UNSKILLED, extra_checks = CALLBACK(weldingtool, PROC_REF(isOn))))
			return FALSE
	playsound(loc, 'sound/items/weldingtool_weld.ogg', 25)
	user.visible_message(span_notice("[user] starts welding [src]'s internal damage."),
	span_notice("You start welding [src]'s internal damage."))
	add_overlay(GLOB.welding_sparks)
	if(!do_after(user, 200, NONE, src, BUSY_ICON_BUILD, extra_checks = CALLBACK(weldingtool, PROC_REF(isOn))))
