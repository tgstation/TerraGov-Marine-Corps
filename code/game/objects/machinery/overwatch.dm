...
/obj/machinery/computer/camera_advanced/overwatch/Topic(href, href_list)
	...
	switch(href_list["operation"])
		...
		if("squad_transfer")
			transfer_squad()
		if("dropsupply")
			to_chat(usr, "[icon2html(src, usr)] <span class='warning'>This feature is no longer available from the overwatch console. Please try the supply drop computer instead!</span>")
		if("dropbomb")
			handle_bombard()
		if("shootrailgun")
