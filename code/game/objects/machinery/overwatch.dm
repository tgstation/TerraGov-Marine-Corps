				...
				if(!current_squad) //No squad has been set yet. Pick one.
					...
					if(length(GLOB.active_laser_targets))
						for(var/obj/effect/overlay/temp/laser_target/LT in current_squad.squad_laser_targets)
							if(!istype(LT))
								continue
							dat += "<a href='byond://?src=[REF(src)];operation=use_cam;cam_target=[REF(LT)];selected_target=[REF(LT)]'>[LT]</a><br>"
					else
...
/obj/machinery/computer/camera_advanced/overwatch/main/interact(mob/living/user)
	...
	var/dat
	if(!operator)
		dat += "<B>Main Operator:</b> <A href='byond://?src=[text_ref(src)];operation=change_main_operator'>----------</A><BR>"
	else
		dat += "<B>Main Operator:</b> <A href='byond://?src=[text_ref(src)];operation=change_main_operator'>[operator.name]</A><BR>"
		dat += "   <A href='byond://?src=[text_ref(src)];operation=logout_main'>{Stop Overwatch}</A><BR>"
		dat += "----------------------<br>"
		switch(state)
			if(OW_MAIN)
...
/obj/machinery/computer/camera_advanced/overwatch/proc/get_squad_info_ending()
	...
	dat += "<A href='byond://?src=[text_ref(src)];operation=change_sort'>{Change Sorting Method}</a><br>"
	dat += "<A href='byond://?src=[text_ref(src)];operation=hide_dead'>{[dead_hidden ? "Show Dead Marines" : "Hide Dead Marines" ]}</a><br>"
