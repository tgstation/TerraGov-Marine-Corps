			...
			if(OW_MAIN)
				if(!current_squad) //No squad has been set yet. Pick one.
					...
					if(length(GLOB.active_orbital_beacons))
						for(var/obj/item/orbital_bombardment_beacon/OB in current_squad.squad_orbital_beacons)
							if(!istype(OB))
								continue
	...
	if(!operator)
		...
		switch(state)
			if(OW_MAIN)
				...
				if(length(GLOB.active_orbital_beacons))
					for(var/obj/item/orbital_bombardment_beacon/OB in GLOB.active_orbital_beacons)
						if(!istype(OB))
							continue
						dat += "<a href='?src=\ref[src];operation=use_cam;cam_target=[REF(OB)];selected_target=[REF(OB)]'>[OB]</a><br>"
				else
