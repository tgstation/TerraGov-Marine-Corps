...
/mob/living/carbon/human/verb/issue_order(command_aura as null|text)
	...
	switch(command_aura)
		if("move")
			...
			message = pick(";ДВИГАЕМ БУЛКАМИ!", ";ПОШЁЛ, ПОШЁЛ, ПОШЁЛ!", ";ВЫДВИГАЕМСЯ!", ";ВПЕРЁД!", ";ВПЕРЁД! БЫСТРЕЕ!", ";ПРОДВИГАЕМСЯ!", ";ДВИГАЙ, ДВИГАЙ, ДВИГАЙ!", ";БЕГОМ, МАРШ!", ";ШИРЕ ШАГ!", ";ЧАЩЕ ШАГ!", ";ЗА МНОЙ!", ";ВПЕРЁД!", ";ДАВАЙ БЫСТРЕЕ!", ";УСКОРЯЕМСЯ!", ";НА ПРОРЫВ!")
			say(message)
			add_emote_overlay(move)
		if("hold")
			var/image/hold = image('icons/mob/talk.dmi', src, icon_state = "order_hold")
			message = pick(";НИ ШАГУ НАЗАД!", ";СТОЯТЬ НАСМЕРТЬ!", ";ДЕРЖАТЬ СТРОЙ!", ";ДЕРЖАТЬ ПОЗИЦИЮ!", ";ДЕРЖИМ УДАР!", ";ВСТАТЬ И СРАЖАТЬСЯ!", ";В УКРЫТИЕ!", ";ВЗЯТЬ ПОД КОНТРОЛЬ ТЕРРИТОРИЮ!", ";ПРИГОТОВИТЬСЯ К СТОЛКНОВЕНИЮ!", ";ДЕРЖАТЬСЯ!", ";ПРИБЛИЖАЮТСЯ!", ";НАЙДИТЕ УКРЫТИЕ!", ";БЕРЕЧЬ ГОЛОВУ!")
			say(message)
