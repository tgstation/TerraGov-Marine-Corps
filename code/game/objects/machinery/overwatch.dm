...
/mob/living/carbon/human/verb/issue_order(which as null|text)
	...
	switch(command_aura)
		...
		if("hold")
			...
			message = pick("ДЕРЖАТЬ СТРОЙ!", "УДЕРЖАТЬ ПОЗИЦИЮ!", "ВСЕМ В УКРЫТИЕ!", "ДЕРЖАТЬ ОБОРОНУ!")
			say(message)
			add_emote_overlay(hold)
		if("focus")
			var/image/focus = image('icons/mob/talk.dmi', src, icon_state = "order_focus")
			message = pick("НЕ ПАЛИТЕ ПО СВОИМ!", "СОСРЕДОТОЧИТЬ ОГОНЬ!", "ОГОНЬ ПО ГОТОВНОСТИ!", "ОРУЖИЕ НА ИЗГОТОВКУ!", "ЦЕЛЬСЯ!", "УСТРОИМ ИМ СВИНЦОВЫЙ ДОЖДЬ!")
			say(message)
