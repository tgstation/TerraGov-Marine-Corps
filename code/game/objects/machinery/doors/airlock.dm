...
/obj/machinery/door/airlock/proc/slide_through(mob/living/carbon/xenomorph/slider)
	for(var/atom/movable/AM in get_turf(src))
		if(AM != src && AM.density && !AM.CanPass(slider, slider.loc))
			to_chat(slider, span_warning("\The [AM] prevents you from squeezing under \the [src]!"))
			return
	if(locked || welded) //Can't pass through airlocks that have been bolted down or welded
		to_chat(slider, span_warning("\The [src] is locked down tight. You can't squeeze underneath!"))
		return
	slider.visible_message(span_warning("\The [slider] scuttles underneath \the [src]!"), \
	span_warning("You squeeze and scuttle underneath \the [src]."), null, 5)
	slider.forceMove(loc)
