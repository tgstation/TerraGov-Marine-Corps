...
/obj/machinery/computer/dropship_weapons/Topic(href, href_list)
	...
	if(href_list["open_fire"])
		...
		if(!istype(L))
			return
		if(!HAS_SKILL_LEVEL(usr, SKILL_PILOTING, SKILL_LEVEL_TRAINED)) //everyone can fire dropship weapons while fumbling.
			...
			var/fumbling_time = 100 - 20 * GET_SKILL(usr, SKILL_PILOTING)
