...
/obj/structure/computerframe/attackby(obj/item/attackedby, mob/user, params)
	switch(state)
		if(0)
		...
		if(1)
			...
			else if(istype(attackedby, /obj/item/circuitboard/computer) && !circuit)
			...
			else if(isscrewdriver(attackedby) && circuit)
				playsound(loc, 'sound/items/screwdriver.ogg', 25, 1)
				to_chat(user, "<span class='notice'>You screw the circuit board into place.</span>")
				state = 2
				icon_state = "2"

			else if(iscrowbar(attackedby) && circuit)
