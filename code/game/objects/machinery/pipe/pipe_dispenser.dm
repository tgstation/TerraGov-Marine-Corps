...
/obj/machinery/pipedispenser/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/pipe) || istype(I, /obj/item/pipe_meter))
		balloon_alert(user, "You put [I] back into [src].")
		...
		if(anchored)
			...
			balloon_alert(user, "You begin to unfasten \the [src] from the floor...")
			...
			playsound(loc, 'sound/items/ratchet.ogg', 25, 1)
			balloon_alert(user, "You begin to fasten \the [src] to the floor...")
			...
			balloon_alert(user, "You have fastened \the [src]. Now it can dispense pipes.")
