...
/datum/signal/subspace/vocal/broadcast()
	...
	if(speaker && speaker.voice && !banned)
		...
		for(var/mob/living/carbon/human/potential_hearer in receive)
			if(potential_hearer.stat >= UNCONSCIOUS || potential_hearer.disabilities & DEAF || !(potential_hearer.client?.prefs.sound_tts != TTS_SOUND_OFF) || potential_hearer == speaker)
				continue
