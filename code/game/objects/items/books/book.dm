...
/obj/item/book/attackby(obj/item/attackedby, mob/user, params)
	. = ..()
	...
	if(istype(attackedby, /obj/item/tool/pen))
		...
		if(attackedby.w_class >= 3)
			to_chat(user, "<span class='notice'>[attackedby] won't fit in [title].</span>")
			return

		user.drop_held_item()
		attackedby.forceMove(src)
		store = attackedby
		to_chat(user, "<span class='notice'>You put [attackedby] in [title].</span>")

	else if(istype(attackedby, /obj/item/tool/kitchen/knife) || iswirecutter(attackedby))
		if(carved)
			return
