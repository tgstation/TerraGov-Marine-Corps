...
/obj/item/restraints/handcuffs/proc/place_handcuffs(mob/living/carbon/target, mob/user)
	...
	if (ishuman(target))
		...
		if(do_mob(user, H, cuff_delay, BUSY_ICON_HOSTILE, BUSY_ICON_HOSTILE, extra_checks = CALLBACK(user, .proc/Adjacent, H)) && !H.handcuffed)
			if(H.has_limb_for_slot(SLOT_HANDCUFFED))
		...
		if(do_mob(user, target, 30, BUSY_ICON_HOSTILE, BUSY_ICON_HOSTILE, extra_checks = CALLBACK(user, .proc/Adjacent, target)) && !target.handcuffed)
			user.dropItemToGround(src)
