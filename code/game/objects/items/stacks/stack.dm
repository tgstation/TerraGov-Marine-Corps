...
/obj/item/stack/Topic(href, href_list)
	...
	if(href_list["make"])
		...
		if(R.skill_req && !HAS_SKILL_LEVEL(usr, SKILL_CONSTRUCTION, R.skill_req))
			building_time += R.time * D_GET_SKILL_DIFF(usr, SKILL_CONSTRUCTION, R.skill_req) * 0.5 // +50% time each skill point lacking.
		if(building_time)
