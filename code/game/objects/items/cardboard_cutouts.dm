...
/obj/item/cardboard_cutout/attack_hand(mob/living/user)
	if(user.used_intent.type == INTENT_HELP || pushed_over)
		return ..()
	user.visible_message("<span class='warning'>[user] pushes over [src]!</span>", "<span class='danger'>I push over [src]!</span>")
...
/obj/item/cardboard_cutout/attack_self(mob/living/user)
	if(!pushed_over)
		return
	to_chat(user, "<span class='notice'>I right [src].</span>")
	desc = initial(desc)
	icon = initial(icon)
...
/obj/item/cardboard_cutout/attackby(obj/item/I, mob/living/user, params)
	...
	if(I.force)
		user.visible_message("<span class='danger'>[user] hits [src] with [I]!</span>", \
			"<span class='danger'>I hit [src] with [I]!</span>")
		if(prob(I.force))
			push_over()
