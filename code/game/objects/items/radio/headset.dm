...
/obj/item/radio/headset/mainship/marine/Initialize(mapload, datum/squad/squad, rank)
	if(squad)
		...
		if(ispath(rank, /datum/job/terragov/squad/leader))
			dat += " leader"
			keyslot2 = squad.faction == FACTION_TERRAGOV ? /obj/item/encryptionkey/squadlead
			use_command = TRUE
			command = TRUE
		else if(ispath(rank, /datum/job/terragov/squad/engineer))
			dat += " engineer"
			keyslot2 = squad.faction == FACTION_TERRAGOV ? /obj/item/encryptionkey/engi
		else if(ispath(rank, /datum/job/terragov/squad/corpsman))
			dat += " corpsman"
			keyslot2 = squad.faction == FACTION_TERRAGOV ? /obj/item/encryptionkey/med
