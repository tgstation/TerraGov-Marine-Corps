...
/obj/item/bodybag/bluespace/examine(mob/user)
	...
	if(contents.len)
		...
		. += "<span class='notice'>I can make out the shape[s] of [contents.len] object[s] through the fabric.</span>"

/obj/item/bodybag/bluespace/Destroy()
	for(var/atom/movable/A in contents)
		A.forceMove(get_turf(src))
		if(isliving(A))
			to_chat(A, "<span class='notice'>I suddenly feel the space around you torn apart! You're free!</span>")
	return ..()
...
/obj/item/bodybag/bluespace/deploy_bodybag(mob/user, atom/location)
	...
	for(var/atom/movable/A in contents)
		A.forceMove(R)
		if(isliving(A))
			to_chat(A, "<span class='notice'>I suddenly feel air around you! You're free!</span>")
	R.open(user)
	R.add_fingerprint(user)
	R.foldedbag_instance = src
	moveToNullspace()
...
/obj/item/bodybag/bluespace/container_resist(mob/living/user)
	if(user.incapacitated())
		to_chat(user, "<span class='warning'>I can't get out while you're restrained like this!</span>")
		return
	user.changeNext_move(CLICK_CD_BREAKOUT)
	user.last_special = world.time + CLICK_CD_BREAKOUT
	to_chat(user, "<span class='notice'>I claw at the fabric of [src], trying to tear it open...</span>")
	...
	if(!do_after(user, 200, target = src))
		...
		return
	loc.visible_message("<span class='warning'>[user] suddenly appears in front of [loc]!</span>", "<span class='danger'>[user] breaks free of [src]!</span>")
	qdel(src)
