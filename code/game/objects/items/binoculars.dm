...
/obj/item/binoculars/tactical/examine(mob/user)
	...
	switch(mode)
		if(MODE_CAS)
			balloon_alert(user, "They are currently set to CAS marking mode.")
			to_chat(user, span_notice("They are currently set to CAS marking mode."))
		if(MODE_RANGE_FINDER)
			balloon_alert(user, "They are currently set to range finding mode.")
			to_chat(user, span_notice("They are currently set to range finding mode."))
		if(MODE_RAILGUN)
			balloon_alert(user, "They are currently set to railgun targeting mode.")
			to_chat(user, span_notice("They are currently set to railgun targeting mode."))
		if(MODE_ORBITAL)
			balloon_alert(user, "They are currently set to orbital bombardment mode.")
...
/obj/item/binoculars/tactical/verb/toggle_mode(mob/user)
	...
	if (laser)
		...
		return
	if(!changable)
		balloon_alert(user, "These binoculars only have one mode.")
	...
	switch(mode)
		if(MODE_CAS)
			balloon_alert(user, "You switch [src] to CAS marking mode.")
			to_chat(user, span_notice("You switch [src] to CAS marking mode."))
		if(MODE_RAILGUN)
			balloon_alert(user, "You switch [src] to railgun targeting mode.")
			to_chat(user, span_notice("You switch [src] to railgun targeting mode."))
		if(MODE_ORBITAL)
			balloon_alert(user, "You switch [src] to orbital bombardment targeting mode.")
			to_chat(user, span_notice("You switch [src] to orbital bombardment targeting mode."))
		if(MODE_RANGE_FINDER)
			balloon_alert(user, "You switch [src] to range finding mode.")
