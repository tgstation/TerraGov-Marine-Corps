...
/obj/item/binoculars/tactical/proc/acquire_target(atom/A, mob/living/carbon/human/user)
	...
	switch(mode)
		...
		if(MODE_RAILGUN)
			...
			if((SSmarine_main_ship.rail_gun?.last_firing + COOLDOWN_RAILGUN_FIRE) > world.time)
				...
				while(laser)
					SSmarine_main_ship.rail_gun?.fire_rail_gun(TU,user)
...
/obj/item/binoculars/tactical/proc/try_fire_ob(atom/A, mob/living/carbon/human/user)
	...
	var/warhead_type = SSmarine_main_ship.orbital_cannon.tray.warhead.name
