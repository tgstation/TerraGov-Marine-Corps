...
/obj/item/binoculars/tactical/examine(mob/user)
	...
	switch(mode)
		...
		if(MODE_RANGE_FINDER)
			user.balloon_alert(user, "They are set to range finding mode")
		if(MODE_RAILGUN)
			user.balloon_alert(user, "They are set to railgun targeting mode")
		if(MODE_ORBITAL)
...
/obj/item/binoculars/tactical/verb/toggle_mode(mob/user)
	...
	switch(mode)
		if(MODE_CAS)
			user.balloon_alert(user, "You switch to CAS marking mode")
		if(MODE_RAILGUN)
			user.balloon_alert(user, "You switch to railgun targeting mode.")
		if(MODE_ORBITAL)
			user.balloon_alert(user, "You switch to orbital bombardment targeting mode")
		if(MODE_RANGE_FINDER)
			user.balloon_alert(user, "You switch to range finding mode")
	update_icon()
