...
/obj/item/defibrillator/attack(mob/living/carbon/human/H, mob/living/carbon/human/user)
	...
	if(istype(G))
		notify_ghost(G, "<font size=3>Someone is trying to revive your body. Return to it if you want to be resurrected!</font>", ghost_sound = 'sound/objects/bikehorn.ogg', enter_text = "Enter", enter_link = "reentercorpse=1", source = H, action = NOTIFY_JUMP)
	else if(!H.client)
