			...
			if(1)
				...
				if(isfloorturf(A))
					...
					if(checkResource(3, user))
						...
						playsound(src.loc, 'sound/machines/click.ogg', 15, 1)
						if(do_after(user, 20, TRUE, T, USER_ICON_BUILD, extra_checks = CALLBACK(src, .proc/checkResource, 3, user)))
							useResource(3, user)
							activate()
			...
			if(2)
				if(isfloorturf(A))
					if(checkResource(10, user))
						...
						playsound(src.loc, 'sound/machines/click.ogg', 15, 1)
						if(do_after(user, 50, TRUE, A, USER_ICON_BUILD, extra_checks = CALLBACK(src, .proc/checkResource, 10, user)))
							useResource(10, user)
							activate()
			...
			if(3)
				if(iswallturf(A))
					...
					if(checkResource(5, user))
						...
						playsound(src.loc, 'sound/machines/click.ogg', 15, 1)
						if(do_after(user, 40, TRUE, WL, USER_ICON_BUILD, extra_checks = CALLBACK(src, .proc/checkResource, 5, user)))
							useResource(5, user)
							activate()
				...
				if(isfloorturf(A))
					...
					if(checkResource(5, user) && !F.is_plating())
						...
						playsound(src.loc, 'sound/machines/click.ogg', 15, 1)
						if(do_after(user, 50, TRUE, F, USER_ICON_BUILD, extra_checks = CALLBACK(src, .proc/checkResource, 5, user)))
							useResource(5, user)
							activate()
