	...
	if(I.tool_behaviour == TOOL_WRENCH)
		if(mode == DISCONNECTED)
			...
			if(isturf(T) && !T.intact)
				...
				if(!attached)
					...
					user.visible_message( \
						...
						"<span class='notice'>I bolt \the [src] into the floor and connect it to the cable.</span>",
						"<span class='hear'>I hear some wires being connected to something.</span>")
			else
				to_chat(user, "<span class='warning'>\The [src] must be placed over an exposed, powered cable node!</span>")
		else
			...
			user.visible_message( \
				...
				"<span class='notice'>I unbolt \the [src] from the floor and detach it from the cable.</span>",
				"<span class='hear'>I hear some wires being disconnected from something.</span>")

	else if(I.tool_behaviour == TOOL_SCREWDRIVER)
		user.visible_message( \
			"[user] messes with \the [src] for a bit.", \
			"<span class='notice'>I can't fit the screwdriver into \the [src]'s bolts! Try using a wrench.</span>")
	else
		return ..()
	...
	switch(mode)
		...
		if(CLAMPED_OFF)
			user.visible_message( \
				...
				"<span class='hear'>I hear a click.</span>")
