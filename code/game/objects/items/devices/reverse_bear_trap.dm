...
/obj/item/reverse_bear_trap/process()
	...
	if(!time_left || !isliving(loc))
		...
		soundloop.stop()
		soundloop2.stop()
		to_chat(loc, "<span class='danger'>*ding*</span>")
	...
	if(iscarbon(user))
		...
		if(C.get_item_by_slot(SLOT_HEAD) == src)
			if(HAS_TRAIT_FROM(src, TRAIT_NODROP, REVERSE_BEAR_TRAP_TRAIT) && !struggling)
				...
				"<span class='danger'>I [fear_string] try to pull at [src]...</span>", "<i>I hear clicking and ticking.</i>")
				...
				if(!prob(escape_chance))
					...
					"<span class='danger'>I force open the padlock!</span>", "<i>I hear a single, pronounced click!</i>")
...
/obj/item/reverse_bear_trap/attack(mob/living/target, mob/living/user)
	...
	"<span class='danger'>[target] starts forcing [src] onto your head!</span>", "<i>I hear clanking.</i>")
	to_chat(user, "<span class='danger'>I start forcing [src] onto [target]'s head...</span>")
	...
	"<span class='danger'>[target] locks [src] onto your head!</span>", "<i>I hear a click, and then a timer ticking down.</i>")
	to_chat(user, "<span class='danger'>I force [src] onto [target]'s head and click the padlock shut.</span>")
...
/obj/item/reverse_bear_trap/proc/snap()
	...
	if(!istype(H) || H.get_item_by_slot(SLOT_HEAD) != src)
		...
		jill.visible_message("<span class='boldwarning'>[src] goes off in [jill]'s mouth, ripping [jill.p_their()] head apart!</span>", "<span class='danger'>[src] goes off!</span>")
		jill.emote("scream")
