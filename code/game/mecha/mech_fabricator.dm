...
/obj/machinery/mecha_part_fabricator/proc/emag()
	...
	switch(emagged)
		if(0)
			emagged = 0.5
			src.visible_message("[icon2html(src, viewers(src))] <b>[src]</b> beeps: \"DB error \[Code 0x00F1\]\"")
			sleep(10)
			src.visible_message("[icon2html(src, viewers(src))] <b>[src]</b> beeps: \"Attempting auto-repair\"")
			sleep(15)
			src.visible_message("[icon2html(src, viewers(src))] <b>[src]</b> beeps: \"User DB corrupted \[Code 0x00FA\]. Truncating data structure...\"")
			sleep(30)
			src.visible_message("[icon2html(src, viewers(src))] <b>[src]</b> beeps: \"User DB truncated. Please contact your WY system operator for future assistance.\"")
			req_access = null
			emagged = 1
		if(0.5)
			src.visible_message("[icon2html(src, viewers(src))] <b>[src]</b> beeps: \"DB not responding \[Code 0x0003\]...\"")
		if(1)
			src.visible_message("[icon2html(src, viewers(src))] <b>[src]</b> beeps: \"No records in User DB\"")
	return
...
/obj/machinery/mecha_part_fabricator/proc/build_part(var/obj/item/part)
	...
	if(being_built)
		...
		src.visible_message("[icon2html(src, viewers(src))] <b>[src]</b> beeps, \"The following has been completed: [src.being_built] is built\".")
...
/obj/machinery/mecha_part_fabricator/proc/process_queue()
	...
	src.visible_message("[icon2html(src, viewers(src))] <b>[src]</b> beeps, \"Queue processing finished successfully\".")
	return 1
...
*/
	...
	for(var/obj/machinery/computer/rdconsole/RDC in get_area(src))
		...
		if(!silent)
			...
			src.updateUsrDialog()
		if(i || tech_output)
			src.visible_message("[icon2html(src, viewers(src))] <b>[src]</b> beeps, \"Successfully synchronized with R&D server. New data processed.\"")
	if(found == 0)
		temp = "Couldn't contact R&D server.<br>"
		temp += "<a href='?src=\ref[src];clear_temp=1'>Return</a>"
		src.updateUsrDialog()
		src.visible_message("[icon2html(src, viewers(src))] <b>[src]</b> beeps, \"Error! Couldn't connect to R&D server.\"")
	return
...
/obj/machinery/mecha_part_fabricator/attack_hand(mob/user as mob)
	...
	if(exit.density)
		src.visible_message("[icon2html(src, viewers(src))] <b>[src]</b> beeps, \"Error! Part outlet is obstructed\".")
		return
	if(temp)
		left_part = temp
