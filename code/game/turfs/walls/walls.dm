...
/turf/closed/wall/attackby(obj/item/W, mob/user)
	...
	if(damage && iswelder(W))
		...
		if(WT.remove_fuel(0, user))
			...
			if(do_after(user, max(5, round(damage / 5)), TRUE, src, extra_checks = CALLBACK(WT, /obj/item/tool/weldingtool/proc/isOn)) && iswallturf(src))
	...
	switch(d_state)
		if(0)
			if(iswelder(W))
				...
				if(do_after(user, 60, TRUE, src, extra_checks = CALLBACK(WT, /obj/item/tool/weldingtool/proc/isOn)) && !d_state)
					d_state++
		...
		if(1)
			if(isscrewdriver(W))
				...
				if(do_after(user, 60, TRUE, src) && d_state == 1)
					d_state++
		...
		if(2)
			if(iswelder(W))
				...
				if(do_after(user, 60, TRUE, src, extra_checks = CALLBACK(WT, /obj/item/tool/weldingtool/proc/isOn)) && d_state == 2)
					d_state++
		...
		if(3)
			if(iscrowbar(W))
				...
				if(do_after(user, 60, TRUE, src) && d_state == 3)
					d_state++
		...
		if(4)
			if(iswrench(W))
				...
				if(do_after(user, 60, TRUE, src) && d_state == 4)
					d_state++
		...
		if(5)
			if(iswirecutter(W))
				...
				if(do_after(user, 60, TRUE, src) && d_state == 5)
					d_state++
		...
		if(6)
			if(iscrowbar(W))
				...
				if(do_after(user, 60, TRUE, src) && d_state == 6)
					d_state++
		...
		if(7)
			if(iswelder(W))
				...
				if(do_after(user, 60, TRUE, src, extra_checks = CALLBACK(WT, /obj/item/tool/weldingtool/proc/isOn)))
					if(!iswallturf(src))
						return
