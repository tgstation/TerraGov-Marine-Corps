...
/datum/game_mode/huntergames/declare_completion()
	...
	if(round_finished == 1 && !isnull(winner) && istype(winner))
		...
		to_chat(world, "<span class='warning'> <FONT size = 4><B>We have a winner! >> [winner.real_name] << defeated all enemies!</B></FONT></span>")
		...
		to_chat(world, "<span class='warning'> <FONT size = 4><B>NOBODY WON!?</B></FONT></span>")
