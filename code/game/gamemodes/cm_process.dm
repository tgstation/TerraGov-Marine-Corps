...
/datum/game_mode/proc/declare_completion_announce_round_stats()
	set waitfor = 0
	sleep(140)
	if(prefs.toggles_chat & CHAT_STATISTICS)
		to_chat(world, "<span class='round_body'>The end of round statistics are:</span>")
		to_chat(world, " ")//gotta get that formatting on.
		to_chat(world, "There were [round_statistics.total_bullets_fired] total bullets fired.")
		to_chat(world, "[round_statistics.total_bullet_hits_on_humans] bullets managed to hit marines. For a [(round_statistics.total_bullet_hits_on_humans / max(round_statistics.total_bullets_fired, 1)) * 100]% friendly fire rate!")
		if(round_statistics.total_bullet_hits_on_xenos)
			to_chat(world, "[round_statistics.total_bullet_hits_on_xenos] bullets managed to hit xenomorphs. For a [(round_statistics.total_bullet_hits_on_xenos / max(round_statistics.total_bullets_fired, 1)) * 100]% accuracy total!")
		if(round_statistics.grenades_thrown)
			to_chat(world, "[round_statistics.grenades_thrown] total grenades exploding.")
		else
			to_chat(world, "No grenades exploded.")
		...
		if(round_statistics.carrier_traps)
			to_chat(world, "[round_statistics.carrier_traps] hidey holes for huggers were made.")
