	...
	for(var/mob/player in player_list)
		if(player?.client?.prefs?.toggles_chat & CHAT_STATISTICS)
			...
			if(round_statistics.grenades_thrown)
				to_chat(player, "[round_statistics.grenades_thrown] total grenades exploding.")
			else
				to_chat(player, "No grenades exploded.")
			if(round_statistics.now_pregnant)
				to_chat(player, "[round_statistics.now_pregnant] people infected among which [round_statistics.total_larva_burst] burst. For a [(round_statistics.total_larva_burst / max(round_statistics.now_pregnant, 1)) * 100]% successful delivery rate!")
			if(round_statistics.queen_screech)
				to_chat(player, "[round_statistics.queen_screech] Queen screeches.")
			if(round_statistics.ravager_ravage_victims)
				to_chat(player, "[round_statistics.ravager_ravage_victims] ravaged victims. Damn, Ravagers!")
			if(round_statistics.warrior_limb_rips)
				to_chat(player, "[round_statistics.warrior_limb_rips] limbs ripped off by Warriors.")
			if(round_statistics.crusher_stomp_victims)
				to_chat(player, "[round_statistics.crusher_stomp_victims] people stomped by crushers.")
			if(round_statistics.praetorian_spray_direct_hits)
				to_chat(player, "[round_statistics.praetorian_spray_direct_hits] people hit directly by Praetorian acid spray.")
			if(round_statistics.weeds_planted)
				to_chat(player, "[round_statistics.weeds_planted] weed nodes planted.")
			if(round_statistics.weeds_destroyed)
				to_chat(player, "[round_statistics.weeds_destroyed] weed tiles removed.")
			if(round_statistics.carrier_traps)
				to_chat(player, "[round_statistics.carrier_traps] hidey holes for huggers were made.")
