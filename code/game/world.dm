...
/world/Reboot(ping)
	if(ping)
		...
		send2update("Round ID [GLOB.round_id] finished | Next Ground Map: [SSmapping?.next_map_configs[GROUND_MAP] ? SSmapping.next_map_configs[GROUND_MAP].map_name : "Default"] | Next Ship Map: [SSmapping?.next_map_configs[SHIP_MAP] ? SSmapping.next_map_configs[SHIP_MAP].map_name : "Default"] | Round End State: [SSticker?.mode?.round_finished] | Players: [length(GLOB.clients)]")
	TgsReboot()
