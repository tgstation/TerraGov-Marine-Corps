	...
	switch(build_step)
		...
		if(PTURRET_START_INTERNAL_ARMOUR)
			if(I.tool_behaviour == TOOL_WRENCH)
				...
				to_chat(user, "<span class='notice'>I start to remove the turret's interior metal armor...</span>")

				if(I.use_tool(src, user, 20, volume=50, amount=5)) //uses up 5 fuel
					build_step = PTURRET_BOLTED
					to_chat(user, "<span class='notice'>I remove the turret's interior metal armor.</span>")
		...
		if(PTURRET_GUN_EQUIPPED)
			if(isprox(I))
				...
				if(!user.temporarilyRemoveItemFromInventory(I))
					return
				to_chat(user, "<span class='notice'>I add the proximity sensor to the turret.</span>")
				qdel(I)
				return
		...
		if(PTURRET_START_EXTERNAL_ARMOUR)
			if(I.tool_behaviour == TOOL_WELDER)
				...
				to_chat(user, "<span class='notice'>I begin to weld the turret's armor down...</span>")
...
/obj/machinery/porta_turret_construct/attack_hand(mob/user)
	...
	switch(build_step)
		if(PTURRET_GUN_EQUIPPED)
			...
			to_chat(user, "<span class='notice'>I remove [installed_gun] from the turret frame.</span>")
			installed_gun = null
