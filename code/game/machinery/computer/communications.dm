	...
	switch(href_list["operation"])
		...
		if("distress")
			if(state == STATE_DISTRESS)
				...
				message_admins("L1 (Humans everywhere) [L[1]]")
				message_admins("L2 (Xenos everywhere) [L[2]]")
				message_admins("M1 (Humans ship) [M[1]]")
				message_admins("M2 (Xenos ship) [M[2]]")
				message_admins("Humans everywhere weighted [round(L[1] * 0.8)]")
				message_admins("Humans ship weighted [round(M[1] * 0.5)]")
				message_admins("Condition everywhere? [L[2] > round(L[1] * 0.8)]")
				message_admins("Condition ship? [M[2] > round(M[1] * 0.5)]")
