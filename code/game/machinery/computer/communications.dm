...
/obj/machinery/computer/communications/CMO
	...
	authenticated = 0
	list/messagetitle = list()
	list/messagetext = list()
	currmsg = 0
	aicurrmsg = 0
	state = STATE_DEFAULT
	aistate = STATE_DEFAULT
	cooldown_message = FALSE //Based on world.time.
	cooldown_request = FALSE
	cooldown_central = FALSE
	just_called = FALSE
	...
	datum/announcement/priority/command/crew_announcement = new

/obj/machinery/computer/communications/cmo/Topic(href, href_list)
	. = ..()
	if(.)
		return FALSE
...
/obj/machinery/computer/communications/cmo/attack_hand(var/mob/user as mob)
	if(..())
		return FALSE

	//Should be refactored later, if there's another ship that can appear during a mode with a comm console.
	if(!istype(loc.loc, /area/almayer/medical/lower_medical)) //Has to be in the CIC. Can also be a generic CIC area to communicate, if wanted.
		to_chat(usr, "<span class='warning'>Unable to establish a connection.</span>")
		return FALSE
...
/obj/machinery/computer/communications/cmo/post_status(command, data1, data2)
