...
//PORTA-SLEEPER
//Intended to be backpack sized when folded, and needs setup.
...
/obj/machinery/sleeper/portable_sleeper/attack_hand(mob/living/user)
	if(..())
		return
	if(machine_stat & (NOPOWER|BROKEN))
		return
	...
	if (occupant)
		...
		if(occupant.reagents)
			for(var/chemical in available_chemicals)
				dat += "[available_chemicals[chemical]]: [occupant.reagents.get_reagent_amount(chemical)] units<br>"
		dat += "<A href='?src=\ref[src];refresh=1'>Refresh Meter Readings</A><BR>"
		if(ishuman(occupant))
			if(stasis)
				dat += "<HR><A href='?src=\ref[src];togglestasis=1'>Deactivate Cryostasis</A><HR>"
			else
...
/obj/machinery/sleeper/portable_sleeper/Initialize()
	. = ..()
	if(orient == "RIGHT")
		icon_state = "sleeper_0-r"
		
/obj/machinery/sleeper/portable_sleeper/Destroy()
	occupant?.in_stasis = FALSE //clean up; end stasis; remove from processing
	occupant = null
	STOP_PROCESSING(SSobj, src)
	return ..()
...
/obj/machinery/sleeper/portable_sleeper/Topic(href, href_list)
	if (!href_list["scanreport"])
		return
	if(!hasHUD(usr,"medical"))
		return
...
/obj/machinery/sleeper/portable_sleeper/on_stored_atom_del(atom/movable/AM)
	if(AM == beaker)
		beaker = null

/obj/machinery/sleeper/portable_sleeper/process()
	if (machine_stat & (NOPOWER|BROKEN))
		if(occupant)
			occupant.in_stasis = null
		stasis = FALSE
...
/obj/machinery/sleeper/portable_sleeper/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/reagent_container/glass))
		if(beaker)
			to_chat(user, "<span class='warning'>The porta sleeper has no space for a beaker.</span>")
			return
...
/obj/machinery/sleeper/portable_sleeper/ex_act(severity)
	if(filtering)
		toggle_filter()
	switch(severity)
		if(1)
...
/obj/machinery/sleeper/portable_sleeper/emp_act(severity)
	if(filtering)
		toggle_filter()
	if(stasis)
		toggle_stasis()
...
/obj/machinery/sleeper/portable_sleeper/toggle_stasis()
	if(!occupant)
		stasis = FALSE
		return
	if(stasis)
...
/obj/machinery/sleeper/portable_sleeper/inject_chemical(mob/living/user as mob, chemical, amount)
	if(occupant && occupant.reagents)
		if(occupant.reagents.get_reagent_amount(chemical) + amount <= 20)
			occupant.reagents.add_reagent(chemical, amount)
			to_chat(user, "<span class='notice'>Occupant now has [occupant.reagents.get_reagent_amount(chemical)] units of [available_chemicals[chemical]] in his/her bloodstream.</span>")
...
/obj/machinery/sleeper/portable_sleeper/check(mob/living/user)
	if(occupant)
		to_chat(user, text("<span class='boldnotice'>Occupant ([]) Statistics:</span>", occupant))
		var/t1
		switch(occupant.stat)
...
/obj/machinery/sleeper/portable_sleeper/eject()
	set name = "Eject Sleeper"
	set category = "Object"
	set src in oview(1)
	if(usr.stat != 0)
...
/obj/machinery/sleeper/portable_sleeper/move_inside()
	set name = "Enter Sleeper"
	set category = "Object"
	set src in oview(1)
...
/obj/machinery/sleeper/portable_sleeper/Topic(href, href_list)
	...
	if(get_dist(src, user) > 1)
		return FALSE
	user.set_interaction(src)
	if(href_list["chemical"] && occupant)
		if (occupant.stat == DEAD)
			to_chat(usr, "<span class='warning'>This person has no life for to preserve anymore.</span>")
		else if(!(href_list["chemical"] in available_chemicals))
			message_admins("[ADMIN_TPMONTY(usr)] has tried to inject an invalid chem with the sleeper. Looks like an exploit attempt, or a bug.")
		else
			var/amount = text2num(href_list["amount"])
			if(amount == 5 || amount == 10)
