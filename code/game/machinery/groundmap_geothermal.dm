...
/obj/machinery/power/geothermal/process()
	...
	if(!powernet && !powernet_connection_failed) //Powernet checking, make sure there's valid cables & powernets
		...
		if(!check_failure()) //Wait! Check to see if it breaks during processing
			...
			switch(power_gen_percent)
				if(10) visible_message("[icon2html(src, viewers(src))] <span class='notice'><b>[src]</b> begins to whirr as it powers up.</span>")
				if(50) visible_message("[icon2html(src, viewers(src))] <span class='notice'><b>[src]</b> begins to hum loudly as it reaches half capacity.</span>")
				if(99) visible_message("[icon2html(src, viewers(src))] <span class='notice'><b>[src]</b> rumbles loudly as the combustion and thermal chambers reach full strength.</span>")
