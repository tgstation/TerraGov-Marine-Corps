...
/obj/machinery/power/geothermal/proc/check_failure()
	...
	if(cur_tick < fail_check_ticks) //Nope, not time for it yet
		return 0
	if(cur_tick > fail_check_ticks) //Went past with no fail, reset the timer
		cur_tick = 0
		return 0
...
/obj/machinery/power/geothermal/attackby(var/obj/item/O as obj, var/mob/user as mob)
	if(user.loc) start_loc = user.loc
...
/obj/machinery/colony_floodlight/attack_hand(mob/user as mob)
	if(ishuman(user))
		...
		if(damaged)
			...
			start_loc = user.loc
			if(do_after(user,50))
				if(user.loc != start_loc) return 0
				if(!src) return 0
				damaged = 1
				SetLuminosity(0)
