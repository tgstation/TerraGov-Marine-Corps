...
/obj/machinery/suit_storage_unit/carbon_unit
	SUIT_TYPE = /obj/item/clothing/suit/space/uscm
	HELMET_TYPE = /obj/item/clothing/head/helmet/space/uscm
	...
	for(i=0,i<4,i++)
		...
		if(src.OCCUPANT)
			if(src.issuperUV)
				...
				OCCUPANT.take_limb_damage(0,burndamage)
...
/*	spawn(200) //Let's clean dat shit after 20 secs  //Eh, this doesn't work
	...
	for(i=0,i<4,i++) //Gradually give the guy inside some damaged based on the intensity
		spawn(50)
			if(src.OCCUPANT)
				if(src.issuperUV)
					OCCUPANT.take_limb_damage(0,40)
					user << "Test. You gave him 40 damage"
				else
					OCCUPANT.take_limb_damage(0,8)
...
/obj/machinery/suit_storage_unit/attackby(obj/item/I as obj, mob/user as mob)
	...
	if ( istype(I, /obj/item/grab) )
		...
		if(do_after(user, 20, 5, BUSY_ICON_GENERIC))
...
/obj/machinery/suit_cycler/process()
	...
	if(occupant)
		...
		if(radiation_level > 2)
			occupant.take_limb_damage(0,radiation_level*2 + rand(1,3))
		if(radiation_level > 1)
			occupant.take_limb_damage(0,radiation_level + rand(1,3))
