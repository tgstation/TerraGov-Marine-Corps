...
/proc/get_forward_square_to_target(atom/center, width, height, requires_openturf = TRUE, requires_lineofsight = TRUE)
	...
	switch(center.dir)
		...
		if(SOUTH)
			lower_left = locate(center.x - width, center.y - width, center.z)
			upper_right = locate(center.x + width, center.y - 1, center.z)
		if(WEST)
			lower_left = locate(center.x - width, center.y - width, center.z)
			upper_right = locate(center.x - 1, center.y + width, center.z)
		if(EAST)
			lower_left = locate(center.x + 1, center.y - width, center.z)
			upper_right = locate(center.x + width, center.y + width, center.z)
