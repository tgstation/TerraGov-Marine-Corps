...
/proc/get_nearest_target(atom/source, distance, flags_target, attacker_faction, attacker_hive)
	if(!source)
		return
	var/atom/nearest_target
	var/shorter_distance = distance + 1
	if(flags_target & TARGET_HUMAN)
	...
	if(flags_target & TARGET_XENO)
		for(var/mob/nearby_xeno AS in cheap_get_xenos_near(source, shorter_distance - 1))
			...
			if(nearby_xeno.stat == DEAD || nearby_xeno.alpha <= HUNTER_STEALTH_RUN_ALPHA)
				continue
			if((nearby_xeno.flags_status & GODMODE) || (nearby_xeno.flags_status & INCORPOREAL)) //No attacking invulnerable/ai's eye!
				continue
			if(get_dist(source, nearby_xeno) < shorter_distance)
				nearest_target = nearby_xeno
				shorter_distance = get_dist(source, nearby_xeno)
	if(flags_target & TARGET_HUMAN_TURRETS)
	...
	if(flags_target & TARGET_UNMANNED_VEHICLE)
