...
/proc/cheap_get_humans_near(atom/movable/source, distance)
	. = list()
	for(var/mob/living/carbon/human/nearby_human as anything in GLOB.humans_by_zlevel["[source.z]"])
...
/proc/cheap_get_xenos_near(atom/movable/source, distance)
	. = list()
	for(var/mob/living/carbon/xenomorph/nearby_xeno as anything in GLOB.alive_xeno_list)
...
/proc/get_nearest_target(atom/source, distance, target_flags, attacker_faction, attacker_hive)
	...
	if(target_flags & TARGET_XENO)
		for(var/mob/nearby_xeno as anything in cheap_get_xenos_near(source, shorter_distance - 1))
	...
	if(target_flags & TARGET_HUMAN_TURRETS)
		for(var/atom/nearby_turret as anything in GLOB.marine_turrets)
	...
	if(target_flags & TARGET_UNMANNED_VEHICLE)
		for(var/atom/nearby_vehicle as anything in GLOB.unmanned_vehicles)
