...
proc/dress_for_job_default(var/mob/living/carbon/human/employee as mob, var/job_alias)
	...
	if(JOB)
		...
		employee.equip_to_slot_or_del(new item(employee), employee.WEAR_BODY)


		/*
	src.equip_to_slot_or_del(new /obj/item/weapon/storage/backpack/industrial (src), WEAR_BACK)
	src.equip_to_slot_or_del(new /obj/item/weapon/storage/box/engineer(src), WEAR_IN_BACK)
	src.equip_to_slot_or_del(new /obj/item/device/radio/headset/headset_eng (src), slot_ears) // -- TLE
	src.equip_to_slot_or_del(new /obj/item/device/pda/engineering(src), WEAR_WAIST)
	src.equip_to_slot_or_del(new /obj/item/clothing/under/rank/engineer(src), WEAR_BODY)
	src.equip_to_slot_or_del(new /obj/item/clothing/shoes/orange(src), WEAR_FEET)
	src.equip_to_slot_or_del(new /obj/item/clothing/head/helmet/hardhat(src), WEAR_HEAD)
	src.equip_to_slot_or_del(new /obj/item/weapon/storage/utilitybelt/full(src), WEAR_L_HAND) //currently spawns in hand due to traitor assignment requiring a PDA to be on the belt. --Errorage
	//src.equip_to_slot_or_del(new /obj/item/clothing/gloves/yellow(src), WEAR_HANDS) removed as part of Dangercon 2011, approved by Urist_McDorf --Errorage
	src.equip_to_slot_or_del(new /obj/item/device/t_scanner(src), WEAR_R_STORE)
	*/
