...
/datum/disease/wizarditis/stage_act()
	...
	switch(stage)
		if(2)
			...
			if(prob(1)&&prob(50))
				to_chat(affected_mob, "<span class='danger'>I feel [pick("that you don't have enough mana", "that the winds of magic are gone", "an urge to summon familiar")].</span>")
		...
		if(3)
			...
			if(prob(1)&&prob(50))
				to_chat(affected_mob, "<span class='danger'>I feel [pick("the magic bubbling in your veins","that this location gives you a +1 to INT","an urge to summon familiar")].</span>")
		...
		if(4)
			...
			if(prob(1))
				...
				return
			if(prob(1)&&prob(50))
				to_chat(affected_mob, "<span class='danger'>I feel [pick("the tidal wave of raw power building inside","that this location gives you a +2 to INT and +1 to WIS","an urge to teleport")].</span>")
...
/datum/disease/wizarditis/proc/spawn_wizard_clothes(chance = 0)
	if(ishuman(affected_mob))
		...
		if(prob(chance))
			...
			return
		if(prob(chance))
			if(!istype(H.wear_armor, /obj/item/clothing/suit/wizrobe))
				if(!H.dropItemToGround(H.wear_armor))
					qdel(H.wear_armor)
				H.equip_to_slot_or_del(new /obj/item/clothing/suit/wizrobe(H), SLOT_ARMOR)
			return
		if(prob(chance))
