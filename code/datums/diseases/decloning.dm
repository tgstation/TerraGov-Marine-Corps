...
/datum/disease/decloning/stage_act()
	...
	switch(stage)
		...
		if(3)
			...
			if(prob(2))
				to_chat(affected_mob, "<span class='danger'>My skin feels strange.</span>")
		...
		if(5)
			...
			if(prob(2))
				affected_mob.emote("drool")
			if(prob(5))
				to_chat(affected_mob, "<span class='danger'>My skin starts degrading!</span>")
			if(prob(10))
				affected_mob.adjustCloneLoss(5)
				affected_mob.adjustOrganLoss(ORGAN_SLOT_BRAIN, 2, 170)
			if(affected_mob.cloneloss >= 100)
				affected_mob.visible_message("<span class='danger'>[affected_mob] skin turns to dust!</span>", "<span class'boldwarning'>My skin turns to dust!</span>")
				affected_mob.dust()
