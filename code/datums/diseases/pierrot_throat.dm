...
/datum/disease/pierrot_throat/stage_act()
	...
	switch(stage)
		if(1)
			if(prob(10))
				to_chat(affected_mob, "<span class='danger'>I feel a little silly.</span>")
		if(2)
			if(prob(10))
				to_chat(affected_mob, "<span class='danger'>I start seeing rainbows.</span>")
		if(3)
			if(prob(10))
				to_chat(affected_mob, "<span class='danger'>My thoughts are interrupted by a loud <b>HONK!</b></span>")
		if(4)
			if(prob(5))
