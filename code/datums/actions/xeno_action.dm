...
/datum/action/xeno_action/can_use_action(silent = FALSE, override_flags)
	...
	if(!(flags_to_check & XACT_IGNORE_COOLDOWN) && !action_cooldown_check())
		if(!silent)
			owner.balloon_alert(usr, "We can't use [ability_name] yet, we must wait [cooldown_remaining()] seconds!")
	...
	if(!(flags_to_check & XACT_IGNORE_PLASMA) && X.plasma_stored < plasma_cost)
		if(!silent)
			owner.balloon_alert(usr, "We don't have enough plasma, we need [plasma_cost - X.plasma_stored] more.")
	...
	if(!(flags_to_check & XACT_USE_CLOSEDTURF) && isclosedturf(get_turf(X)))
		if(!silent)
			owner.balloon_alert(usr, "We can't do this while in a solid object!")
