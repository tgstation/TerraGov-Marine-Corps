...
/datum/component/storage/proc/quick_empty(mob/M)
	var/atom/A = parent
	if(!M.canUseStorage() || !A.Adjacent(M) || M.is_mob_incapacitated())
		return
	if(locked)
		to_chat(M, "<span class='warning'>[parent] seems to be locked!</span>")
		return FALSE
...
/datum/component/storage/proc/attackby(datum/source, obj/item/I, mob/M, params)
	if(istype(I, /obj/item/tool/hand_labeler))
		var/obj/item/tool/hand_labeler/labeler = I
		if(labeler.on)
			return FALSE
	. = TRUE //no afterattack
	if(iscyborg(M))
...
/datum/component/storage/proc/mousedrop_receive(datum/source, atom/movable/O, mob/M)
	if(isitem(O))
		...
		if(iscarbon(M))
			var/mob/living/L = M
			if(!L.is_mob_incapacitated() && I == L.get_active_held_item())
...
/datum/component/storage/proc/can_be_inserted(obj/item/I, stop_messages = FALSE, mob/M)
	if(!istype(I) || (I.flags_item & ABSTRACT))
...
/datum/component/storage/proc/remaining_space_items()
	var/atom/real_location = real_location()
	return max(0, max_items - length(real_location.contents))
