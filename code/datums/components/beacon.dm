///This component is used to give items beacon functionality.
...
/datum/component/beacon/proc/activate(atom/movable/source, mob/user)
	...
	message_admins("[ADMIN_TPMONTY(user)] set up a supply beacon.")
	...
	RegisterSignal(source, COMSIG_MOVABLE_Z_CHANGED, PROC_REF(on_beacon_z_change))
...
/datum/component/beacon/proc/on_beacon_z_change(atom/source, old_z, new_z)
	SIGNAL_HANDLER
	beacon_datum.drop_location = source.loc

/datum/component/beacon/proc/on_update_name(atom/source, updates)
	SIGNAL_HANDLER
	if(active)
		source.name += " - [get_area(parent)] - [activator]"
		return
