...
/datum/component/chem_booster/proc/examine(datum/source, mob/user)
	SIGNAL_HANDLER
	to_chat(user, span_notice("The chemical system currently holds [resource_storage_current]u of green blood. Its' enhancement level is set to [boost_amount+1]."))
...
/datum/component/chem_booster/proc/load_up()
	...
	if(!istype(held_item, /obj/item/reagent_containers) && meds_beaker.reagents.total_volume)
		...
		to_chat(wearer, span_notice("The chemical system will <b>[automatic_meds_use ? "inject" : "not inject"]</b> loaded reagets on activation."))
		return
