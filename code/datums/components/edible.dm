...
/datum/component/edible
	...
	var/eat_time = 30
	///Defines how much it lowers someones nutrition (Need to eat, essentialy)
	var/junkiness = 0
...
/* Commenting out TryToEat because Idk if we want the progress bars on eating like on TG -Xander
...
/datum/component/edible/proc/TryToEat(mob/living/eater, mob/living/feeder)
	...
	if(feeder.a_intent != INTENT_HELP) //Can only feed on help intent
		return
	...
	if(eater == feeder)//If you're eating it yourself.
		...
		if(junkiness && eater.nutrition < -150 && eater.nutrition > NUTRITION_LEVEL_STARVING + 50 && !HAS_TRAIT(eater, TRAIT_VORACIOUS))
