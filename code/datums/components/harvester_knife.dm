#define MAX_LOADABLE_REAGENT_AMOUNT_KNIFE 5
#define NO_REAGENT_COLOR "#FFFFFF"

/datum/component/harvester_knife
...
/datum/component/harvester_knife/Initialize(chem_component)
	if(!isitem(parent))
...
/datum/component/harvester_knife/Destroy(force, silent)
	var/obj/item/item_parent = parent
...
/datum/component/harvester_knife/proc/examine(datum/source, mob/user, list/examine_list)
	SIGNAL_HANDLER
	var/output = ""
...
/datum/component/harvester_knife/proc/attackby_async(datum/source, obj/item/cont, mob/user)
	if(user.do_actions)
		return
	...
	if(loaded_reagents[reagent_to_load] > MAX_LOADABLE_REAGENT_AMOUNT_KNIFE)
	...
	var/added_amount = min(container.reagents.total_volume, MAX_LOADABLE_REAGENT_AMOUNT_KNIFE - loaded_reagents[reagent_to_load])
...
/datum/component/harvester_knife/proc/activate_blade_async(datum/source, mob/user)
...
/datum/component/harvester_knife/proc/attack_async(datum/source, mob/living/target, mob/living/user, obj/item/weapon)
...
/datum/component/harvester_knife/proc/attackby(datum/source, obj/item/cont, mob/user)
	SIGNAL_HANDLER
	INVOKE_ASYNC(src, .proc/attackby_async, source, cont, user)

///Signal handler calling activation of the harvester
/datum/component/harvester_knife/proc/activate_blade(datum/source, mob/user)
	SIGNAL_HANDLER
	INVOKE_ASYNC(src, .proc/activate_blade_async, source, user)

///Signal handler calling when user attacks
/datum/component/harvester_knife/proc/attack(datum/source, mob/living/target, mob/living/user, obj/item/weapon)
	SIGNAL_HANDLER
	. = FALSE
...
#undef MAX_LOADABLE_REAGENT_AMOUNT_KNIFE

/datum/component/harvester_knife/proc/select_reagent(datum/source)
...
/datum/component/harvester_knife/proc/update_selected_reagent(datum/reagent/new_reagent)
...
/datum/action/harvester/reagent_select
	name = "Select Reagent"
	image/selected_reagent_overlay
