...
/datum/item_representation/uniform_representation/instantiate_object(datum/loadout_seller/seller, master = null, datum/loadout/loadout = null)
	. = ..()
	if(!.)
		return
	tie?.install_on_uniform(seller, ., loadout)
...
/datum/item_representation/tie/instantiate_object(datum/loadout_seller/seller, master = null, datum/loadout/loadout = null)
	. = ..()
	if(!.)
		return
	var/obj/item/clothing/tie/storage/tie = .
	tie.hold = hold.instantiate_object(seller, tie, loadout)

///Attach the tie to a uniform
/datum/item_representation/tie/proc/install_on_uniform(datum/loadout_seller/seller, obj/item/clothing/under/uniform, datum/loadout/loadout)
	var/obj/item/clothing/tie/storage/tie = instantiate_object(seller, null, loadout)
