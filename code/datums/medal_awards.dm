 ...
 * - mob/user - The mob awarding the medal.
 * - client/medal_awardee - The client, if found/available, being awarded the medal.
...
/proc/award_medal_to(
	...
	for(var/datum/data/record/record as anything in GLOB.datacore.general)
		if(record.fields["name"] == awardee_real_name)
			awardee_record = record
			continue
		if(record.fields["name"] == issuer.real_name)
	...
	if(!is_posthumous && !isnull(medal_awardee))
		var/datum/medal_persistence/medals = get_medal_persistence_for_ckey(medal_awardee.ckey)
	...
	if(!(awardee_real_name in GLOB.medal_awards))
		...
		GLOB.medal_awards[awardee_real_name] = new_award
