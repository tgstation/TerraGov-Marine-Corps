...
/datum/element/footstep/proc/prepare_step(mob/living/source)
	...
	if(source.buckled || source.throwing || source.is_ventcrawling || source.lying_angle || CHECK_MULTIPLE_BITFIELDS(source.allow_pass_flags, HOVERING) || HAS_TRAIT(source, TRAIT_IMMOBILE))
		return
