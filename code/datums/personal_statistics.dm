...
/datum/personal_statistics
	...
	var/engine_repairs_performed = 0
	var/miner_repairs_performed = 0

	var/engine_sabotages_performed = 0
...
/datum/personal_statistics/proc/compose_report()
	...
	if(melee_hits)
		...
		stats += "<br>"
	stats += friendly_fire_damage ? "You caused [friendly_fire_damage] damage to allies..." : "You avoided committing acts of friendly fire!"
	stats += "<br>"
	if(grenades_primed)
	...
	if(grenades_primed || traps_created)
		stats += "<br>"
	stats += "You lost [limbs_lost] limb[limbs_lost > 1 ? "s" : ""]."
	if(delimbs)
	...
	if(internal_injuries_inflicted)
		stats += "You inflicted [internal_injuries_inflicted] internal injur[internal_injuries_inflicted > 1 ? "ies" : "y"] on [internal_injuries_inflicted > 1 ? "others" : "somebody"]."
	...
	if(heals)
		stats += "You healed others [heals] time[heals > 1 ? "s" : ""]."
	if(surgical_actions_performed)
		stats += "You performed [surgical_actions_performed] surgical operation[surgical_actions_performed > 1 ? "s" : ""]."
	if(revives)
		stats += "You performed [revives] revive[revives > 1 ? "s" : ""]."
	stats += "<br>"
	if(times_revived)
	...
	if(time_sleeping)
		downtime_stats += "You slept for [time_sleeping] minute[time_sleeping MINUTES != 1 MINUTES ? "s" : ""]."
	if(time_in_stasis)
		downtime_stats += "You spent [time_in_stasis] minute[time_in_stasis MINUTES != 1 MINUTES ? "s" : ""] in stasis."
	if(time_in_cryo)
		downtime_stats += "You spent [time_in_cryo] minute[time_in_cryo MINUTES != 1 MINUTES ? "s" : ""] in cryo."

	if(downtime_stats)
		stats += "<hr>" + downtime_stats

	//Support & logistics
	var/list/support_stats = list()
	if(weeds_planted)
		support_stats += "You planted [weeds_planted] weed node[weeds_planted > 1 ? "s" : ""]."
	if(structures_built)
		support_stats += "You built [structures_built] structure[structures_built > 1 ? "s" : ""]."
	if(times_repaired)
		support_stats += "You repaired others [times_repaired] time[times_repaired > 1 ? "s" : ""]."
	if(integrity_repaired)
		support_stats += "You repaired [integrity_repaired] point[integrity_repaired > 1 ? "s" : ""] of integrity."
	if(engine_repairs_performed)
		support_stats += "You performed [engine_repairs_performed] engine repair[engine_repairs_performed > 1 ? "s" : ""]."
	if(miner_repairs_performed)
		support_stats += "You performed [miner_repairs_performed] miner repair[miner_repairs_performed > 1 ? "s" : ""]."

	if(engine_sabotages_performed)
		support_stats += "You sabotaged [engine_sabotages_performed] engine[engine_sabotages_performed > 1 ? "s" : ""]."
	if(miner_sabotages_performed)
		support_stats += "You sabotaged [miner_sabotages_performed] miner[miner_sabotages_performed > 1 ? "s" : ""]."
	if(apcs_slashed)
		support_stats += "You slashed [apcs_slashed] APC[apcs_slashed > 1 ? "s" : ""]."

	if(artillery_fired)
		support_stats += "You fired [artillery_fired] artillery shell[artillery_fired > 1 ? "s" : ""]."

	if(req_points_used)
		support_stats += "You used [req_points_used] requisition point[req_points_used > 1 ? "s" : ""]."

	if(support_stats)
		stats += "<hr>" + support_stats

	//Close air support
	var/list/cas_stats = list()
	if(cas_cannon_shots)
		support_stats += "You shot [cas_cannon_shots] GAU-21 cannon voll[cas_cannon_shots > 1 ? "ies" : "ey"]."
	if(cas_laser_shots)
	...
	if(cas_minirockets_fired)
		support_stats += "You launched [cas_minirockets_fired] air-to-surface mini-rocket[cas_minirockets_fired > 1 ? "s" : ""]."
	if(cas_rockets_fired)
		support_stats += "You launched [cas_rockets_fired] air-to-surface laser-guide rocket[cas_rockets_fired > 1 ? "s" : ""]."
	if(cas_points_used)
		support_stats += "You used [cas_points_used] dropship fabricator point[cas_points_used > 1 ? "s" : ""]."
	if(cas_stats)
		stats += "<hr>" + cas_stats

	//The funnies

	return jointext(stats, "<br>")
