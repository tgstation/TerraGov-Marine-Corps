...
/datum/status_effect/incapacitating/sleeping/tick()
    ...
    if((locate(/obj/structure/bed) in owner.loc))
        ...
        if(locate(/obj/item/bedsheet/bedsheet) in owner.loc) && (locate(/obj/item/toy/plush) in owner.loc) // plushie bonus in bed with a blanket
            healing += BASE_HEAL_RATE
    else if((locate(/obj/structure/table) in owner.loc))
        healing += BASE_HEAL_RATE
    if(locate(/obj/item/bedsheet/bedsheet) in owner.loc))
        healing += BASE_HEAL_RATE
