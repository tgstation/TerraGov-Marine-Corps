...
// *********** Wrath Handler
// ***************************************
#define WRATH_HANDLER_DECAY_MULTIPLIER 1.2
#define WRATH_HANDLER_ACTIVE_DECAY_MULTIPLIER 0.05
#define WRATH_HANDLER_GAIN_MULTIPLIER 0.8

/datum/status_effect/wrath_handler
	id = "wrath_handler"
	tick_interval = 2 SECONDS
	/// The xenomorph owner of this status effect.
	...
	var/decay_time = 20 SECONDS
	/// Base amount of Wrath lost every valid tick.
	var/decay_amount = 10

/datum/status_effect/wrath_handler/on_creation(mob/living/new_owner, new_action)
	. = ..()
	owner = new_owner
	if(!new_action)
		qdel(src)
	...
	primal_wrath_action = new_action
...
/datum/status_effect/wrath_handler/on_remove()
	. = ..()
	UnregisterSignal(xeno_owner, list(COMSIG_XENOMORPH_BRUTE_DAMAGE, COMSIG_XENOMORPH_BURN_DAMAGE, COMSIG_MOB_DEATH))

/datum/status_effect/wrath_handler/tick()
	. = ..()
	if(xeno_owner.wrath_stored <= 0)
		return
	if(primal_wrath_action.ability_active)
		message_admins("wrath_handler/tick(): primal wrath was active")
		xeno_owner.wrath_stored -= xeno_owner.xeno_caste.wrath_max * WRATH_HANDLER_ACTIVE_DECAY_MULTIPLIER
		return
	if(decay_time > 0)
		message_admins("wrath_handler/tick(): decay_time was greater than 0 ([decay_time])")
		decay_time -= initial(tick_interval)
		return
	xeno_owner.wrath_stored = max(0, xeno_owner.wrath_stored - decay_amount)
	decay_amount = round(decay_amount * WRATH_HANDLER_DECAY_MULTIPLIER)

/// Removes the debuff if the owner dies.
/datum/status_effect/wrath_handler/proc/owner_death(datum/source)
	SIGNAL_HANDLER
	xeno_owner.remove_status_effect(STATUS_EFFECT_XENO_WRATH_HANDLER)

/// Resets decay, and gives the owner an amount of Wrath proportional to the damage taken.
/datum/status_effect/wrath_handler/proc/taking_damage(datum/source, amount, list/amount_mod)
	SIGNAL_HANDLER
	if(amount <= 0 || primal_wrath_action.ability_active)
		return
	decay_time = initial(decay_time)
	decay_amount = initial(decay_amount)
	xeno_owner.wrath_stored += round(amount * WRATH_HANDLER_GAIN_MULTIPLIER)

/// Increases the owner's damage based on a multiplier.
/// - multiplier: The multiplier applied to the owner's damage.
/datum/status_effect/wrath_handler/proc/toggle_damage_increase(multiplier)
	if(xeno_owner.xeno_melee_damage_modifier != initial(xeno_owner.xeno_melee_damage_modifier))
		xeno_owner.xeno_melee_damage_modifier = initial(xeno_owner.xeno_melee_damage_modifier)
		return
	xeno_owner.xeno_melee_damage_modifier = multiplier
