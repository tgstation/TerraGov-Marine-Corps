...
/datum/emergency_call/freelancers/print_backstory(mob/living/carbon/human/H)
	...
	to_chat(H, "")
	to_chat(H, "<B>Today, a NTC vessel, [SSmapping.configs[SHIP_MAP].map_name], has sent out a distress signal on the orbit of [SSmapping.configs[GROUND_MAP].map_name]. You hope you can come back alive to get your pay!</b>")
...
/datum/emergency_call/freelancers/create_member(datum/mind/M)
	...
	if(!leader)
		...
		to_chat(H, "<p style='font-size:1.5em'><span class='notice'>You are the Freelancer mercenary assigned to lead this group in responding to the NTC distress signal sent nearby. Keep your team in one piece to make sure they earn their payment!</notice></p>")
		return
	...
	if(medics < max_medics)
		...
		H.apply_assigned_role_to_spawn(J)
		medics++
		to_chat(H, "<p style='font-size:1.5em'><span class='notice'>You are a Freelancer mercenary medic assigned to this group to respond to the NTC distress signal sent nearby. Do not let your teammates fall in battle!</notice></p>")
		return
	...
	H.apply_assigned_role_to_spawn(J)
	to_chat(H, "<p style='font-size:1.5em'><span class='notice'>You are a Freelancer mercenary assigned to this group to respond to the NTC distress signal sent nearby. Don't let you and your team's guard down!</notice></p>")
