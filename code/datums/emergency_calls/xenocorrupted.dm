...
/datum/emergency_call/xenomorphs_corrupted/print_backstory(mob/living/carbon/xenomorph/X)
	...
	to_chat(X, "<B>Help the new Queen take over this sector. For the corrupted Hive!</b>")
...
/datum/emergency_call/xenomorphs_corrupted/create_member(datum/mind/M)
	...
	if(!leader)
		. = new /mob/living/carbon/xenomorph/ravager/Corrupted(spawn_loc, TRUE) //TRUE for the can_spawn_in_centcomm, so they don't get sent to a different hive.
		leader = .
		M.transfer_to(., TRUE)
		print_backstory(.)
		return
