...
/datum/emergency_call/upp/print_backstory(mob/living/carbon/human/H)
	...
	to_chat(H, "")
	to_chat(H, "<B>Today, a NTC vessel, [SSmapping.configs[SHIP_MAP].map_name], has sent out a distress signal on the orbit of [SSmapping.configs[GROUND_MAP].map_name]. Your USL assault pirate warband heads out and your stealing begins!</b>")
...
/datum/emergency_call/upp/create_member(datum/mind/M)
	...
	if(!leader)
		...
		to_chat(H, "<p style='font-size:1.5em'>[span_notice("You are the leader of the USL pirate band in responding to the NTC distress signal sent nearby. Let your squadmates march to battle, for the USL!")]</p>")
		return

	if(medics < max_medics)
		var/datum/job/J = SSjob.GetJobType(/datum/job/upp/medic)
		H.apply_assigned_role_to_spawn(J)
		to_chat(H, "<p style='font-size:1.5em'>[span_notice("You are a medic of the USL pirate band to respond to the NTC distress signal sent nearby. Kit up and get ready to tend wounds!")]</p>")
		medics++
		return
	...
	if(prob(20))
		...
		H.apply_assigned_role_to_spawn(J)
		to_chat(H, "<p style='font-size:1.5em'>[span_notice("You are a specialist of the USL pirate band to respond to the NTC distress signal sent nearby. Crush the vermin!")]</p>")
		return

	var/datum/job/J = SSjob.GetJobType(/datum/job/upp/standard)
	H.apply_assigned_role_to_spawn(J)
	to_chat(H, "<p style='font-size:1.5em'>[span_notice("You are a member of the USL pirate band to respond to the NTC distress signal sent nearby. Do not forget your training, stand tall with your other pirates!")]</p>")
