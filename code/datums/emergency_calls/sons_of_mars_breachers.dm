...
/datum/emergency_call/som_breachers/create_member(datum/mind/M)
	...
	if(medics < max_medics)
		...
		H.apply_assigned_role_to_spawn(selected_job)
		to_chat(H, "<p style='font-size:1.5em'>[span_notice("You are a Sons of Mars medic assigned to this elite breaching team, sent to response to the NTC distress signal sent out nearby. Keep your squad alive in this fight!")]</p>")
		medics++
		return
	if(max_specialists > 0)
		selected_job = SSjob.GetJobType(/datum/job/som/ert/breacher/specialist)
		H.apply_assigned_role_to_spawn(selected_job)
		to_chat(H, "<p style='font-size:1.5em'>[span_notice("You are a veteran of the Sons of Mars trusted with specialised weaponry. You are assigned to this elite breaching team, sent to response to the NTC distress signal sent out nearby. Do them proud and kill all who stand in your way!")]</p>")
		max_specialists --
		return
