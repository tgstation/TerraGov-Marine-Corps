...
/datum/emergency_call/pmc/create_member(datum/mind/M)
	...
	if(!leader)
		...
		to_chat(H, "<p style='font-size:1.5em'>[span_notice("You are the leader of this private military contractor team in responding to the NTC distress signal sent out nearby. Address the situation and get your team to safety!")]</p>")
		return

	if(smartgunners < max_smartgunners)
		var/datum/job/J = SSjob.GetJobType(/datum/job/pmc/gunner)
		H.apply_assigned_role_to_spawn(J)
		to_chat(H, "<p style='font-size:1.5em'>[span_notice("You are a PMC heavy gunner assigned to this team to respond to the NTC distress signal sent out nearby. Be the back guard of your squad!")]</p>")
		smartgunners++
		return
	...
	if(prob(30))
		...
		H.apply_assigned_role_to_spawn(J)
		to_chat(H, "<p style='font-size:1.5em'>[span_notice("You are a PMC combat medic assigned to this team to respond to the NTC distress signal sent out nearby. Support your squad with bleeding edge treatments!")]</p>")
		return

	var/datum/job/J = SSjob.GetJobType(/datum/job/pmc/standard)
	H.apply_assigned_role_to_spawn(J)
	to_chat(H, "<p style='font-size:1.5em'>[span_notice("You are a private military contractor assigned to this team to respond to the NTC distress signal sent out nearby. Assist your team and protect NT's interests whenever possible!")]</p>")
