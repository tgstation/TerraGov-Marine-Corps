...
/datum/emergency_call/xenomorphs_alpha/print_backstory(mob/living/carbon/xenomorph/X)
	...
	to_chat(X, "<B>Help the new Queen take over this sector. For the Alpha Hive!</b>")
...
/datum/emergency_call/xenomorphs_alpha/create_member(datum/mind/M)
	...
	if(!leader)
		. = new /mob/living/carbon/xenomorph/ravager/Alpha(spawn_loc, TRUE) //TRUE for the can_spawn_in_centcomm, so they don't get sent to a different hive.
		leader = .
		M.transfer_to(., TRUE)
		print_backstory(.)
		return
	...
	if(prob(35))
		. = new /mob/living/carbon/xenomorph/drone/elder/Alpha(spawn_loc, TRUE)
		M.transfer_to(., TRUE)
		print_backstory(.)
		return

	if(prob(35))
		. = new /mob/living/carbon/xenomorph/spitter/mature/Alpha(spawn_loc, TRUE)
		M.transfer_to(., TRUE)
		print_backstory(.)
		return

	. = new /mob/living/carbon/xenomorph/hunter/mature/Alpha(spawn_loc, TRUE)
	M.transfer_to(., TRUE)
	print_backstory(.)
