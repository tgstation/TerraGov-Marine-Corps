...
/datum/emergency_call/xenomorphs_zeta/print_backstory(mob/living/carbon/xenomorph/X)
	to_chat(X, "<B>You are Xenomorph from the Zeta hive.</b>")
	...
	to_chat(X, "<B>Expand your hive and destroy any who dare to oppose your influence especially other hives. For the Zeta Hive!</b>")
...
/datum/emergency_call/xenomorphs_zeta/create_member(datum/mind/M)
	...
	if(!leader)
		. = new /mob/living/carbon/xenomorph/ravager/Zeta(spawn_loc, TRUE) //TRUE for the can_spawn_in_centcomm, so they don't get sent to a different hive.
		leader = .
		M.transfer_to(., TRUE)
		print_backstory(.)
		return
