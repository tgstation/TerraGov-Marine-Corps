...
/datum/game_mode/infestation/psy_sensors
	...
	valid_job_types = list(
		/datum/job/terragov/squad/engineer/uscm = 4,
		/datum/job/terragov/squad/corpsman/uscm = 8,
		/datum/job/terragov/squad/smartgunner/uscm = 4,
		/datum/job/terragov/squad/leader/uscm = 4,
		/datum/job/terragov/squad/standard/uscm = -1,
	...
	blacklist_ground_maps = list(MAP_GELIDA_IV,MAP_CHIGUSA,MAP_MAGMOOR_DIGSITE,MAP_DELTA_STATION, MAP_WHISKEY_OUTPOST, MAP_OSCAR_OUTPOST, MAP_BIG_RED, MAP_ICE_COLONY, MAP_ICY_CAVES, MAP_LV_624, MAP_RESEARCH_OUTPOST, MAP_PRISON_STATION)
	whitelist_ground_maps = list(MAP_PATRICKS_REST)

/datum/game_mode/infestation/psy_sensors/post_setup()
	. = ..()
	for(var/turf/T AS in GLOB.psy_towers)
		new /obj/structure/sensor_tower_patrol/psy(T)

/datum/game_mode/infestation/psy_sensors/setup_blockers()
	. = ..()
	//Starts the round timer when the game starts proper
	var/datum/game_mode/infestation/psy_sensors/D = SSticker.mode
	addtimer(CALLBACK(D, TYPE_PROC_REF(/datum/game_mode/infestation/psy_sensors, set_game_timer)), SSticker.round_start_time + shutters_drop_time)
	addtimer(CALLBACK(D, TYPE_PROC_REF(/datum/game_mode/infestation/psy_sensors, respawn_wave)), SSticker.round_start_time + shutters_drop_time) //starts wave respawn on shutter drop and begins timer
	addtimer(CALLBACK(D, TYPE_PROC_REF(/datum/game_mode/infestation/psy_sensors, intro_sequence)), SSticker.round_start_time + shutters_drop_time - 50 SECONDS) //starts intro sequence 10 seconds before shutter drop

/datum/game_mode/infestation/psy_sensors/announce()
	to_chat(world, "<b>The current game mode is - Psychic Sensors!</b>")
	to_chat(world, "<b>Xeno</b>")

///plays the intro sequence
/datum/game_mode/infestation/psy_sensors/proc/intro_sequence()
	Cinematic(CINEMATIC_BRIEFING, world)

/datum/game_mode/infestation/psy_sensors/proc/respawn_wave()
	var/datum/game_mode/infestation/psy_sensors/D = SSticker.mode
	D.wave_timer = addtimer(CALLBACK(D, TYPE_PROC_REF(/datum/game_mode/infestation/psy_sensors, respawn_wave)), wave_timer_length, TIMER_STOPPABLE)
...
///round timer
/datum/game_mode/infestation/psy_sensors/proc/set_game_timer()
	...
	D.game_timer = addtimer(CALLBACK(D, TYPE_PROC_REF(/datum/game_mode/infestation/psy_sensors, set_game_end)), max_game_time, TIMER_STOPPABLE)

/datum/game_mode/infestation/psy_sensors/game_end_countdown()
	if(!game_timer)
		return
...
/datum/game_mode/infestation/psy_sensors/proc/set_game_end()
	max_time_reached = TRUE

//End game checks
/datum/game_mode/infestation/psy_sensors/check_finished()
	if(round_finished)
		return TRUE
