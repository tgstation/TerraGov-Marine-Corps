...
/datum/game_mode/distress/mode_new_player_panel(mob/new_player/NP)
	...
	output += "<i>You are part of the <b>TerraGov Marine Corps</b>, a military branch of the TerraGov council. Their mission is to enforce space law for the purpose of defending Earth's orbit as well as other solar colonies around the galaxy under the conflict of the Independent Colonial Confederation and the intelligent xenomorph threat. \nThe TGMC is composed of willing men and women all across the TerraGov systems, from all races and quirks. \nAs the vessel approaches the ordered location on the map, the cryostasis pods deactivate and awake you from your long-term stasis. Knowing that it's one of those days again, you hope that you'll make this out alive...</i>"
	output +="<hr>"
	output += "<p><a href='byond://?src=[REF(NP)];lobby_choice=show_preferences'>Setup Character</A> | <a href='byond://?src=[REF(NP)];lobby_choice=observe'>Observe</A></p>"
	output +="<hr>"
	...
	if(SSticker.current_state <= GAME_STATE_PREGAME)
		...
		output += "<p><a href='byond://?src=[REF(NP)];lobby_choice=late_join'>Join the TGMC!</A> | <a href='byond://?src=[REF(NP)];lobby_choice=late_join_xeno'>Join the Hive!</A></p>"

	output += append_player_votes_link(NP)

	output += "</div>"

	var/datum/browser/popup = new(NP, "playersetup", "<div align='center'>Welcome to TGMC[SSmapping?.configs ? "- [SSmapping.configs[SHIP_MAP].map_name]" : ""]</div>", 560, 325)
