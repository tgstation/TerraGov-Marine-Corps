...
/datum/game_mode/campaign/scale_roles()
	. = ..()
	if(!.)
		return
...
/datum/game_mode/campaign/announce()
...
/datum/game_mode/campaign/set_valid_squads()
...
/datum/game_mode/campaign/get_joinable_factions(should_look_balance)
...
/datum/game_mode/campaign/setup_blockers()
	. = ..()
	//Starts the round timer when the game starts proper
	var/datum/game_mode/campaign/D = SSticker.mode
	addtimer(CALLBACK(D, TYPE_PROC_REF(/datum/game_mode/campaign, set_game_timer)), SSticker.round_start_time + shutters_drop_time + game_timer_delay) //game cannot end until at least 5 minutes after shutter drop
	addtimer(CALLBACK(D, TYPE_PROC_REF(/datum/game_mode/campaign, respawn_wave)), SSticker.round_start_time + shutters_drop_time) //starts wave respawn on shutter drop and begins timer
	addtimer(CALLBACK(D, TYPE_PROC_REF(/datum/game_mode/campaign, intro_sequence)), SSticker.round_start_time + shutters_drop_time - 10 SECONDS) //starts intro sequence 10 seconds before shutter drop
...
/datum/game_mode/campaign/proc/intro_sequence()
...
///round timer
/datum/game_mode/campaign/proc/set_game_timer()
	...
	D.game_timer = addtimer(CALLBACK(D, TYPE_PROC_REF(/datum/game_mode/campaign, set_game_end)), max_game_time, TIMER_STOPPABLE)

/datum/game_mode/campaign/game_end_countdown()
	if(!game_timer)
		return
...
/datum/game_mode/campaign/wave_countdown()
	if(!wave_timer)
		return
...
/datum/game_mode/campaign/proc/set_game_end()
	max_time_reached = TRUE

/datum/game_mode/campaign/process()
	if(round_finished)
		return PROCESS_KILL
...
/datum/game_mode/campaign/proc/announce_bioscans_marine_som(show_locations = TRUE, delta = 2, announce_marines = TRUE, announce_som = TRUE)
...
/datum/game_mode/campaign/proc/respawn_wave()
	var/datum/game_mode/campaign/D = SSticker.mode
	D.wave_timer = addtimer(CALLBACK(D, TYPE_PROC_REF(/datum/game_mode/campaign, respawn_wave)), wave_timer_length, TIMER_STOPPABLE)
...
/datum/game_mode/campaign/proc/count_humans(list/z_levels = SSmapping.levels_by_trait(ZTRAIT_GROUND), count_flags)
...
//End game checks
/datum/game_mode/campaign/check_finished()
	if(round_finished)
		return TRUE
...
/datum/game_mode/campaign/declare_completion()
	. = ..()
...
/datum/game_mode/campaign/announce_round_stats()
