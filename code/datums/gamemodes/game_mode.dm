...
/datum/game_mode/proc/announce_round_stats()
	...
	if(GLOB.round_statistics.total_human_deaths)
		dat += "[GLOB.round_statistics.total_human_deaths] people were killed, among which [GLOB.round_statistics.total_human_revives] were revived or respawned. For a [(GLOB.round_statistics.total_human_revives / max(GLOB.round_statistics.total_human_deaths, 1)) * 100]% human revival rate."
