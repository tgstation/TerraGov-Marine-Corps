...
/turf/AICtrlShiftClick(mob/living/silicon/ai/user)
	...
	if(A.ceiling > CEILING_OBSTRUCTED)
		...
		return
	if((SSmarine_main_ship.rail_gun?.last_firing_ai + COOLDOWN_RAILGUN_FIRE) > world.time)
	...
	while(laser)
		...
		if(length(possible_zombies))
			...
			SSmarine_main_ship.rail_gun?.fire_rail_gun(nuked_zombie, user, TRUE, TRUE)
			++timesfired
		else if(length(possible_xenos))
			var/mob/living/carbon/xenomorph/nuked_xeno = pick(possible_xenos)
			SSmarine_main_ship.rail_gun?.fire_rail_gun(nuked_xeno, user, TRUE, TRUE)
			++timesfired
			...
			while(possible_humans)
				...
				if(length(possible_humans))
					...
					SSmarine_main_ship.rail_gun?.fire_rail_gun(targetturf, user, TRUE, TRUE)
					++timesfired
					break
		else
			SSmarine_main_ship.rail_gun?.fire_rail_gun(laser, user, TRUE, TRUE)
			++timesfired
