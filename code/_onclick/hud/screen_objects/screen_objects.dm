...
/obj/screen/ammo/proc/add_hud(mob/living/user, obj/item/weapon/gun/G)
	...
	if((user.get_active_held_item() != G && user.get_inactive_held_item() != G && !CHECK_BITFIELD(G.flags_item, IS_DEPLOYED)) || !G.hud_enabled || !CHECK_BITFIELD(G.flags_gun_innate_features, GUN_INNATE_AMMO_COUNTER))
		return
...
/obj/screen/ammo/proc/update_hud(mob/living/user, obj/item/weapon/gun/G)
	...
	if(!G || !(G.flags_gun_innate_features & GUN_INNATE_AMMO_COUNTER) || !G.hud_enabled || !G.get_ammo_type() || isnull(G.get_ammo_count()))
		remove_hud(user)
		return
