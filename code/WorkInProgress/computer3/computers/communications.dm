		...
		if("MessageCentcomm" in href_list)
			...
			if(authenticated==2)
				if(centcomm_message_cooldown)
					...
					return
				var/input = stripped_input(usr, "Please choose a message to transmit to USCM via quantum entanglement.  Please be aware that this process is very expensive, and abuse will lead to... termination.  Transmission does not guarantee a response.", "To abort, send an empty message.", "")
				if(!input || !interactable())
					return
				Centcomm_announce(input, usr)
				usr << "Message transmitted."
				log_say("[key_name(usr)] has made a USCM announcement: [input]")
		...
		if (authenticated)
			...
			if (authenticated==2)
				...
				if(computer.emagged == 0)
					dat += "<BR>\[ <A HREF='?src=\ref[src];MessageCentcomm'>Send an emergency message to USCM</A> \]"
				else
