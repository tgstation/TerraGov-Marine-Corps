...
/obj/machinery/lapvend/proc/choose_progs(var/obj/item/weapon/card/id/C)
	if(access_sulaco_brig in C.access)
	...
	if(access_sulaco_engineering in C.access)
	...
	if(access_sulaco_bridge in C.access)
		newlap.spawn_files += (/datum/file/program/communications)
	if((access_sulaco_medbay in C.access))
	...
	if(access_sulaco_research in C.access)
	...
	if(access_centcomm in C.access)
