...
/datum/controller/subsystem/monitor/proc/balance_xeno_team()
	var/datum/job/xeno_job = SSjob.GetJobType(/datum/job/xenomorph)
	if(current_state >= STATE_BALANCED || ((xeno_job.total_positions - xeno_job.current_positions) <= (length_char(GLOB.alive_xeno_list) * TOO_MUCH_BURROWED_PROPORTION)) || length_char(GLOB.xeno_resin_silos_by_hive[XENO_HIVE_NORMAL]) == 0)
		return 1
