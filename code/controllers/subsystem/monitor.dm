...
/datum/controller/subsystem/monitor/fire(resumed = 0)
	var/total_living_players = length(GLOB.alive_human_list_faction[FACTION_NTC]) + length(GLOB.alive_xeno_list_hive[XENO_HIVE_NORMAL])
...
/datum/controller/subsystem/monitor/proc/calculate_state_points()
	...
	switch(gamestate)
		if(GROUNDSIDE)
			...
			. += (length(GLOB.alive_human_list_faction[FACTION_NTC]) - human_on_ground) * HUMAN_LIFE_ON_SHIP_WEIGHT
			...
			. += SSpoints.supply_points[FACTION_NTC] * REQ_POINTS_WEIGHT
		if(SHUTTERS_CLOSED)
			. += length(GLOB.alive_human_list_faction[FACTION_NTC]) * HUMAN_LIFE_WEIGHT_PREGAME
			. += length(GLOB.alive_xeno_list_hive[XENO_HIVE_NORMAL]) * XENOS_LIFE_WEIGHT_PREGAME
		if(SHIPSIDE)
			. += length(GLOB.alive_human_list_faction[FACTION_NTC]) * HUMAN_LIFE_WEIGHT_SHIPSIDE
...
/datum/controller/subsystem/monitor/proc/process_human_positions()
	...
	human_in_FOB = 0
	human_on_ship = 0
	for(var/human in GLOB.alive_human_list_faction[FACTION_NTC])
