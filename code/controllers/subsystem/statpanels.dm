...
/datum/controller/subsystem/statpanels/fire(resumed = FALSE)
	if (!resumed)
		...
		var/list/global_data = list(
			...
			"Current Map: [length(SSmapping.configs) ? SSmapping.configs[GROUND_MAP].map_name : "Loading..."]",
	...
	while(length(currentrun))
		...
		if(target.stat_tab == "Status") // fire every other fire (buttery smooth (it's not that intensive))
