...
#define FOLLOW_LINK(observer, target) "<a href=byond://?_src_=usr;track=[REF(target)]>(F)</a>"
#define TURF_LINK(observer, turfy) "<a href=byond://?_src_=usr;jump=1;x=[turfy.x];y=[turfy.y];z=[turfy.z]>(T)</a>"
#define FOLLOW_OR_TURF_LINK(observer, target, turfy) "<a href=byond://?_src_=usr;track=[REF(target)];jump=1;x=[turfy.x];y=[turfy.y];z=[turfy.z]>(F)</a>"
