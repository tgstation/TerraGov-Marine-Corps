...
/datum/hallucination/xeno_attack/New(mob/living/carbon/C, forced = TRUE)
	...
	if(pump)
		...
		sleep(1 SECONDS)
		xeno.balloon_alert(xeno.name, "[xeno.name] begins climbing into the ventilation system")
		sleep(1.5 SECONDS)
		xeno.balloon_alert(xeno.name, "[xeno.name] scrambles into the ventilation ducts")
		playsound(src, get_sfx("alien_ventpass"), 35, 1)
		qdel(xeno)
	qdel(src)
