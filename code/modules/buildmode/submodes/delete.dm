...
/datum/buildmode_mode/delete/handle_click(client/c, params, object)
	...
	if(LAZYACCESS(modifiers, RIGHT_CLICK))
		if(check_rights(R_DEBUG|R_SERVER))	//Prevents buildmoded non-admins from breaking everything.
			...
			if(tgui_alert(usr, "Are you really sure you want to delete all instances of type [deleting.type]?",,"Yes","No") != "Yes")
				return

			if(tgui_alert(usr, "Second confirmation required. Delete?",,"Yes","No") != "Yes")
				return
