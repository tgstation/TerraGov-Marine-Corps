	...
	switch(act)
		...
		if("roar")
			if(!muzzled)
				...
				if(caste == "Predalien")
					playsound(loc, 'sound/voice/predalien_roar.ogg', 75, 1, 1)
				else
					if(mob_size != MOB_SIZE_BIG)
						playsound(loc, 'sound/voice/alien_roar_small.ogg', 25, 1, 1)
					else
						playsound(loc, 'sound/voice/alien_roar_large.ogg', 50, 1, 1)
		if("growl")
			if(!muzzled)
				m_type = 2
