...
/obj/structure/mineral_door/resin/attack_alien(mob/living/carbon/Xenomorph/M)
	...
	if(do_after(M, 80, FALSE, src, USER_ICON_HOSTILE))
...
/obj/structure/tunnel/attack_alien(mob/living/carbon/Xenomorph/M)
	...
	if(M.a_intent == INTENT_HARM && M == creator)
		...
		if(do_after(M, HIVELORD_TUNNEL_DISMANTLE_TIME, FALSE, src, USER_ICON_BUILD))
			health = 0
			healthcheck()
		return
	...
	if(do_after(M, tunnel_time, FALSE, src, USER_ICON_GENERIC))
