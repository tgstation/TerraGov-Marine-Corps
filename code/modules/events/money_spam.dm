	...
	if(useMS)
		if(prob(5))
			...
			switch(pick(1,2,3,4,5,6,7))
				...
				if(6)
					sender = pick("Weyland-Yutani Morale Divison","Feeling Lonely?","Bored?","www.wetskrell.nt")
					message = pick("The Weyland-Yutani Morale Division wishes to provide you with quality entertainment sites.",\
					...
					"Wetskrell.nt only provides the higest quality of male entertaiment to Weyland-Yutani Employees.",\
					"Simply enter your Weyland-Yutani Bank account system number and pin. With three easy steps this service could be yours!")
				if(7)
