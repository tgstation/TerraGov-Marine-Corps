...
/obj/machinery/coffeemaker/attackby(obj/item/attack_item, mob/living/user, params)
	...
	if(!coffeepot && isscrewdriver(attack_item))
		return FALSE

	if(iscrowbar(attack_item))
		return
	...
	if (istype(attack_item, /obj/item/reagent_containers/cup/glass/coffee_cup) && attack_item.is_open_container())
