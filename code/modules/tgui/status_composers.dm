...
/proc/ui_status_user_is_adjacent(mob/user, atom/source, viewcheck = TRUE)
	if (isliving(user))
		var/mob/living/living_user = user
		return living_user.shared_living_ui_distance(source, viewcheck = viewcheck)
	else
		return UI_UPDATE
...
/proc/ui_status_only_living(mob/user, source)
	...
	if(isobserver(user))
		...
		if(client)
			var/clientviewlist = getviewsize(client.view_size.get_client_view_size())
...
/proc/ui_status_user_is_advanced_tool_user(mob/user)
	return ishuman(user) ? UI_INTERACTIVE : UI_UPDATE //note unlkike tg we just use ishuman insteaed of a trait lol
...
/proc/ui_status_user_is_conscious_and_lying_down(mob/user)
	...
	var/mob/living/living_user = user
	return (living_user.lying_angle && living_user.stat == CONSCIOUS) \
		? UI_INTERACTIVE \
		: UI_UPDATE
