 ...
 */
/proc/tgui_alert(mob/user, message, title, list/buttons, timeout = 60 SECONDS, autofocus = TRUE)
	if (!user)
		user = usr
	if (!istype(user))
 ...
 * * autofocus - The bool that controls if this alert should grab window focus.
 */
/proc/tgui_alert_async(mob/user, message, title, list/buttons, datum/callback/callback, timeout = 60 SECONDS, autofocus = TRUE)
	if (!user)
		user = usr
	if (!istype(user))
		if (istype(user, /client))
