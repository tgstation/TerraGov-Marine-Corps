...
#define APPLY_WOUND_FLAG(FLAG) \
	if(CHECK_BITFIELD(wound_flags, WOUND_INTERNAL)) {\
		return FALSE;\
	}\
	if(!CHECK_BITFIELD(wound_flags, FLAG)) {\
		. = TRUE;\
	}\
	ENABLE_BITFIELD(wound_flags, FLAG);

/datum/wound/proc/bandage()
	APPLY_WOUND_FLAG(WOUND_BANDAGED)

/datum/wound/proc/disinfect()
	APPLY_WOUND_FLAG(WOUND_DISINFECTED)
	germ_level = 0

/datum/wound/proc/clamp()
	APPLY_WOUND_FLAG(WOUND_CLAMPED)
...
/datum/wound/proc/is_treated()
	if(damage_type == BRUISE || damage_type == CUT)
		return CHECK_BITFIELD(wound_flags, WOUND_BANDAGED)
	else if(damage_type == BURN)
		return CHECK_BITFIELD(wound_flags, WOUND_SALVED)
...
/datum/wound/proc/can_merge(var/datum/wound/other)
	...
	if (!(other.can_autoheal()) != !(src.can_autoheal()))
		return FALSE
	if(other.wound_flags != src.wound_flags)
		return FALSE
...
/datum/wound/proc/infection_check()
	...
	if (is_treated() && damage < 25)	//anything less than a flesh wound (or equivalent) isn't infectable if treated properly
		return FALSE
	if (CHECK_BITFIELD(wound_flags, WOUND_DISINFECTED))
...
/datum/wound/proc/heal_damage(amount, heals_internal = 0)
	if(CHECK_BITFIELD(wound_flags, WOUND_INTERNAL) && !heals_internal)
		// heal nothing
		return amount
