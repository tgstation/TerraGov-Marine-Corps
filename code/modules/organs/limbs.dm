...
/datum/limb/proc/droplimb(amputation, delete_limb = FALSE)
	...
	switch(body_part)
		...
		if(ARM_RIGHT)
			if(amputation)
				organ = new /obj/item/robot_parts/biotic/r_arm(owner.loc, owner)
			else if(limb_status & LIMB_ROBOT)
				organ = new /obj/item/robot_parts/r_arm(owner.loc)
			else
				organ = new /obj/item/limb/r_arm(owner.loc, owner)
		if(ARM_LEFT)
			if(amputation)
				organ = new /obj/item/robot_parts/biotic/l_arm(owner.loc, owner)
			else if(limb_status & LIMB_ROBOT)
		...
		if(LEG_RIGHT)
			if(amputation)
				organ = new /obj/item/robot_parts/biotic/r_leg(owner.loc, owner)
			else if(limb_status & LIMB_ROBOT)
		...
		if(LEG_LEFT)
			if(amputation)
				organ = new /obj/item/robot_parts/biotic/l_leg(owner.loc, owner)
			else if(limb_status & LIMB_ROBOT)
				organ = new /obj/item/robot_parts/l_leg(owner.loc)
			else
				organ = new /obj/item/limb/l_leg(owner.loc, owner)
		if(HAND_RIGHT)
			if(!amputation && !(limb_status & LIMB_ROBOT))
