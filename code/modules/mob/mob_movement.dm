...
/mob/proc/Process_Spacemove(var/check_drift = 0)
	...
	if(istype(src,/mob/living/carbon/human/))
		...
		if(istype(H.shoes, /obj/item/clothing/shoes/magboots) && (H.shoes.flags_inv & NOSLIPPING))  //magboots + dense_object = no floaty effect
			make_floating(0)
		else
			make_floating(1)
	else
...
/mob/proc/Check_Dense_Object() //checks for anything to push off in the vicinity. also handles magboots on gravity-less floors tiles
	...
	for(var/turf/turf in oview(1,src))
		...
		if(istype(src,/mob/living/carbon/human/))  // Only humans can wear magboots, so we give them a chance to.
			var/mob/living/carbon/human/H = src
			if((istype(turf,/turf/simulated/floor)) && (src.lastarea.has_gravity == 0) && !(istype(H.shoes, /obj/item/clothing/shoes/magboots) && (H.shoes.flags_inv & NOSLIPPING)))
				continue
