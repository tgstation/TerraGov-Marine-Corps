...
/proc/play_fov_effect(atom/center, range, icon_state, dir = SOUTH, ignore_self = FALSE, angle = 0, time = 1.5 SECONDS, list/override_list)
	...
	for(var/mob/living/living_mob in override_list || get_hearers_in_view(range, center))
		...
		if(!mob_client)
			continue
		if(isdeaf(living_mob)) //Deaf people can't hear sounds so no sound indicators
			continue
	...
	if(clients_shown)
		addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(remove_images_from_clients), fov_image, clients_shown), time)
