...
/mob/living/proc/smoke_contact(obj/effect/particle_effect/smoke/S)
	var/bioProtection = max(1 - get_permeability_protection() * S.bio_protection, 0)
	var/acidProtection = max(1 - get_soft_acid_protection(), 0)
	var/acidHardProtection = get_hard_acid_protection()
	if(CHECK_BITFIELD(S.smoke_traits, SMOKE_EXTINGUISH))
		ExtinguishMob()
	if(CHECK_BITFIELD(S.smoke_traits, SMOKE_BLISTERING))
		adjustFireLoss(15 * bioProtection)
		to_chat(src, span_danger("It feels as if you've been dumped into an open fire!"))
	if(CHECK_BITFIELD(S.smoke_traits, SMOKE_XENO_ACID))
		if(prob(25 * acidProtection))
			to_chat(src, span_danger("Your skin feels like it is melting away!"))
		adjustFireLoss(max(S.strength * rand(20, 23) * acidProtection - acidHardProtection, 0))
	if(CHECK_BITFIELD(S.smoke_traits, SMOKE_XENO_TOXIC))
		...
		adjustFireLoss(SENTINEL_TOXIC_GRENADE_GAS_DAMAGE * bioProtection)
