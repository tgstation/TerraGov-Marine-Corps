		...
		if(M in view(src,1)) // Can I see my target?
			if(prob(max(feral * 10, 0)))	// Am I mad enough to want to sting? And yes, when I initially appear, I AM mad enough
				...
				if(worn_suit) // Are you wearing clothes?
					sting_prob -= min(worn_suit.armor.getRating("bio"),70) // Is it sealed? I can't get to 70% of your body.
				if(worn_helmet)
					sting_prob -= min(worn_helmet.armor.getRating("bio"),30) // Is your helmet sealed? I can't get to 30% of your body.
