...
/mob/living/simple_animal/hostile/guardian/protector/ToggleMode()
	...
	if(toggle)
		...
		to_chat(src, "<span class='danger'><B>I switch to protection mode.</span></B>")
		toggle = TRUE
	...
	if(summoner)
		if(get_dist(get_turf(summoner),get_turf(src)) <= range)
			...
			if(istype(summoner.loc, /obj/effect))
				...
				to_chat(summoner, "<span class='holoparasite'>I moved out of range, and were pulled back! You can only move [range] meters from <font color=\"[namedatum.colour]\"><b>[real_name]</b></font>!</span>")
