...
/mob/living/proc/take_overall_damage_armored(damage, damagetype, armortype, sharp = FALSE, edge = FALSE, updating_health = FALSE) //This proc is overrided on humans, otherwise it just applies some damage and checks armor on chest if not human.
	if(damagetype == BRUTE)
		return take_overall_damage(modify_by_armor(damage, damagetype), 0, 0, sharp, edge, updating_health)
	if(damagetype == BURN)
		return take_overall_damage(0, modify_by_armor(damage, damagetype), 0, sharp, edge, updating_health)
	return FALSE
