...
/mob/living/proc/adjust_blood_volume(amount)
	if(!amount)
		CRASH("adjust_blood_volume called with [isnull(amount) ? "null" : "[amount]"] amount of blood.")
...
/mob/living/proc/set_blood_volume(amount)
	if(!amount)
		CRASH("set_blood_volume called with [isnull(amount) ? "null" : "[amount]"] amount of blood.")
