	...
	if(isliving(A))
		...
		if(!L.buckled && !L.anchored)
			...
			//restrained people act if they were on 'help' intent to prevent a person being pulled from being seperated from their puller
				...
				mob_swap_mode = SWAPPING
			else if(get_xeno_hivenumber() == L.get_xeno_hivenumber() && (L.allow_pass_flags & PASS_XENO || allow_pass_flags & PASS_XENO))
			...
			if(mob_swap_mode)
				...
				var/L_passmob = (L.allow_pass_flags & PASS_MOB) // we give PASS_MOB to both mobs to avoid bumping other mobs during swap.
				var/src_passmob = (allow_pass_flags & PASS_MOB)
				L.allow_pass_flags |= PASS_MOB
