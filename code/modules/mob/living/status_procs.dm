...
///////////////////////////////// DISABLEHANDS //////////////////////////////////
/mob/living/proc/IsDisableHands() //If we're immobilized
	return has_status_effect(STATUS_EFFECT_DISABLEHANDS)

/mob/living/proc/AmountDisableHands() //How many deciseconds remain in our Paralyzed status effect
	var/datum/status_effect/incapacitating/disablehands/effect_datum = IsDisableHands(FALSE)
	if(effect_datum)
		return effect_datum.duration - world.time
	return 0

/mob/living/proc/DisableHands(amount, ignore_canstun = FALSE) //Can't go below remaining duration
	if(SEND_SIGNAL(src, COMSIG_LIVING_STATUS_DISABLEHANDS, amount, ignore_canstun) & COMPONENT_NO_STUN)
		return
	if(((status_flags & CANSTUN) && !HAS_TRAIT(src, TRAIT_STUNIMMUNE)) || ignore_canstun)
		if(absorb_stun(amount, ignore_canstun))
			return
		var/datum/status_effect/incapacitating/disablehands/effect_datum = IsDisableHands(FALSE)
		if(effect_datum)
			effect_datum.duration = max(world.time + amount, effect_datum.duration)
		else if(amount > 0)
			effect_datum = apply_status_effect(STATUS_EFFECT_DISABLEHANDS, amount)
		return effect_datum

/mob/living/proc/SetDisableHands(amount, ignore_canstun = FALSE) //Sets remaining duration
	if(SEND_SIGNAL(src, COMSIG_LIVING_STATUS_DISABLEHANDS, amount, ignore_canstun) & COMPONENT_NO_STUN)
		return
	if(((status_flags & CANSTUN) && !HAS_TRAIT(src, TRAIT_STUNIMMUNE)) || ignore_canstun)
		var/datum/status_effect/incapacitating/disablehands/effect_datum = IsDisableHands(FALSE)
		if(amount <= 0)
			...
			if(absorb_stun(amount, ignore_canstun))
				return
			if(effect_datum)
				effect_datum.duration = world.time + amount
			else
				effect_datum = apply_status_effect(STATUS_EFFECT_DISABLEHANDS, amount)
		return effect_datum

/mob/living/proc/AdjustDisableHands(amount, ignore_canstun = FALSE) //Adds to remaining duration
	if(SEND_SIGNAL(src, COMSIG_LIVING_STATUS_DISABLEHANDS, amount, ignore_canstun) & COMPONENT_NO_STUN)
		return
	if(((status_flags & CANSTUN) && !HAS_TRAIT(src, TRAIT_STUNIMMUNE)) || ignore_canstun)
		if(absorb_stun(amount, ignore_canstun))
		...
		var/datum/status_effect/incapacitating/disablehands/effect_datum = IsDisableHands(FALSE)
		if(effect_datum)
			effect_datum.duration += amount
		else if(amount > 0)
			effect_datum = apply_status_effect(STATUS_EFFECT_DISABLEHANDS, amount)
		return effect_datum

///////////////////////////////// WORMED //////////////////////////////////
/mob/living/proc/IsWormed() //If we're immobilized
	return has_status_effect(STATUS_EFFECT_WORMED)

/mob/living/proc/AmountWormed() //How many deciseconds remain in our Paralyzed status effect
	var/datum/status_effect/incapacitating/wormed/effect_datum = IsWormed(FALSE)
	if(effect_datum)
		return effect_datum.duration - world.time
	return 0

/mob/living/proc/Wormed(amount, ignore_canstun = FALSE) //Can't go below remaining duration
	if(SEND_SIGNAL(src, COMSIG_LIVING_STATUS_WORMED, amount, ignore_canstun) & COMPONENT_NO_STUN)
		return
	if(((status_flags & CANKNOCKDOWN) && !HAS_TRAIT(src, TRAIT_STUNIMMUNE)) || ignore_canstun)
		if(absorb_stun(amount, ignore_canstun))
			return
		...
		if(effect_datum)
			...
			effect_datum = apply_status_effect(STATUS_EFFECT_WORMED, amount)
		return effect_datum

/mob/living/proc/SetWormed(amount, ignore_canstun = FALSE) //Sets remaining duration
	if(SEND_SIGNAL(src, COMSIG_LIVING_STATUS_WORMED, amount, ignore_canstun) & COMPONENT_NO_STUN)
		return
	if(((status_flags & CANKNOCKDOWN) && !HAS_TRAIT(src, TRAIT_STUNIMMUNE)) || ignore_canstun)
		var/datum/status_effect/incapacitating/wormed/effect_datum = IsWormed(FALSE)
...
/mob/living/proc/AdjustWormed(amount, ignore_canstun = FALSE) //Adds to remaining duration
	if(SEND_SIGNAL(src, COMSIG_LIVING_STATUS_WORMED, amount, ignore_canstun) & COMPONENT_NO_STUN)
		return
	if(((status_flags & CANKNOCKDOWN) && !HAS_TRAIT(src, TRAIT_STUNIMMUNE)) || ignore_canstun)
		if(absorb_stun(amount, ignore_canstun))
...
/mob/living/proc/set_frozen(freeze = TRUE)
	frozen = freeze
	if(frozen)
		add_immobile_flags(IMMOBILE_FROZEN)
	else
		remove_immobile_flags(IMMOBILE_FROZEN)
