...
/obj/machinery/computer/drone_control/attack_hand(var/mob/user as mob)
	...
	dat += "[dronefab ? "<A href='?src=\ref[src];toggle_fab=1'>[(dronefab.produce_drones && !(dronefab.machine_stat & NOPOWER)) ? "ACTIVE" : "INACTIVE"]</A>" : "<font color='red'><b>FABRICATOR NOT DETECTED.</b></font> (<A href='?src=\ref[src];search_fab=1'>search</a>)"]"
...
/obj/machinery/computer/drone_control/Topic(href, href_list)
	...
	if (href_list["setarea"])
		...
		if(D.stat != 2)
			...
			message_admins("[ADMIN_TPMONTY(usr)] issued kill order for drone [ADMIN_TPMONTY(D)] from control console.")
			D.shut_down()
