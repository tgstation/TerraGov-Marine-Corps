...
/datum/action/xeno_action/activable/behemoth
	...
	/// References the owner's Landslide action, if any.
	var/datum/action/xeno_action/activable/behemoth/landslide/landslide_action
	/// References the owner's Earth Riser ability, if any.
	...
	/// References the owner's Primal Wrath action, if any.
...
/datum/action/xeno_action/activable/behemoth/use_ability(atom/target)
	...
	if(!earth_riser_action)
		earth_riser_action = xeno_owner.actions_by_path?[/datum/action/xeno_action/activable/behemoth/earth_riser]
...
/// Stomps anyone on the same tile as the Behemoth, damaging them and knocking them down.
/datum/action/xeno_action/activable/behemoth/proc/do_stomp(turf/target_turf)
	if(!target_turf)
		return
...
/datum/action/xeno_action/activable/behemoth/proc/do_warning(list/turf/target_turfs, duration, enhanced, action)
	if(!length(target_turfs) || !duration)
		return
	...
	for(var/turf/target_turf AS in target_turfs)
		if(duration <= wind_up_duration && warning_sound)
			playsound(target_turf, warning_sound, 15, TRUE)
		...
		if(action == landslide_action && enhanced)
			landslide_action.enhanced_warnings += new warning_type(target_turf)
			continue
		new warning_type(target_turf, duration)

/datum/action/xeno_action/activable/behemoth/succeed_activate(plasma_cost_override)
	if(QDELETED(owner))
		return
	var/ability_cost = plasma_cost
	if(plasma_cost_override)
		ability_cost = plasma_cost_override
	var/mob/living/carbon/xenomorph/xeno_owner = owner
	if(primal_wrath_action?.ability_active)
		xeno_owner.wrath_stored = max(0, xeno_owner.wrath_stored - ability_cost)
		return
	if(ability_cost > 0)
...
/datum/action/xeno_action/activable/behemoth/landslide/use_ability(atom/target)
	...
	var/mob/living/carbon/xenomorph/xeno_owner = owner
	addtimer(CALLBACK(src, PROC_REF(do_charge), owner_turf, direction, xeno_owner.xeno_caste.melee_damage * xeno_owner.xeno_melee_damage_modifier, which_step), wind_up_duration)
...
/datum/action/xeno_action/activable/behemoth/landslide/proc/enhanced_prepare_charge(datum/source, atom/selected_atom)
	...
	do_warning(get_affected_turfs(origin_turf, get_dir(origin_turf, corrected_turf), get_dist(origin_turf, corrected_turf)), LANDSLIDE_ENHANCED_WAIT_TIME, TRUE, src)
...
/datum/action/xeno_action/activable/behemoth/landslide/proc/enhanced_next_charge(direction, which_charge, damage)
...
/datum/action/xeno_action/activable/behemoth/earth_riser/use_ability(atom/target)
	...
	do_warning(list(target_turf), wind_up_duration)
...
/datum/action/xeno_action/activable/behemoth/earth_riser/proc/do_projectile(obj/structure/earth_pillar/target_pillar, explode)
	if(!target_pillar)
		return
...
/datum/action/xeno_action/activable/behemoth/earth_riser/proc/change_maximum_pillars(amount)
	if(!amount)
		return
...
/datum/action/xeno_action/activable/behemoth/seismic_fracture/use_ability(atom/target)
	...
	new /obj/effect/temp_visual/behemoth/crack(owner_turf, owner.dir)
	do_stomp(owner_turf)
	do_ability(get_turf(target), wind_up_duration, primal_wrath_action?.ability_active? TRUE : FALSE)
...
/datum/action/xeno_action/activable/behemoth/seismic_fracture/proc/do_ability(turf/target_turf, wind_up = wind_up_duration, enhanced)
	if(!target_turf)
		return
	...
	add_cooldown()
	succeed_activate()
	do_warning(turfs_to_attack, wind_up, enhanced)
	...
	do_warning(extra_turfs_to_warn, wind_up + SEISMIC_FRACTURE_ENHANCED_DELAY, enhanced)
...
/datum/action/xeno_action/activable/behemoth/seismic_fracture/proc/do_attack_extra(turf/origin_turf, list/turf/extra_turfs, list/turf/excepted_turfs, enhanced, range, iteration)
...
/// - target_turf: The turf we are targeting.
/// - turfs_to_attack:
/datum/action/xeno_action/activable/behemoth/seismic_fracture/proc/do_attack(list/turf/turfs_to_attack, enhanced, instant)
	if(!length(turfs_to_attack))
		return
	var/mob/living/carbon/xenomorph/xeno_owner = owner
...
/// Living mobs that were previously caught in the attack's radius are subject to a landing effect.
...
#define PRIMAL_WRATH_ACTIVE_DECAY_DIVISION 25
...
/particles/primal_wrath
	...
	gravity = list(0, 0.25)
	scale = generator(GEN_NUM, 0.2, 0.4, UNIFORM_RAND)
	rotation = 0
...
/datum/action/xeno_action/primal_wrath
	...
	/// The xeno owner of this ability.
	var/mob/living/carbon/xenomorph/xeno_owner = owner
	/// References the owner's Earth Riser ability, if any.
	var/datum/action/xeno_action/activable/behemoth/earth_riser/earth_riser_action
...
/datum/action/xeno_action/primal_wrath/give_action(mob/living/L)
	...
	xeno_owner.vis_contents += block_overlay
	START_PROCESSING(SSprocessing, src)
	RegisterSignal(xeno_owner, list(COMSIG_XENOMORPH_BRUTE_DAMAGE, COMSIG_XENOMORPH_BURN_DAMAGE), PROC_REF(taking_damage))
	RegisterSignal(xeno_owner, list(COMSIG_PARENT_QDELETING, COMSIG_MOB_DEATH, COMSIG_XENOMORPH_EVOLVED, COMSIG_XENOMORPH_DEEVOLVED), PROC_REF(stop_process))
...
/// Resets decay, and gives the owner an amount of Wrath proportional to the damage taken.
...
/datum/action/xeno_action/primal_wrath/proc/taking_damage(datum/source, amount, list/amount_mod)
	...
	if(primal_wrath_action.ability_active)
		if(amount >= xeno_owner.health)
			xeno_owner.wrath_stored = max(0, xeno_owner.wrath_stored - amount)
			amount_mod += (amount - xeno_owner.health) + 1
			var/atom/movable/vis_obj/wrath_block
		if(xeno_owner.wrath_stored <= 0)
			toggle_buff(FALSE)
		return
...
/datum/action/xeno_action/primal_wrath/proc/stop_ability()
	SIGNAL_HANDLER
...
/obj/structure/earth_pillar/Destroy()
	playsound(src, destroy_sound, 40, TRUE)
...
/datum/ammo/xeno/earth_pillar/proc/on_hit_anything(atom/hit_atom, obj/projectile/proj, explosion)
	...
	playsound(atom_turf, sound_hit, 40, TRUE)
...
/datum/ammo/xeno/earth_pillar/explosive/on_hit_anything(atom/hit_atom, obj/projectile/proj)
	...
	seismic_fracture_action?.do_ability(get_turf(hit_atom), 0, FALSE)
