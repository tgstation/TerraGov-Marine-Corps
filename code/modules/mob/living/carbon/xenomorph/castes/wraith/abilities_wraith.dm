...
/datum/action/xeno_action/activable/rewind/proc/rewind()
	...
	if(!loc_b)
		...
		targeted.heal_overall_damage(targeted.getBruteLoss() - target_initial_brute_damage, BRUTE, updating_health = TRUE)
		targeted.heal_overall_damage(targeted.getFireLoss() - target_initial_burn_damage, BURN, updating_health = TRUE)
		if(isxeno(target))
			var/mob/living/carbon/xenomorph/xeno_target = targeted
