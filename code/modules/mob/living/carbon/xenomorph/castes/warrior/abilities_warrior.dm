...
/datum/action/ability/xeno_action/toggle_agility/action_activate()
	...
	if(!toggled)
		...
		attached_armor = new()
		attached_armor.modifyAllRatings(WARRIOR_AGILITY_ARMOR_MODIFIER)
...
/datum/action/ability/activable/xeno/warrior/fling
	...
	target_flags = ABILITY_MOB_TARGET
	/// The percentage to reduce the cooldown duration by if the ability was used on a friendly xenomorph.
	var/friendly_cooldown_reduction_percentage
	/// The starting amount of distance that Fling can go.
