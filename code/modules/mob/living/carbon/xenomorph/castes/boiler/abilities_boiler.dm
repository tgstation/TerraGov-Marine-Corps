...
/datum/action/xeno_action/bombard/action_activate()
	...
	if(do_after(X, 30, FALSE, icon_display = USER_ICON_HOSTILE) && !X.is_bombarding)
		X.is_bombarding = 1
...
/mob/living/carbon/Xenomorph/Boiler/proc/bomb_turf(var/turf/T)
	...
	if(!do_after(src, 50, FALSE, target, icon_display = USER_ICON_HOSTILE))
		bomb_cooldown = FALSE
...
/mob/living/carbon/Xenomorph/proc/acid_spray(atom/T, plasmacost = 250, acid_d = xeno_caste.acid_delay)
	...
	if(!do_after(src, 5, TRUE, T, icon_display = USER_ICON_DANGER))
		return
