...
/datum/action/xeno_action/activable/acid_dash/proc/dash_complete()
	...
	SIGNAL_HANDLER
	if(recast_available)
		addtimer(CALLBACK(src, PROC_REF(dash_complete)), 6 SECONDS) //Delayed recursive call, this time you won't gain a recast so it will go on cooldown in 6 SECONDS.
		recast = TRUE
	else
		recast = FALSE
		add_cooldown()
