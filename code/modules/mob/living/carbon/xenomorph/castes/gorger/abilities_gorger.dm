...
/datum/action/xeno_action/activable/rejuvenate
	...
	mechanics_text = "When used on self drains 10 blood and restores 40% health/plasma over 5s. When used on another xenomorph - restores 30% of their health after 2s. When used on a dead human you grab them and heal by draining their blood"
	...
	if(isxeno(target))
		if(X == target)
			if(!COOLDOWN_CHECK(X, rejuvenate_self_cooldown))
				to_chat(X, "<span class='notice'>We need another [round(COOLDOWN_TIMELEFT(X, rejuvenate_self_cooldown) / 10)] seconds before we can revitalize ourselves.</span>")
				return FALSE
			if(X.blood_bank < 10)
