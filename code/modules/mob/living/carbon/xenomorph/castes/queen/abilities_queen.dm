...
/datum/action/xeno_action/summon_king/action_activate()
	...
	var/atom/pod = new /obj/structure/resin/king_pod(X.loc, X.hivenumber)
	log_game("[key_name(X)] has created a pod in [AREACOORD(X)]")
	xeno_message("<B>[X] has created a king pod at [get_area(X)]. Defend it until the Queen Mother summons a king!</B>", size = 3, hivenumber = X.hivenumber, target = pod, arrow_type = /obj/screen/arrow/leader_tracker_arrow)
