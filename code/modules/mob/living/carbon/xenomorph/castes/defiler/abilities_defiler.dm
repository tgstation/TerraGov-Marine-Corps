	...
	if(!emitted_gas)
		switch(defiler_owner.selected_reagent)
			...
			if(/datum/reagent/toxin/xeno_aphrotoxin)
				emitted_gas = new /datum/effect_system/smoke_spread/xeno/aphrotoxin(defiler_owner)
...
/datum/action/xeno_action/select_reagent
	...
	desc = "Selects which reagent to use for reagent slash and noxious gas. Neuro causes increasing pain and stamina damage. Hemodile slows targets down, multiplied by each other xeno-based toxin. Transvitox converts burns to toxin, and causes additional toxin damage when they take brute damage, both effects multiplied by other xeno-based toxins. Ozelomelyn purges all medicines from their system rapidly and causes minor toxin damage. Aphrotoxin boosts larva growth in hosts and makes them horny, will make their legs weak and make them stumble around."
