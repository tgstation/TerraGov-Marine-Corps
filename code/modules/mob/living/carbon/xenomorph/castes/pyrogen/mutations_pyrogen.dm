...
/datum/mutation_upgrade/shell/flame_cloak
	...
	/// For each structure, the additional armor gained.
...
/datum/mutation_upgrade/shell/flame_cloak/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
	return "If you are ontop of fire, you gain [armor_per_structure * new_amount] in all categories."
...
/datum/mutation_upgrade/spur/only_fire/on_mutation_enabled()
	var/datum/action/ability/activable/xeno/charge/fire_charge/ability = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/charge/fire_charge]
	if(!ability)
		return FALSE
...
/datum/mutation_upgrade/spur/only_fire/on_structure_update(previous_amount, new_amount)
	...
	if(!ability)
		return FALSE
	ability.stacks_to_add += (new_amount - previous_amount) * stacks_per_structure
...
/datum/mutation_upgrade/veil/burnt_wounds
	name = "Burnt Wounds"
	desc = "Melting fire stacks you inflict through your slashes cause 15/25/35% healing reduction against brute and burn."
	/// For the first structure, the percentage amount of healing reduction.
	var/percentage_initial = 0.05
	/// For each structure, the additional percentage amount of healing reduction.
	var/percentage_per_structure = 0.1

/datum/mutation_upgrade/veil/burnt_wounds/get_desc_for_alert(new_amount)
	if(!new_amount)
		return ..()
	return "Melting fire stacks you inflict through your slashes cause [(percentage_initial + (percentage_per_structure * new_amount)) * 100]% healing reduction against brute and burn."
...
/datum/mutation_upgrade/veil/burnt_wounds/on_mutation_enabled()
	...
	pyrogen_owner.melting_fire_healing_reduction += percentage_initial
	return ..()
...
/datum/mutation_upgrade/veil/burnt_wounds/on_structure_update(previous_amount, new_amount)
	...
	if(!. || !isxenopyrogen(xenomorph_owner))
		return FALSE
	var/mob/living/carbon/xenomorph/pyrogen/pyrogen_owner = xenomorph_owner
	pyrogen_owner.melting_fire_healing_reduction += (new_amount - previous_amount) * percentage_per_structure
