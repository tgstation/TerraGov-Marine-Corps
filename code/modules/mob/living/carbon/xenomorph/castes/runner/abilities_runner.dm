...
/datum/action/xeno_action/toggle_savage/action_activate()
	...
	if(X.savage)
		X.savage = FALSE
		X.balloon_alert(X, "We will no longer savage when pouncing")
	else
		X.savage = TRUE
		X.balloon_alert(X, "We will savage when pouncing.[X.savage_used ? " However, we're not quite yet able to savage again" : ""]")
...
/datum/action/xeno_action/evasion/action_activate()
	...
	R.balloon_alert(R, "We take evasive action, making us impossible to hit with projectiles")
...
/datum/action/xeno_action/evasion/proc/evasion_flamer_hit(datum/source, obj/projectile/proj)
	...
	if(evasion_stacks)
		owner.balloon_alert(owner, "The fire compromises our ability to dodge!")
...
/datum/action/xeno_action/evasion/proc/evasion_burn_check()
	...
	if(evasion_stacks)
		R.balloon_alert(R, "We have lost all evasion stacks!")
...
/datum/action/xeno_action/evasion/proc/evasion_deactivate()
	...
	owner.balloon_alert(owner, "Projectiles will hit us normally again")
...
/datum/action/xeno_action/evasion/proc/evasion_dodge_sfx(atom/movable/proj)
	...
	X.balloon_alert(X, "We effortlessly dodge the [proj.name]")
	...
	if(!stolen_item)
		...
		for(var/slot in slots_to_steal_from)
			...
			if(stolen_item)
				break
	if(!stolen_item)
		owner.balloon_alert(owner, "They have nothing to steal")
