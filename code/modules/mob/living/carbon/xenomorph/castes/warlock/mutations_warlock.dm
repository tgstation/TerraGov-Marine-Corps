...
/datum/mutation_upgrade/spur/draining_blast
	...
	var/cooldown_multiplier_per_structure = -0.05
...
/datum/mutation_upgrade/spur/draining_blast/on_mutation_enabled()
	var/datum/action/ability/activable/xeno/psy_blast/psy_blast = xenomorph_owner.actions_by_path[/datum/action/ability/activable/xeno/psy_blast]
	if(!psy_blast)
		return FALSE
	psy_blast.selectable_ammo_types += /datum/ammo/energy/xeno/psy_blast/psy_drain
	return ..()
...
/datum/mutation_upgrade/veil/mobile_mind
	name = "Mobile Mind"
	desc = "Psychic Shield no longer requires you to stand still, but cannot be canceled or detonated manually. While it is active, you are 0.8/0.6/0.4 slower."
