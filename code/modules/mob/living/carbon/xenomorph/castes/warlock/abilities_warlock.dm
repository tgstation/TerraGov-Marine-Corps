...
/datum/action/ability/activable/xeno/psychic_shield
	...
	/// The amount of plasma required and consumed to reactivate the ability in order to detonate the shield.
	var/detonation_cost = 200
	/// Instead of canceling normally, attempt to detonate it instead.
	var/always_detonate = FALSE
	/// Can they manually detonate this? This does not prevent automatic detonation.
	var/can_manually_detonate = TRUE
	/// Can they move while the shield is active? If so, what movement speed modifier to give them?
	var/movespeed_modifier_amount = 0
...
/datum/action/ability/activable/xeno/psychic_shield/use_ability(atom/targetted_atom)
	if(active_shield)
		if(!can_manually_detonate)
			return FALSE
		if(detonation_cost > xeno_owner.plasma_stored)
			owner.balloon_alert(owner, "[detonation_cost - xeno_owner.plasma_stored] more plasma!")
			return FALSE
	...
	if(alternative_reflection)
		active_shield = new /obj/effect/xeno/shield/special(target_turf, owner, src)
	else
		active_shield = new(target_turf, owner, src)
	if(movespeed_modifier_amount)
		xeno_owner.add_movespeed_modifier(MOVESPEED_ID_WARLOCK_PSY_SHIELD, TRUE, 0, NONE, TRUE, movespeed_modifier_amount)
	if(!do_after(owner, 6 SECONDS, movespeed_modifier_amount ? IGNORE_LOC_CHANGE : NONE, owner, BUSY_ICON_DANGER, extra_checks = CALLBACK(src, PROC_REF(can_use_action), FALSE, ABILITY_USE_BUSY|ABILITY_IGNORE_PLASMA)))
		if(always_detonate && active_shield && xeno_owner.plasma_stored >= detonation_cost)
			shield_blast()
		cancel_shield()
		return
	if(always_detonate && active_shield && xeno_owner.plasma_stored >= detonation_cost)
...
/datum/action/ability/activable/xeno/psychic_shield/proc/cancel_shield()
	...
	if(movespeed_modifier_amount)
		xeno_owner.remove_movespeed_modifier(MOVESPEED_ID_WARLOCK_PSY_SHIELD)
	update_button_icon()
	add_cooldown()
	if(active_shield)
		active_shield.release_projectiles()
...
/datum/action/ability/activable/xeno/psychic_shield/proc/shield_blast(atom/targetted_atom)
	succeed_activate(detonation_cost)
