...
/obj/item/xeno_egg/proc/plant_egg_in_containment(mob/living/carbon/human/user, turf/T)
	...
	if(!do_after(user, 50, TRUE, T, USER_ICON_BUILD))
		return
	for (var/obj/O in T)
		if (!istype(O,/obj/machinery/light/small))
			return
...
/obj/item/xeno_egg/proc/plant_egg(mob/living/carbon/Xenomorph/user, turf/T)
	...
	if(!isxenodrone(user))
		plant_time = 25
	if(!do_after(user, plant_time, TRUE, T, USER_ICON_BUILD, extra_checks = CALLBACK(T, /turf/proc/check_alien_construction, user)))
		return
	if(!user.check_plasma(30))
		return
	if(locate(/obj/effect/alien/weeds) in T)
