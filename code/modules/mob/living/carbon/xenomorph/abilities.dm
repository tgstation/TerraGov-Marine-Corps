...
/datum/action/xeno_action/activable/larval_growth_sting/use_ability(mob/living/carbon/A)
	...
	if(locate(/obj/item/alien_embryo) in A)
		...
		X.recurring_injection(A, list(/datum/reagent/toxin/xeno_neurotoxin,/datum/reagent/toxin/acid,/datum/reagent/consumable/larvajelly), XENO_LARVAL_CHANNEL_TIME, XENO_LARVAL_AMOUNT_RECURRING, 3)
...
/datum/action/xeno_action/activable/tail_stab
	...
	target_flags = XABB_MOB_TARGET|XABB_MOB_TARGET
	var/range = 2
	var/punch_description = "swift tail-jab!"

/datum/action/xeno_action/activable/tail_stab/on_cooldown_finish()
	var/mob/living/carbon/xenomorph/X = owner
	to_chat(X, span_xenodanger("We gather our strength to stab again."))
	owner.playsound_local(owner, 'sound/effects/xeno_newlarva.ogg', 25, 0, 1)
	return ..()

/datum/action/xeno_action/activable/tail_stab/can_use_ability(atom/A, silent = FALSE, override_flags)
	. = ..()
	if(!.)
		return
...
/datum/action/xeno_action/activable/tail_stab/use_ability(atom/A)
	var/mob/living/carbon/xenomorph/X = owner
...
/datum/action/xeno_action/activable/tail_stab/ai_should_start_consider()
	return TRUE

/datum/action/xeno_action/activable/tail_stab/ai_should_use(atom/target)
	if(!iscarbon(target))
		return FALSE
