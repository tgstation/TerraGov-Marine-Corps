...
/datum/action/xeno_action/activable/headbite/can_use_ability(atom/A, silent = FALSE, override_flags)
	...
	if(!X.Adjacent(victim)) //checks if owner next to target
		return FALSE
	if(X.on_fire)
		to_chat(X, "<span class='warning'>We're too busy being on fire to enter to this!</span>")
		return FALSE
	if(victim.stat != DEAD)
		if(!silent)
	...
	if(!do_after(X, 10 SECONDS, FALSE, victim, BUSY_ICON_DANGER, extra_checks = CALLBACK(X, /mob/living/proc/break_do_after_checks, list/checked_health, null, null)))
