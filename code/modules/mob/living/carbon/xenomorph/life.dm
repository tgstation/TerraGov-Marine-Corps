...
/mob/living/carbon/xenomorph/proc/handle_living_health_updates()
	...
	if(hive?.living_xeno_ruler?.loc?.z == T.z || xeno_caste.xeno_can_flags & CASTE_CAN_HEAL_WITHOUT_QUEEN || (SSticker?.mode.flags_round_type & MODE_XENO_RULER)) //if the living queen's z-level is the same as ours.
