...
/mob/living/carbon/xenomorph/proc/handle_living_health_updates()
	...
	if(hive?.living_xeno_ruler?.loc?.z == T.z || xeno_caste.caste_flags & CASTE_CAN_HEAL_WITHOUT_QUEEN || iscrashgamemode(SSticker.mode)) //if the living queen's z-level is the same as ours.
