...
/mob/living/carbon/xenomorph/proc/handle_critical_health_updates()
	...
	if((istype(T) && locate(/obj/effect/alien/weeds) in T))
		heal_wounds(XENO_RESTING_HEAL + (warding_aura * 0.5) * 0.5) //Warding pheromones provides 0.25 HP per second per step, up to 2.5 HP per tick. //Values halved -SplinterGP
	else
