...
/mob/living/carbon/xenomorph/proc/damage_taken()
	...
	if((health < max(XENO_HEALTH_ALERT_TRIGGER_THRESHOLD, maxHealth * XENO_HEALTH_ALERT_TRIGGER_PERCENT)) & ( last_xeno_health_alert < (world.time - XENO_HEALTH_ALERT_COOLDOWN) ) )
		...
		xeno_message("<span class='xenoannounce'>Our sister [src.name] is badly hurt with ([health]/[maxHealth]) health remaining at [location_description]!</span>", 2, src.hivenumber, 'sound/voice/alien_help2.ogg', src.hive.xenos_health_alert_filter)
