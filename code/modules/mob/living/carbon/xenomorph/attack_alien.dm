...
/mob/living/proc/attack_alien_grab(mob/living/carbon/Xenomorph/M)
...
/mob/living/carbon/human/attack_alien_grab(mob/living/carbon/Xenomorph/M)
...
/mob/living/proc/attack_alien_disarm(mob/living/carbon/Xenomorph/M, dam_bonus)
	playsound(loc, 'sound/weapons/alien_knockdown.ogg', 25, 1)
	M.visible_message("<span class='warning'>\The [M] shoves [src]!</span>", \
	"<span class='warning'>You shove [src]!</span>", null, 5)
	return TRUE

/mob/living/carbon/monkey/attack_alien_disarm(mob/living/carbon/Xenomorph/M, dam_bonus)
	. = ..()
	KnockDown(8)

/mob/living/carbon/human/attack_alien_disarm(mob/living/carbon/Xenomorph/M, dam_bonus)
	...
	if(M.stealth_router(HANDLE_STEALTH_CHECK))
		if(M.stealth_router(HANDLE_SNEAK_ATTACK_CHECK))
			...
			if(m_intent == MOVE_INTENT_RUN && ( M.last_move_intent > (world.time - HUNTER_SNEAK_ATTACK_RUN_DELAY) ) ) //Allows us to slash while running... but only if we've been stationary for awhile
...
/mob/living/proc/can_xeno_slash(mob/living/carbon/Xenomorph/M)
	if(CHECK_BITFIELD(M.xeno_caste.caste_flags, CASTE_IS_INTELLIGENT)) // intelligent ignore restrictions
		return TRUE
...
/mob/living/carbon/human/can_xeno_slash(mob/living/carbon/Xenomorph/M)
	. = ..()
	if(!.)
		return FALSE
...
/mob/living/proc/get_xeno_slash_zone(mob/living/carbon/Xenomorph/M, set_location = FALSE, random_location = FALSE, no_head = FALSE)
	return

/mob/living/carbon/get_xeno_slash_zone(mob/living/carbon/Xenomorph/M, set_location = FALSE, random_location = FALSE, no_head = FALSE)
...
/mob/living/proc/attack_alien_harm(mob/living/carbon/Xenomorph/M, dam_bonus, set_location = FALSE, random_location = FALSE, no_head = FALSE, no_crit = FALSE, force_intent = null)
	if(!can_xeno_slash(M))
		return FALSE
	...
	var/datum/limb/affecting = get_xeno_slash_zone(M, set_location, random_location, no_head)
	...
	if(M.stealth_router(HANDLE_STEALTH_CHECK)) //Cancel stealth if we have it due to aggro.
		if(M.stealth_router(HANDLE_SNEAK_ATTACK_CHECK)) //Pouncing prevents us from making a sneak attack for 4 seconds
			...
			var/staggerslow_stacks = 2
			var/knockout_stacks = 1
			damage *= M.sneak_bonus //Massive damage on the sneak attack... hope you have armour.
			if(m_intent == MOVE_INTENT_RUN && ( M.last_move_intent > (world.time - HUNTER_SNEAK_ATTACK_RUN_DELAY) ) ) //Allows us to slash while running... but only if we've been stationary for awhile
...
/mob/living/silicon/attack_alien_harm(mob/living/carbon/Xenomorph/M, dam_bonus, set_location = FALSE, random_location = FALSE, no_head = FALSE, no_crit = FALSE, force_intent = null)
...
/mob/living/carbon/Xenomorph/attack_alien_harm(mob/living/carbon/Xenomorph/M, dam_bonus, set_location = FALSE, random_location = FALSE, no_head = FALSE, no_crit = FALSE, force_intent = null)
	if(issamexenohive(M))
...
/mob/living/carbon/human/attack_alien_harm(mob/living/carbon/Xenomorph/M, dam_bonus, set_location = FALSE, random_location = FALSE, no_head = FALSE, no_crit = FALSE, force_intent = null)
	if(stat == DEAD)
		if(luminosity > 0)
			playsound(loc, "alien_claw_metal", 25, 1)
...
/mob/living/attack_alien(mob/living/carbon/Xenomorph/M, dam_bonus, set_location = FALSE, random_location = FALSE, no_head = FALSE, no_crit = FALSE, force_intent = null)
	...
	switch(intent)
		...
		if(INTENT_GRAB)
			return attack_alien_grab(M)

		if(INTENT_HARM)
			return attack_alien_harm(M, dam_bonus, set_location, random_location, no_head, no_crit, force_intent)

		if(INTENT_DISARM)
			return attack_alien_disarm(M, dam_bonus)
	return FALSE
