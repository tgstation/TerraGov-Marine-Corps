/datum/hive_status
	...
	// Any active HUD timers that you want to display on a xeno's screen
	var/list/hud_timers = list()
...
/datum/hive_status/proc/setup_hud_timer(list/arguments)
	var/atom/movable/screen/text/screen_timer/hud_timer = new(null, arglist(arguments))
...
/datum/hive_status/normal/handle_ruler_timer()
	...
	if(living_xeno_ruler)
		if(D.orphan_hive_timer)
			...
			hud_timers.Remove(D.orphan_hive_timer)
		return
	...
	setup_hud_timer(D.orphan_hive_timer, "Orphan Hivemind Collapse: ${timer}", 150, -80)
