...
/datum/action/xeno_action/ready_charge/proc/do_crush(datum/source, atom/crushed)
	...
	if(isliving(crushed))
		...
		if(precrush > 0)
			...
			crushed_living.apply_damage(precrush, BRUTE, BODY_ZONE_CHEST, crushed_living.get_soft_armor("melee", BODY_ZONE_CHEST), updating_health = TRUE) //There is a chance to do enough damage here to gib certain mobs. Better update immediately.
