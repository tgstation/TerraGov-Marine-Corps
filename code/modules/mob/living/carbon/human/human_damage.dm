...
/mob/living/carbon/human/proc/adjustBruteLossByPart(var/amount, var/organ_name, var/obj/damage_source = null)
	...
	for(var/X in limbs)
		...
		if(O.name == organ_name)
			if(amount > 0)
				O.limb_take_damage(amount, 0, is_sharp(damage_source), has_edge(damage_source))
			else
...
/mob/living/carbon/human/proc/adjustFireLossByPart(var/amount, var/organ_name, var/obj/damage_source = null)
	...
	for(var/X in limbs)
		...
		if(O.name == organ_name)
			if(amount > 0)
				O.limb_take_damage(0, amount, is_sharp(damage_source), has_edge(damage_source))
			else
				//if you don't want to heal robot limbs, they you will have to check that yourself before using this proc.
...
/mob/living/carbon/human/take_limb_damage(var/brute, var/burn, var/sharp = 0, var/edge = 0)
	...
	if(!parts.len)	return
	var/datum/limb/picked = pick(parts)
	if(picked.limb_take_damage(brute, burn, sharp, edge))
		UpdateDamageIcon()
...
/mob/living/carbon/human/take_overall_damage(var/brute, var/burn, var/sharp = 0, var/edge = 0, var/used_weapon = null, var/blocked = 0)
	...
	while(parts.len && (brute>0 || burn>0) )
		...
		update |= picked.limb_take_damage(brute, burn, sharp, edge)
...
/mob/living/carbon/human/apply_damage(var/damage = 0, var/damagetype = BRUTE, var/def_zone = null, var/blocked = 0, var/sharp = 0, var/edge = 0, var/obj/used_weapon = null)
	...
	switch(damagetype)
		if(BRUTE)
			...
			if(organ.limb_take_damage(damage, 0, sharp, edge))
				UpdateDamageIcon()
		if(BURN)
			damageoverlaytemp = 20
			if(species && species.burn_mod)
				damage = damage*species.burn_mod
			if(organ.limb_take_damage(0, damage, sharp, edge))
				UpdateDamageIcon()
