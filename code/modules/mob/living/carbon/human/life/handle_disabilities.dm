...
/mob/living/carbon/human/handle_disabilities()
	...
	switch(rand(0, 200))
		if(0 to 3)
			if(getBrainLoss() >= 5)
				balloon_alert(usr, "Your head feels numb and painful.")
				custom_pain("Your head feels numb and painful.")
		if(4 to 6)
			if(eye_blurry <= 0 && getBrainLoss() >= 15 )
				balloon_alert(usr, "It becomes hard to see for some reason.")
				to_chat(src, span_danger("It becomes hard to see for some reason."))
				blur_eyes(10)
		if(7 to 9)
			if((hand && get_held_item()) && getBrainLoss())
				balloon_alert(usr, "Your hand won't respond properly, you drop what you're holding.")
