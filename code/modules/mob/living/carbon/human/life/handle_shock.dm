...
/mob/living/carbon/human/handle_shock()
	...
	switch(shock_stage)
		...
		if(40 to 59)
			...
			blur_eyes(1)
			set_timed_status_effect(10 SECONDS, /datum/status_effect/speech/stutter, only_if_higher = TRUE)
			adjust_stagger(1 SECONDS, FALSE, 1 SECONDS)
			add_slowdown(1)
		if(60 to 79)
			...
			blur_eyes(2)
			set_timed_status_effect(10 SECONDS, /datum/status_effect/speech/stutter, only_if_higher = TRUE)
			adjust_stagger(3 SECONDS, FALSE, 3 SECONDS)
			add_slowdown(3)
			if(prob(20))
				to_chat(src, span_danger("[pick("The pain is excruciating", "Please, just end the pain", "Your whole body is going numb")]!"))
		if(80 to 119)
			blur_eyes(2)
			set_timed_status_effect(10 SECONDS, /datum/status_effect/speech/stutter, only_if_higher = TRUE)
			adjust_stagger(6 SECONDS, FALSE, 6 SECONDS)
			add_slowdown(6)
			if(prob(20))
				to_chat(src, span_danger("[pick("The pain is excruciating", "Please, just end the pain", "Your whole body is going numb")]!"))
		if(120 to 149)
			blur_eyes(2)
			set_timed_status_effect(10 SECONDS, /datum/status_effect/speech/stutter, only_if_higher = TRUE)
			adjust_stagger(9 SECONDS, FALSE, 9 SECONDS)
			add_slowdown(9)
			if(prob(20))
		...
		if(150 to INFINITY)
			blur_eyes(2)
			set_timed_status_effect(10 SECONDS, /datum/status_effect/speech/stutter, only_if_higher = TRUE)
			adjust_stagger(12 SECONDS, FALSE, 12 SECONDS)
			add_slowdown(12)
			if(prob(20))
				to_chat(src, span_danger("[pick("The pain is excruciating", "Please, just end the pain", "Your whole body is going numb", "You feel like you could die any moment now")]!"))
