...
/mob/living/carbon/human/proc/update_fire_resistant(equipped)
	...
	if(istype(wear_suit) && worn_suit.hard_armor && worn_suit.hard_armor.getRating("fire") >= 100) // outer suits provide total protection
		...
		for(var/part in required_for_full_coverage)
			...
			if(istype(worn_item) && worn_item.hard_armor && worn_item.hard_armor.getRating("fire") < 100)
