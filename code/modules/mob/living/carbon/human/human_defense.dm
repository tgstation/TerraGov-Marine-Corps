		...
		if("l_hand", "r_hand")
			...
			if(c_hand && (stun_amount || agony_amount > 10))
				...
				if (affected.limb_status & LIMB_ROBOT)
					emote("emote_me", 1, "drops what they were holding, their [affected.display_name] malfunctioning!")
				else
					var/emote_scream = pick("screams in pain and", "lets out a sharp cry and", "cries out and")
					emote("emote_me", 1, "[(species && species.species_flags & NO_PAIN) ? "" : emote_scream ] drops what they were holding in their [affected.display_name]!")
