...
/mob/living/carbon/human/Topic(href, href_list)
	...
	if(href_list["internal"])
		...
		if(!usr.action_busy)
			...
			if(do_mob(usr, src, POCKET_STRIP_DELAY, USER_ICON_GENERIC))
				if (internal)
	...
	if(href_list["splints"])
		...
		if(!usr.action_busy)
			...
			if(count)
				...
				if(do_mob(usr, src, HUMAN_STRIP_DELAY, TARGET_ICON_MEDICAL))
					var/limbcount = 0
			...
			if(w_uniform && istype(w_uniform, /obj/item/clothing/under))
				...
				if(U.hastie)
					...
					if(istype(U.hastie, /obj/item/clothing/tie/holobadge) || istype(U.hastie, /obj/item/clothing/tie/medal))
						...
						if(do_mob(usr, src, HUMAN_STRIP_DELAY, USER_ICON_HOSTILE))
	...
	if(href_list["sensor"])
		if(!usr.action_busy)
			...
			if(U.has_sensor >= 2)
				...
				if(do_mob(usr, src, HUMAN_STRIP_DELAY, TARGET_ICON_GENERIC))
					if(U == w_uniform)
						if(U.has_sensor >= 2)
							to_chat(usr, "The controls are locked.")
