		...
		switch(place)
			...
			if("sensor")
				if(!target.w_uniform)
					cdel(src)
			if("internal")
				if((!((istype(target.wear_mask, /obj/item/clothing/mask) && (istype(target.back, /obj/item/tank) || istype(target.belt, /obj/item/tank) || istype(target.s_store, /obj/item/tank)) && !(target.internal))) && !(target.internal)))
					cdel(src)
