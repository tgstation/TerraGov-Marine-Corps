...
/mob/living/carbon/proc/inhale_smoke(obj/effect/particle_effect/smoke/S)
	...
	if(CHECK_BITFIELD(S.smoke_traits, SMOKE_SETRAPINE))
		to_chat(src, span_danger("Your eyes are burning!"))
		blur_eyes(4)
		reagents.add_reagent(/datum/reagent/toxin/setrapine, GAS_INHALE_REAGENT_TRANSFER_AMOUNT * S.strength)
...
/mob/living/carbon/smoke_contact(obj/effect/particle_effect/smoke/S)
	...
	if(CHECK_BITFIELD(S.smoke_traits, SMOKE_SETRAPINE) && (internal || has_smoke_protection())) //either inhaled or this.
		reagents.add_reagent(/datum/reagent/toxin/setrapine, round(GAS_INHALE_REAGENT_TRANSFER_AMOUNT * 0.6 * S.strength * protection, 0.1))
		if(prob(10 * S.strength * protection))
