...
/mob/living/start_pulling(atom/movable/AM, lunge, no_msg)
	...
	if(M)
		...
		if(!no_msg)
			visible_message("<span class='warning'>[src] has grabbed [M] [(istype(src, /mob/living/carbon/human) && (zone_selected == "l_hand" || zone_selected == "r_hand")) ? "by their hands":"passively"]!</span>", null, null, 5)
