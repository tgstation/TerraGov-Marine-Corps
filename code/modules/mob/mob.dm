...
/mob/living/start_pulling(atom/movable/AM, suppress_message = FALSE)
	...
	if(M)
		...
		if(!suppress_message)
			visible_message("<span class='warning'>[src] has grabbed [M] [((ishuman(src) && ishuman(M)) && (zone_selected == "l_hand" || zone_selected == "r_hand")) ? "by their wrists":"passively"]!</span>", null, null, 5)
