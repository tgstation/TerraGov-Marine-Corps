/datum/body_marking_set
	...
	var/allowed_species = list(ALL_CUSTOMISH_SPECIES_BARRING_MOTH)
...
/datum/body_marking_set/vox
	allowed_species = list(ALL_CUSTOMISH_SPECIES_BARRING_MOTH)
...
/datum/body_marking_set/moth
	allowed_species = list(MOTH_SPECIES)
...
/proc/get_body_marking_sets_for_species(datum/species/pref_species, mismatched_parts = FALSE)
	var/list/candidates = GLOB.body_marking_sets.Copy()
	if(!mismatched_parts)
		for(var/name in candidates)
			var/datum/body_marking_set/BMS = GLOB.body_marking_sets[name]
			if(BMS.allowed_species && !(BMS.allowed_species[pref_species.name]))
				candidates -= name
	return candidates
