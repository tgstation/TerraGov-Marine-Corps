...
/mob/new_player/proc/handle_player_polling()
	if(!SSdbcore.IsConnected())
		...
		return
	var/datum/DBQuery/query_poll_get = SSdbcore.NewQuery("SELECT id, question FROM [format_table_name("poll_question")] WHERE Now() BETWEEN starttime AND endtime [check_rights(R_ADMIN, FALSE) ? "" : "AND adminonly = false"]")
...
/mob/new_player/proc/vote_valid_check(pollid, type)
	if(!SSdbcore.Connect())
	...
	var/datum/DBQuery/query_validate_poll = SSdbcore.NewQuery("SELECT id FROM [format_table_name("poll_question")] WHERE id = [pollid] AND Now() BETWEEN starttime AND endtime AND polltype = '[type]' [check_rights(R_ASAY, FALSE) ? "" : "AND adminonly = false"]")
...
/mob/new_player/proc/vote_on_irv_poll(pollid, list/votelist)
	...
	var/rank = "Player"
	if(check_rights(R_ASAY, FALSE))
		rank = client.holder.rank.name
	var/ckey = client.ckey
	var/address = client.address

	//validate the poll
	if(!vote_valid_check(pollid, POLLTYPE_IRV))
		return FALSE
...
/mob/new_player/proc/vote_on_poll(pollid, optionid)
	...
	if(!pollid || !optionid)
		return
	//validate the poll
	if(!vote_valid_check(pollid, POLLTYPE_OPTION))
		return FALSE
...
/mob/new_player/proc/log_text_poll_reply(pollid, replytext)
	...
	if(!pollid)
		return
	//validate the poll
	if(!vote_valid_check(pollid, POLLTYPE_TEXT))
		return FALSE
	if(!replytext)
...
/mob/new_player/proc/vote_on_numval_poll(pollid, optionid, rating)
	...
	if(!pollid || !optionid || !rating)
		return
	//validate the poll
	if(!vote_valid_check(pollid, POLLTYPE_RATING))
		return FALSE
...
/mob/new_player/proc/vote_on_multi_poll(pollid, optionid)
	...
	if(!pollid || !optionid)
		return TRUE
	//validate the poll
	if(!vote_valid_check(pollid, POLLTYPE_MULTI))
		return FALSE
	...
	if(!i)
		return 2
	var/adminrank = "Player"
	if(!QDELETED(client) && check_rights(R_ASAY, FALSE))
		adminrank = client.holder.rank.name
	adminrank = sanitizeSQL(adminrank)
