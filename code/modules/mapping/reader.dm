...
/datum/parsed_map/proc/build_coordinate(list/model, turf/crds, no_changeturf as num, placeOnTop as num, new_z)
		...
		if(!area_instance)
			...
			if (!area_instance)
				if(!ispath(area_type, /area))
					CRASH("Invalid area to load! area: [area_type], members: [json_encode(members)]")
	...
	if(members[index] != /turf/template_noop)
		...
		if(placeOnTop)
			instance = crds.PlaceOnTop(members[index], null, CHANGETURF_DEFER_CHANGE | (no_changeturf ? CHANGETURF_SKIP : NONE))
