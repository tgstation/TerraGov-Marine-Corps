...
/datum/tutorial/proc/keybinding_message(datum/keybinding/keybinding_type, message, message_without_keybinds)
	PROTECTED_PROC(TRUE)
	keybinding_type = GLOB.keybindings_by_name[initial(keybinding_type.name)]
	var/key = keybinding_type.get_keys_formatted(user.client)
	return key ? replacetext(message, "%KEY%", "<b>[key]</b>") : message_without_keybinds
/*
...
/datum/tutorial/proc/animate_ui_element(icon_state, initial_screen_loc, target_screen_loc, animate_start_time)
	...
	preview.icon = ui_style2icon(user.client?.prefs.ui_style) || UI_STYLES[1]
