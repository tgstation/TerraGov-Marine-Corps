...
/obj/machinery/power/smes/buildable/attackby(var/obj/item/W as obj, var/mob/user as mob)
	...
	if (..())
		...
		if(iscrowbar(W))
			...
			if (do_after(usr, 10 SECONDS * cur_coils, TRUE, src, USER_ICON_BUILD)) // More coils = takes longer to disassemble. It's complex so largest one with 5 coils will take 50s
