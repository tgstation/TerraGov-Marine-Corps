...
/obj/structure/light_construct
	...
	max_integrity = 200
	soft_armor = list("melee" = 50, "bullet" = 10, "laser" = 10, "energy" = 0, "bomb" = 0, "bio" = 0, "rad" = 0, "fire" = 80, "acid" = 50)
...
/obj/machinery/light/Initialize(mapload)
	...
	if(!mapload) //sync up nightshift lighting for player made lights
		...
		nightshift_enabled = FALSE // temp_apc?.nightshift_lights
...
/obj/machinery/light/update_icon_state()
	switch(status)		// set icon_states
		if(LIGHT_OK)
			var/area/A = get_area(src)
			if(emergency_mode || (A && A.flags_alarm_state == ALARM_WARNING_FIRE))
...
/obj/machinery/light/proc/update(trigger = TRUE)
	...
	if(on)
		...
		if(color)
			CO = color
		var/area/A = get_area(src)
		if (A && A.flags_alarm_state == ALARM_WARNING_FIRE)
...
/obj/machinery/light/attack_hand(mob/living/carbon/human/user)
	...
	if(on)
		...
		if(prot == 0)
			...
			O?.take_damage_limb(burn = 5, updating_health = TRUE)
...
// called when on fire
/obj/machinery/light/flamer_fire_act()
	return fire_act()
...
/obj/item/light/throw_impact(atom/hit_atom, speed)
...
/obj/item/light/Crossed(atom/movable/AM)
	...
	if(!isliving(AM))
		return
	var/mob/living/L = AM
	if(istype(L) && !(L.status_flags & INCORPOREAL) && !L.buckled)
		playsound(src, 'sound/effects/glass_step.ogg', 30, TRUE)
		if(L.m_intent == MOVE_INTENT_RUN && (status == LIGHT_BURNED || status == LIGHT_OK))
			shatter()
