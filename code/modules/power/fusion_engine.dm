...
/obj/machinery/power/fusion_engine/welder_act(mob/living/user, obj/item/O)
	...
	if(user.skills.getRating(SKILL_ENGINEER) < SKILL_ENGINEER_ENGI)
		...
		if(!do_after(user, fumbling_time, NONE, src, BUSY_ICON_UNSKILLED, extra_checks = CALLBACK(WT, PROC_REF(isOn))))
			return FALSE
	playsound(loc, 'sound/items/weldingtool_weld.ogg', 25)
	balloon_alert_to_viewers("Starts welding some damage")
	add_overlay(GLOB.welding_sparks)
	if(!do_after(user, 20 SECONDS - (user.skills.getRating(SKILL_ENGINEER) * 3 SECONDS) , NONE, src, BUSY_ICON_BUILD, extra_checks = CALLBACK(WT, PROC_REF(isOn))))
		return FALSE
