...
/obj/machinery/power/geothermal/attack_alien(mob/living/carbon/xenomorph/X, damage_amount = X.xeno_caste.melee_damage, damage_type = BRUTE, damage_flag = "", effects = TRUE, armor_penetration = 0, isrightclick = FALSE)
	...
	if(CHECK_BITFIELD(X.xeno_caste.caste_flags, CASTE_CAN_CORRUPT_GENERATOR) && is_corruptible)
		X.balloon_alert_to_viewers("[X] starts to corrupt [src]", ignored_mobs = X)
		X.balloon_alert(X, "You start to corrupt [src]")
		if(!do_after(X, 10 SECONDS, TRUE, src, BUSY_ICON_HOSTILE))
			return
		corrupt(X.hivenumber)
		X.balloon_alert_to_viewers("[X] has corrupted [src]", ignored_mobs = X)
	...
	X.balloon_alert(X, "We start tearing at [src]'s components")
...
/obj/machinery/power/geothermal/proc/interact_hand(mob/living/user)
	...
	if(!ishuman(user) && !issilicon(user))
		...
		return FALSE
	if(corrupted)
		user.balloon_alert(user, "You have to clean that generator first")
		return FALSE
	...
	if(buildstate == GEOTHERMAL_HEAVY_DAMAGE)
		...
		return FALSE
	if(is_on)
		src.balloon_alert_to_viewers("<b>[src]</b> beeps softly and the humming stops as the turbines shutdown")
		is_on = FALSE
		power_gen_percent = 0
		cur_tick = 0
		icon_state = "off"
