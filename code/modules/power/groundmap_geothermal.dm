...
/obj/machinery/power/geothermal/welder_act(mob/living/user, obj/item/I)
	...
	if(corrupted)
		...
		if(!do_after(user, 20 SECONDS, NONE, src, BUSY_ICON_BUILD, extra_checks = CALLBACK(WT, PROC_REF(isOn))))
	...
	if(user.skills.getRating(SKILL_ENGINEER) < SKILL_ENGINEER_ENGI)
		...
		var/fumbling_time = 10 SECONDS - 2 SECONDS * user.skills.getRating(SKILL_ENGINEER)
		if(!do_after(user, fumbling_time, NONE, src, BUSY_ICON_UNSKILLED, extra_checks = CALLBACK(WT, PROC_REF(isOn))) || buildstate != GEOTHERMAL_HEAVY_DAMAGE || is_on)
			return
	...
	if(!do_after(user, 20 SECONDS, NONE, src, BUSY_ICON_BUILD, extra_checks = CALLBACK(WT, PROC_REF(isOn))) || buildstate != GEOTHERMAL_HEAVY_DAMAGE || is_on)
		cut_overlay(GLOB.welding_sparks)
		return FALSE
