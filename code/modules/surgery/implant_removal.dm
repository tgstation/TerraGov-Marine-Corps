...
/datum/surgery_step/extract_implant/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	for(var/obj/item/O in target.implants)
		...
		break
	if(I)
		display_results(user, target, "<span class='notice'>I begin to extract [I] from [target]'s [target_zone]...</span>",
		...
		display_results(user, target, "<span class='notice'>I look for an implant in [target]'s [target_zone]...</span>",
...
/datum/surgery_step/extract_implant/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	if(I)
		display_results(user, target, "<span class='notice'>I successfully remove [I] from [target]'s [target_zone].</span>",
		...
		if(case && !case.imp)
			...
			I.forceMove(case)
			case.update_icon()
			display_results(user, target, "<span class='notice'>I place [I] into [case].</span>",
		...
		to_chat(user, "<span class='warning'>I can't find anything in [target]'s [target_zone]!</span>")
	return 1
