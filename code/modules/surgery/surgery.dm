...
proc/do_surgery(mob/living/carbon/M, mob/living/user, obj/item/tool)
	...
	if(user.action_busy) //already doing an action
		return 1
	if(!HAS_SKILL_LEVEL(user, SKILL_SURGERY, SKILL_LEVEL_PROFESSIONAL))
		...
		var/fumbling_time = SKILL_TASK_FORMIDABLE - SKILL_TASK_AVERAGE * D_GET_SKILL(user, SKILL_SURGERY) // 20 secs non-trained, 15 amateur, 10 semi-prof
	...
	for(var/i in GLOB.surgery_steps)
		...
		if(S.tool_quality(tool) && S.is_valid_target(M))
			...
			if(step_is_valid)
				...
				var/step_duration = max(rand(S.min_duration, S.max_duration) - 10 * GET_SKILL(user, SKILL_SURGERY), 5)
