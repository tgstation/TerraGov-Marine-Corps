...
/datum/surgery_step/brainwash/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	...
	if(!objective)
		return -1
	display_results(user, target, "<span class='notice'>I begin to brainwash [target]...</span>",
...
/datum/surgery_step/brainwash/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	...
	if(HAS_TRAIT(target, TRAIT_MINDSHIELD))
		to_chat(user, "<span class='warning'>I hear a faint buzzing from a device inside [target]'s brain, and the brainwashing is erased.</span>")
		return FALSE
	display_results(user, target, "<span class='notice'>I succeed in brainwashing [target].</span>",
	...
	to_chat(target, "<span class='danger'>A new compulsion fills your mind... you feel forced to obey it!</span>")
...
/datum/surgery_step/brainwash/failure(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	if(target.getorganslot(ORGAN_SLOT_BRAIN))
		display_results(user, target, "<span class='warning'>I screw up, bruising the brain tissue!</span>",
			"<span class='warning'>[user] screws up, causing brain damage!</span>",
			"<span class='notice'>[user] completes the surgery on [target]'s brain.</span>")
		target.adjustOrganLoss(ORGAN_SLOT_BRAIN, 40)
	else
		user.visible_message("<span class='warning'>[user] suddenly notices that the brain [user.p_they()] [user.p_were()] working on is not there anymore.</span>", "<span class='warning'>I suddenly notice that the brain you were working on is not there anymore.</span>")
	return FALSE
