...
/datum/surgery_step/limb/biotic/attach
...
/datum/surgery_step/limb/biotic/attach/can_use(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool, datum/limb/affected)
	if(..())
...
/datum/surgery_step/limb/biotic/attach/begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool, datum/limb/affected)
	user.visible_message(span_notice("[user] starts attaching \the [tool] where [target]'s [affected.display_name] used to be."), \
	span_notice("You start attaching \the [tool] where [target]'s [affected.display_name] used to be."))
	target.balloon_alert_to_viewers("Attaching...")

/datum/surgery_step/limb/biotic/attach/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool, datum/limb/affected)
	user.visible_message(span_notice("[user] has attached \the [tool] where [target]'s [affected.display_name] used to be."),	\
	span_notice("You have attached \the [tool] where [target]'s [affected.display_name] used to be."))
	target.balloon_alert_to_viewers("Success")
	...
	//Update our dear victim to have a limb again
	affected.rejuvenate()
	for(var/datum/limb/child_limb in affected.children)
		child_limb.rejuvenate()
...
/datum/surgery_step/limb/biotic/attach/fail_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool, datum/limb/affected)
	user.visible_message(span_warning("[user]'s hand slips, damaging flesh on [target]'s [affected.display_name]!"), \
	span_warning("Your hand slips, damaging flesh on [target]'s [affected.display_name]!"))
	target.balloon_alert_to_viewers("Slipped!")
	target.apply_damage(10, BRUTE, affected, 0, TRUE, updating_health = TRUE)
