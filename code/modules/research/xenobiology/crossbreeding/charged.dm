...
/obj/item/slimecross/charged/green/do_effect(mob/user)
	...
	if(!istype(H))
		to_chat(user, "<span class='warning'>I must be a humanoid to use this!</span>")
		return
	...
	if(!racechoice)
		to_chat(user, "<span class='notice'>I decide not to become a slime for now.</span>")
		return
...
/obj/item/slimecross/charged/black/do_effect(mob/user)
	var/mob/living/carbon/human/H = user
	if(!istype(H))
		to_chat(user, "<span class='warning'>I have to be able to have a species to get your species changed.</span>")
		return
	...
	if(changed)
		...
		to_chat(H, "<span class='danger'>I feel very different!</span>")
	..()
