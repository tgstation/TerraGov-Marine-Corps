...
/obj/item/slimecross/burning/blue/do_effect(mob/user)
	...
	for(var/mob/living/carbon/M in range(5, get_turf(user)))
		if(M != user)
			...
			to_chat(M, "<span class='danger'>I feel a chill run down your spine, and the floor feels a bit slippery with frost...</span>")
	..()
	...
	for(var/mob/living/M in range(4,get_turf(user)))
		if(M != user)
			...
			if(istype(C))
				...
				M.adjustFireLoss(25)
			to_chat(M, "<span class='danger'>I feel a sharp electrical pulse!</span>")
	..()
...
/obj/item/slimecross/burning/oil/do_effect(mob/user)
	user.visible_message("<span class='warning'>[user] activates [src]. It's going to explode!</span>", "<span class='danger'>I activate [src]. It crackles in anticipation</span>")
