...
/obj/machinery/r_n_d/organic_analyzer/attackby(var/obj/O as obj, var/mob/user as mob)
	...
	if (disabled)
		return
	if (!linked_console)
		to_chat(user, "<span class='warning'> The Weyland-Yutani Brand Organic Analyzer must be linked to an R&D console first!</span>")
		return
	if (busy)
		to_chat(user, "<span class='warning'> The Weyland-Yutani Brand Organic Analyzer is busy right now.</span>")
		return
