...
/datum/unit_test/binary_insert/Run()
	...
	TEST_ASSERT_EQUAL(length(nodes), 1, "List should have one node")
	...
	TEST_ASSERT_EQUAL(length(nodes), 2, "List should have two nodes")
	...
	TEST_ASSERT_EQUAL(length(nodes), 3, "List should have three nodes")
	TEST_ASSERT_EQUAL(nodes[1].x, 5, "The first node should be the one with 5")
	TEST_ASSERT_EQUAL(nodes[2].x, 10, "The second node should be the one with 10")
