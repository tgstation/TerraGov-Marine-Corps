...
/datum/unit_test/weed_ability/Run()
	...
	if(!weed.can_use_action())
		TEST_FAIL("Drone could not activate weed ability!")

	weed.action_activate()
	if(!(locate(/obj/alien/weeds/node) in drone.loc))
		TEST_FAIL("Weed action_activate did not spawn weeds under a drone!")
	drone.plasma_stored = 0
	if(weed.can_use_action())
		TEST_FAIL("Drone succeded in using weed ability while having no plasma!")
