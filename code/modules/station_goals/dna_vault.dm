...
/datum/station_goal/dna_vault/get_report()
	return {"Our long term prediction systems indicate a 99% chance of system-wide cataclysm in the near future.
	 We need you to construct a DNA Vault aboard my station.
...
/obj/machinery/dna_vault/proc/upgrade(mob/living/carbon/human/H,upgrade_type)
	...
	switch(upgrade_type)
		if(VAULT_TOXIN)
			to_chat(H, "<span class='notice'>I feel resistant to airborne toxins.</span>")
		...
		if(VAULT_NOBREATH)
			to_chat(H, "<span class='notice'>My lungs feel great.</span>")
			ADD_TRAIT(H, TRAIT_NOBREATH, "dna_vault")
		if(VAULT_FIREPROOF)
			to_chat(H, "<span class='notice'>I feel fireproof.</span>")
			S.burnmod = 0.5
		...
		if(VAULT_STUNTIME)
			...
			S.stunmod = 0.5
		if(VAULT_ARMOUR)
			to_chat(H, "<span class='notice'>I feel tough.</span>")
			S.armor = 30
			ADD_TRAIT(H, TRAIT_PIERCEIMMUNE, "dna_vault")
		if(VAULT_SPEED)
			to_chat(H, "<span class='notice'>My legs feel faster.</span>")
		...
		if(VAULT_QUICK)
			to_chat(H, "<span class='notice'>My arms move as fast as lightning.</span>")
