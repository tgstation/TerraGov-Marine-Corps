...
/obj/screen/text/screen_text/command_order
	...
	title_style = "<span class='maptext' style=font-size:24pt;text-align:center valign='top'><u>"
	title_close = "</u></span><br>"
...
/datum/action/innate/message_squad/action_activate()
	...
	if(human_owner.assigned_squad)
		for(var/mob/living/carbon/human/marine AS in human_owner.assigned_squad.marines_list)
			marine.play_screen_text(text, "SQUAD ORDERS UPDATED:", /obj/screen/text/screen_text/command_order)
		return
	for(var/mob/living/carbon/human/human AS in GLOB.alive_human_list)
		if(human.faction == human_owner.faction)
			human.play_screen_text(text, "ORDERS UPDATED:", /obj/screen/text/screen_text/command_order)
