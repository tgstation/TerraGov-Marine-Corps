	...
	if(on)
		if(bananium.get_material_amount(/datum/material/bananium) < 100)
			...
			if(!always_noslip)
				clothing_flags &= ~NOSLIP
			update_icon()
			to_chat(loc, "<span class='warning'>I ran out of bananium!</span>")
		else
...
/obj/item/clothing/shoes/clown_shoes/banana_shoes/attack_self(mob/user)
	...
	if(sheet_amount)
		to_chat(user, "<span class='notice'>I retrieve [sheet_amount] sheets of bananium from the prototype shoes.</span>")
	else
		to_chat(user, "<span class='warning'>I cannot retrieve any bananium from the prototype shoes!</span>")
...
/obj/item/clothing/shoes/clown_shoes/banana_shoes/ui_action_click(mob/user)
	...
	if(bananium.get_material_amount(/datum/material/bananium))
		on = !on
		update_icon()
		to_chat(user, "<span class='notice'>I [on ? "activate" : "deactivate"] the prototype shoes.</span>")
		if(!always_noslip)
			if(on)
				clothing_flags |= NOSLIP
			else
		...
		to_chat(user, "<span class='warning'>I need bananium to turn the prototype shoes on!</span>")
