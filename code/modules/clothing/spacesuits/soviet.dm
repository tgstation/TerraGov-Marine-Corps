...
/datum/component/armor_protection_limb/proc/on_equip(mob/equipper)
	RegisterSignal(equipper, COMSIG_ATOM_BULLET_ACT, .proc/on_projectile)

/datum/component/armor_protection_limb/proc/on_unequip(mob/unequipper, slot)
	if(isitem(parent))
...
/datum/component/armor_protection_limb/proc/on_projectile(obj/projectile/proj)
	if(proj
...
/datum/component/armor_protection_limb/proc/adjust_health(var/amount)
	...
	if(health <= 0)
		if(!CHECK_BITFIELD(venting, VENTING))
