...
/world/Error(exception/E, datum/e_src)
	...
	if(copytext_char(E.name, 1, 32) == "Maximum recursion level reached")//32 == length_char() of that string + 1
	...
	else if(copytext_char(E.name, 1, 18) == "Out of resources!")//18 == length_char() of that string + 1
	...
	if(LAZYLEN(splitlines) > ERROR_USEFUL_LEN) // If there aren't at least three lines, there's no info
		for(var/line in splitlines)
			...
			if(copytext_char(line, 1, 3) != "  ")//3 == length_char("  ") + 1
