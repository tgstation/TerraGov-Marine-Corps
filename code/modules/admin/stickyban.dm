/datum/admins/proc/stickyban(action, data)
	...
	switch(action)
		...
		if("add")
			...
			message_admins("<span class='adminnotice'>[ADMIN_TPMONTY(usr)] has stickybanned [ckey].\nReason: [ban["message"]]</span>")
		...
		if("remove")
			...
			message_admins("<span class='adminnotice'>[ADMIN_TPMONTY(usr)] removed [ckey]'s stickyban.</span>")
		...
		if("remove_alt")
			...
			message_admins("<span class='adminnotice'>[ADMIN_TPMONTY(usr)] has disassociated [alt] from [ckey]'s sticky ban.</span>")
		...
		if("edit")
			...
			message_admins("<span class='warning'>[ADMIN_TPMONTY(usr)] has edited [ckey]'s sticky ban reason from [oldreason] to [reason]</span>")
		...
		if ("exempt")
			...
			if (!key)
				to_chat(usr, "<span class='warning'>Error: [alt]'s link to [ckey]'s sticky ban disappeared.</span>")
				return
			LAZYREMOVE(ban["keys"], alt)
			key["exempt"] = TRUE
			...
			message_admins("<span class='warning'>[ADMIN_TPMONTY(usr)] has exempted [alt] from [ckey]'s sticky ban.</span>")
		...
		if ("unexempt")
			...
			if (!key)
				to_chat(usr, "<span class='warning'>Error: [alt] is not exempt from [ckey]'s sticky ban!</span>")
				return
			...
			if (!key)
				to_chat(usr, "<span class='warning'>Error: [alt]'s exemption from [ckey]'s sticky ban disappeared.</span>")
				return
			...
			message_admins("<span class='warning'>[ADMIN_TPMONTY(usr)] has unexempted [alt] from [ckey]'s sticky ban.</span>")

		if ("timeout")
			if (!data["ckey"])
				return
			if (!SSdbcore.Connect())
				to_chat(usr, "<span class='warning'>No database connection!</span>")
				return
			...
			message_admins("<span class='warning'>[ADMIN_TPMONTY(usr)] has put [ckey]'s sticky ban on timeout.</span>")
		...
		if ("untimeout")
			...
			message_admins("<span class='warning'>[ADMIN_TPMONTY(usr)] has taken [ckey]'s sticky ban off of timeout.</span>")
		...
		if("revert")
			...
			message_admins("<span class='adminnotice'>[ADMIN_TPMONTY(usr)] has reverted [ckey]'s sticky ban to its state at round start.</span>")
