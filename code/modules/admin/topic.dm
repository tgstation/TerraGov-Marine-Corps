	...
	if(href_list["ahelp"])
		...
		if(isliving(M))
			...
			switch(L.stat)
				...
				if(DEAD)
					status = "Dead"
			health = "Oxy: [L.getOxyLoss()]  Tox: [L.getToxLoss()]  Fire: [L.getFireLoss()]  Brute: [L.getBruteLoss()]  Clone: [L.getCloneLoss()]  Brain: [L.getBrainLoss()]  Stamina: [L.getStaminaLoss()] Integrity: [L.getIntegrityLoss()]"
