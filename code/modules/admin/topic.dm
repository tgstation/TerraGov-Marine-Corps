...
<span class='admin'><span class='message'>[ADMIN_FULLMONTY(M)]</span></span><hr></span>"})
	...
	if(href_list["evac_authority"])
		...
		if(!istype(H))
			to_chat(usr,"<span class='warning'>Human is no longer eligible for changing squads.</span>")
			return
		...
		if(!istype(L))
			to_chat(usr,"<span class='warning'>Target is no longer valid for toggle sleep.</span>")
			return
		...
		if(!istype(C))
			to_chat(usr,"<span class='warning'>The mob has no client currently associated.</span>")
