...
/proc/Centcomm_announce(var/text , var/mob/sender , var/iamessage)
	...
	for(var/client/C in GLOB.admins)
		if(check_other_rights(C, R_ADMIN, FALSE))
			to_chat(C, "<span class='notice'><b><font color=orange>TGMC:</font>[ADMIN_FULLMONTY(usr)] (<a HREF='?src=[REF(C.holder)];[HrefToken(TRUE)];reply=[REF(sender)]'>REPLY</a>):</b> [text]</span>")
