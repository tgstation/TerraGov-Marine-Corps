...
/datum/admins/proc/shutdown_server()
	...
	if(shuttingdown)
		if(tgui_alert(usr, "Are you use you want to cancel the shutdown initiated by [shuttingdown]?", "Cancel the shutdown?", list("Yes, cancel it", "No"), 0) != "Yes, cancel it")
			return
	...
	if(tgui_alert(usr, "ARE YOU SURE YOU WANT TO SHUTDOWN THE SERVER? ONLY SOMEBODY WITH REMOTE ACCESS TO THE SERVER CAN TURN IT BACK ON.", "Shutdown Server?", list("Shutdown The Server!", "Cancel"), 0) != "Shutdown The Server!")
...
/client/proc/toggle_cdn()
	...
	if(!current_transport || current_transport == "simple")
		if(admin_disabled_cdn_transport)
			...
			if(tgui_alert(usr, "The CDN asset transport is not enabled! If you having issues with assets you can also try disabling filename mutations.", "The CDN asset transport is not enabled!", list("Disable", "Nevermind"), 0) == "Disable")
