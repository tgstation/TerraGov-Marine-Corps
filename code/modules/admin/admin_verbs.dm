/datum/admins/proc/admin_ghost()
	...
	if(istype(mob, /mob/dead/observer))
		...
		to_chat(src, "<font color='warning'>Can't admin ghost while in the lobby.</font>")
	else
...
/datum/admins/proc/invisimin()
	...
	log_admin("[key_name(usr)] has turned invisimin [(mob.invisibility == INVISIBILITY_OBSERVER) ? "on" : "off"].")
...
/datum/admins/proc/jobs_list()
	...
	log_admin("[key_name(usr)] has checked job slots.")
...
/datum/admins/proc/change_key(mob/M in living_mob_list)
	...
	log_admin("[key_name(usr)] changed [M.name]'s ckey to [new_ckey].")
...
/datum/admins/proc/rejuvenate(mob/living/M as mob in mob_list)
	...
	if(!M || !istype(M) || !CONFIG_GET(flag/allow_admin_rev))
		return
