...
/datum/admins/proc/offer(mob/living/L in GLOB.mob_living_list)
	...
	if(!istype(L))
		to_chat(usr,"<span class='warning'>Selected mob is no longer valid to offer.</span>")
		return
...
/datum/admins/proc/change_hivenumber(mob/living/carbon/Xenomorph/X in GLOB.xeno_mob_list)
	...
	if(!istype(X))
		to_chat(usr,"<span class='warning'>Selected xeno is no longer valid.</span>")
		return
	if(X.hivenumber != hivenumber_status)
		to_chat(usr,"<span class='warning'>The hivenumber has changed since we started. Aborting.</span>")
		return
