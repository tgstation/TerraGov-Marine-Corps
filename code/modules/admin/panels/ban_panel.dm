	...
	if(edit_id)
		...
		for(var/department in headless_job_lists)
			...
			for(var/job in headless_job_lists[department])
				...
				break_counter++
			output += "</div></div>"
		var/list/long_job_lists = list("Antagonist Positions" = list(ROLE_XENOMORPH, ROLE_QUEEN, ROLE_SURVIVOR))
...
/datum/admins/proc/sticky_ban_panel()
	...
	<body>
		<h2>All Sticky Bans:</h2> <a href='?_src_=[usr.client.holder];[HrefToken()];stickyban=add'>\[+\]</a><br>
		[banhtml]
	</body>
	"}
