	...
	if(length(ticker.mode.xenomorphs))
		...
		for(var/datum/mind/L in ticker.mode.xenomorphs)
			...
			if(M)
				...
				dat += "<tr><td><a href='?priv_msg=[REF(M)]'>[M.real_name]</a>[M.client ? "" : " <i>(logged out)</i>"][M.client.prefs?.xeno_name != "Undefined" ? M.client.prefs.xeno_name : ""][M.stat == DEAD ? " <b><font color=red>(DEAD)</font></b>" : ""]</td>"
				dat += "<td>[location]</td>"
