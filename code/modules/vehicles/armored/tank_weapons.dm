...
/obj/vehicle/armored/proc/handle_fire_main(atom/A) //This is used to shoot your big ass tank cannon, rather than your small MG
	...
	firing_target = A
	RegisterSignal(gunner, COMSIG_MOB_MOUSEUP, /obj/vehicle/armored.proc/on_mouse_up)
...
/obj/vehicle/armored/process()
	if(firing_target)
		if(primary_weapon.fire(firing_target, gunner))
			turret_overlay.setDir(angle2dir_cardinal(Get_Angle(src, firing_target))) //For those instances when you've swivelled your gun round a lot when your main gun wasn't ready to fire. This ensures the gun always faces the desired target.
		else
			stop_firing()
	else
...
/obj/vehicle/armored/proc/on_mouse_up(mob/user, atom/A, targetturf, control, params)
	SIGNAL_HANDLER
	stop_firing()
