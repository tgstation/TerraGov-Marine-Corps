...
/obj/item/armored_weapon/proc/begin_fire(mob/source, atom/target, list/modifiers)
	...
	if(windup_delay && windup_checked == WEAPON_WINDUP_NOT_CHECKED)
		...
		if(!do_after(source, windup_delay, IGNORE_TARGET_LOC_CHANGE|IGNORE_LOC_CHANGE, chassis, BUSY_ICON_DANGER, BUSY_ICON_DANGER, extra_checks = CALLBACK(src, PROC_REF(do_after_checks), current_target)))
