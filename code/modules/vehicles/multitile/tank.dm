...
/obj/vehicle/multitile/root/cm_armored/tank/handle_player_entrance(mob/living/carbon/M)
	...
	if(offhand && !(HAS_TRAIT(offhand, TRAIT_NODROP) || (offhand.item_flags & (DELONDROP|ITEM_ABSTRACT))))
	...
	if(M.skills.getRating(SKILL_LARGE_VEHICLE) < SKILL_LARGE_VEHICLE_TRAINED)
		...
		if(!do_after(M, fumbling_time, TRUE, src, BUSY_ICON_UNSKILLED) || \
			(offhand && !(HAS_TRAIT(offhand, TRAIT_NODROP) || (offhand.item_flags & DELONDROP))))
			return
