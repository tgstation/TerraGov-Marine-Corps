...
/obj/vehicle/ridden/scooter/wrench_act(mob/living/user, obj/item/I)
	..()
	to_chat(user, "<span class='notice'>I begin to remove the handlebars...</span>")
	if(I.use_tool(src, user, 40, volume=50))
		var/obj/vehicle/ridden/scooter/skateboard/S = new(drop_location())
		new /obj/item/stack/rods(drop_location(), 2)
		to_chat(user, "<span class='notice'>I remove the handlebars from [src].</span>")
		if(has_buckled_mobs())
...
/obj/vehicle/ridden/scooter/buckle_mob(mob/living/M, force = FALSE, check_loc = TRUE)
	...
	if(M.get_num_legs() < legs_required && M.get_num_arms() < arms_required)
		to_chat(M, "<span class='warning'>I don't think it'd be a good idea trying to ride \the [src]...</span>")
		return FALSE
	return ..()
...
/obj/vehicle/ridden/scooter/skateboard/MouseDrop(atom/over_object)
	...
	if(has_buckled_mobs() && over_object == M)
		to_chat(M, "<span class='warning'>I can't lift this up when somebody's on it.</span>")
		return
	if(over_object == M)
...
/obj/item/scooter_frame/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/stack/sheet/metal))
		if(!I.tool_start_check(user, amount=5))
			return
		to_chat(user, "<span class='notice'>I begin to add wheels to [src].</span>")
		if(I.use_tool(src, user, 80, volume=50, amount=5))
			to_chat(user, "<span class='notice'>I finish making wheels for [src].</span>")
...
/obj/vehicle/ridden/scooter/skateboard/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/stack/rods))
		if(!I.tool_start_check(user, amount=2))
			return
		to_chat(user, "<span class='notice'>I begin making handlebars for [src].</span>")
		if(I.use_tool(src, user, 25, volume=50, amount=2))
			to_chat(user, "<span class='notice'>I add the rods to [src], creating handlebars.</span>")
...
/obj/vehicle/ridden/scooter/skateboard/screwdriver_act(mob/living/user, obj/item/I)
	if(..())
		return TRUE
	to_chat(user, "<span class='notice'>I begin to deconstruct and remove the wheels on [src]...</span>")
	if(I.use_tool(src, user, 20, volume=50))
		to_chat(user, "<span class='notice'>I deconstruct the wheels on [src].</span>")
...
/obj/vehicle/ridden/scooter/wheelys/post_unbuckle_mob(mob/living/M)
	if(!has_buckled_mobs())
		to_chat(M, "<span class='notice'>I pop the Wheely-Heels' wheels back into place.</span>")
		moveToNullspace()
	return ..()

/obj/vehicle/ridden/scooter/wheelys/post_buckle_mob(mob/living/M)
	to_chat(M, "<span class='notice'>I pop out the Wheely-Heels' wheels.</span>")
	return ..()
