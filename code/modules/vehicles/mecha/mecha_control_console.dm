...
/obj/machinery/computer/mecha/ui_act(action, params)
	...
	switch(action)
		...
		if("shock")
			...
			if(M)
				MT.shock()
				log_game("[logdetails(usr)] has activated remote EMP on exosuit [logdetails(M)], which is currently [LAZYLEN(M.occupants) ? "occupied by [M.occupants.Join(",")][ADMIN_FLW(M)]." : "without a pilot."]")
