	...
	if(beaker)
		if (href_list["analyze"])
			...
			while (count--)
				...
				if(loaded_pill_bottle)
					if(length(loaded_pill_bottle.contents) < loaded_pill_bottle.atom_storage.max_storage_space)
						loaded_pill_bottle.atom_storage.handle_item_insertion(P, TRUE)
						updateUsrDialog()
...
/obj/machinery/chem_master/interact(mob/user)
	...
	if(!beaker)
		...
		if(loaded_pill_bottle)
			dat += "<A href='?src=[text_ref(src)];ejectp=1'>Eject Pill Bottle \[[length(loaded_pill_bottle.contents)]/[loaded_pill_bottle.atom_storage.max_storage_space]\]</A><BR><BR>"
		else
