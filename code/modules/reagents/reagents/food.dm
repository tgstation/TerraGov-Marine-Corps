...
/datum/reagent/consumable/on_mob_life(mob/living/L, metabolism)
	...
	if(iscarbon(L))
		...
		C.adjust_nutrition(nutriment_factor*0.5*rem)
	if(adj_temp)
...
/datum/reagent/consumable/honey/on_mob_life(mob/living/L, metabolism)
	...
	L.adjustBruteLoss(-0,2.5*rem)
	L.adjustFireLoss(-0,2.5*rem)
	L.adjustOxyLoss(-0,2.5*rem)
	L.adjustToxLoss(-0,2.5*rem)
	return ..()
