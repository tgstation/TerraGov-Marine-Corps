...
/datum/reagent/toxin/on_mob_life(mob/living/L, metabolism)
	if(toxpwr)
		L.adjustToxLoss(toxpwr*0.5*rem)
	return ..()
...
/datum/reagent/toxin/phoron/on_mob_life(mob/living/L, metabolism)
	holder.remove_reagent(/datum/reagent/medicine/inaprovaline, rem)
	return ..()
...
/datum/reagent/toxin/chloralhydrate/on_mob_life(mob/living/L, metabolism)
	switch(current_cycle)
		...
		if(61 to INFINITY)
			L.Sleeping(10 SECONDS)
			L.adjustToxLoss((current_cycle/4 - 25)*rem)
	return ..()
...
/datum/reagent/toxin/beer2/on_mob_life(mob/living/L, metabolism)
	switch(current_cycle)
		...
		if(51 to INFINITY)
			L.Sleeping(10 SECONDS)
			L.adjustToxLoss((current_cycle/2 - 25)*rem)
	return ..()
...
/datum/reagent/toxin/nanites/on_mob_life(mob/living/L, metabolism)
	L.apply_damages(5*rem, 3*rem, 3*rem) //DO NOT DRINK THIS. Seriously!
	L.blood_volume -= 10
	return ..()
...
/datum/reagent/toxin/xeno_neurotoxin/on_mob_life(mob/living/L, metabolism)
	switch(current_cycle)
		if(1 to 20)
			L.adjustStaminaLoss(2*rem) //While stamina loss is going, stamina regen apparently doesn't happen, so I can keep this smaller.
			L.reagent_pain_modifier -= PAIN_REDUCTION_LIGHT
		if(21 to 45)
			L.adjustStaminaLoss(6*rem)
			L.reagent_pain_modifier -= PAIN_REDUCTION_HEAVY
		if(46 to INFINITY)
...
/datum/reagent/toxin/xeno_neurotoxin/overdose_process(mob/living/L, metabolism)
	L.adjustToxLoss(0.5*rem) //Overdose starts applying toxin and oxygen damage. Long-term overdose will kill the host.
