...
/obj/machinery/transport/crossing_signal/examine(mob/user)
	. = ..()
	. += span_notice("The maintenance panel is [machine_stat & PANEL_OPEN ? "open" : "closed"].")
...
/obj/machinery/transport/crossing_signal/click_alt(mob/living/user)
	...
	if(!(machine_stat & PANEL_OPEN) || tool?.tool_behaviour != TOOL_WRENCH)
...
/obj/machinery/transport/crossing_signal/attackby_secondary(obj/item/weapon, mob/user, params)
	...
	if(weapon.tool_behaviour == TOOL_WRENCH && machine_stat & PANEL_OPEN)
		switch(sign_dir)
			if(INBOUND)
				sign_dir = OUTBOUND
			if(OUTBOUND)
...
/obj/machinery/transport/guideway_sensor/proc/pair_sensor()
	...
	playsound(src, 'sound/machines/synth/synth_yes.ogg', 75, vary = FALSE)
...
/obj/machinery/transport/guideway_sensor/Destroy()
	...
	if(paired_sensor)
		...
		divorcee.paired_sensor = null
		divorcee.update_appearance()
		playsound(src, 'sound/machines/synth/synth_no.ogg', 75, vary = FALSE)
		paired_sensor = null
	. = ..()
