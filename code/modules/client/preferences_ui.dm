...
/datum/preferences/ui_interact(mob/user, ui_key = "main", datum/tgui/ui = null, force_open = FALSE, datum/tgui/master_ui = null, datum/ui_state/state = GLOB.always_state)
	ui = SStgui.try_update_ui(user, src, ui_key, ui, force_open)
	if(!ui)
		user.client.register_map_obj(screen_main)
		user.client.register_map_obj(screen_bg)

		ui = new(user, src, ui_key, "PlayerPreferences", "Preferences", 1200, 960, master_ui, state)
