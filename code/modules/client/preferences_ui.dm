...
/datum/preferences/ui_data(mob/user)
	...
	switch(tab_index)
		...
		if(GAME_SETTINGS)
			...
			.["quick_equip"] = slot_flag_to_fluff(quick_equip)
		if(KEYBIND_SETTINGS)
...
/datum/preferences/ui_act(action, list/params, datum/tgui/ui)
	...
	switch(action)
		...
		if("select_quick_equip_1")
			var/slot = tgui_input_list(usr, "Which slot would you like to draw/equip from?", "Preferred Slot", SLOT_FLUFF_DRAW)
			quick_equip[0] = slot_fluff_to_flag(slot)
			to_chat(src, span_notice("You will now equip/draw from the [slot] slot first."))

		if("select_quick_equip_2")
			var/slot = tgui_input_list(usr, "Which slot would you like to draw/equip from?", "Preferred Slot", SLOT_FLUFF_DRAW)
			quick_equip[1] = slot_fluff_to_flag(slot)
			to_chat(src, span_notice("You will now equip/draw from the [slot] slot first."))

		if("select_quick_equip_3")
			var/slot = tgui_input_list(usr, "Which slot would you like to draw/equip from?", "Preferred Slot", SLOT_FLUFF_DRAW)
			quick_equip[2] = slot_fluff_to_flag(slot)
			to_chat(src, span_notice("You will now equip/draw from the [slot] slot first."))

		if("select_quick_equip_4")
			var/slot = tgui_input_list(usr, "Which slot would you like to draw/equip from?", "Preferred Slot", SLOT_FLUFF_DRAW)
			quick_equip[3] = slot_fluff_to_flag(slot)
			to_chat(src, span_notice("You will now equip/draw from the [slot] slot first."))

		if("select_quick_equip_5")
			var/slot = tgui_input_list(usr, "Which slot would you like to draw/equip from?", "Preferred Slot", SLOT_FLUFF_DRAW)
			quick_equip[4] = slot_fluff_to_flag(slot)
			to_chat(src, span_notice("You will now equip/draw from the [slot] slot first."))
