...
/obj/item/paper/tutorial/mechanics
	...
	info = {"Up to date as of December 27th '22.<BR>
	<BR>
	Armors fall into three categories: Light, Medium, and Heavy. Other than that, all differences are purely cosmetic. Light Scout Jaeger is identical to Light Skirmisher Jaeger, which is identical to Xenonauten-L. Additional, for Jaeger armors, only the chestplace differs in armor provided for the three aforementioned categories; arm and leg plates all offer the same level of protection, so you're free to change them based on aesthetics. Just don't forget to actually put one on.<BR>
