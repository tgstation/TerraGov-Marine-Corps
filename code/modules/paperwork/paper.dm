...
/obj/item/weapon/paper/prison_station/test_log
	name = "paper- 'Test Log'"
	info = "<p style=\"text-align: center;\"><sub>TEST LOG</sub></p><p>SPECIMEN: Bioweapon candidate Kappa. Individual 3</p><p>PROCEDURE: Observation<BR>\nRESULTS: Specimen paces around cell. Appears agitated. Vocalisations.</p><p>PROCEDURE: Simian test subject<BR>\nRESULTS: Devoured by specimen. No significant difference from last simian test.<BR>\n</p><p><em>Note: Consistent with previous results. Time to amp it up</em></p><p>PROCEDURE: Human test subject (D-1). Instructed to \"pet it like a dog\"<BR>\nRESULTS: Specimen and D-1 stare at each other for approximately 2 seconds. D-1 screams and begins pounding on observation window begging to be released. Specimen pounces on D-1, then kills him with multiple slashes from its foreclaws.</p><p><em>Note: Promising!</em></p><p>PROCEDURE: Two human test subjects (D-2, D-3). Instructed to subdue specimen<BR>\nRESULTS: D-2 and D-3 slowly approach specimen. D-3 punches specimen on forehead to no noticeable effect. Specimen pounces on D-3, then kills him with multiple slashes from its foreclaws. D-2 screams and begins pounding on observation window. Specimen pounces on D-2, then kills him with multiple slashes from its foreclaws.<BR>\nSpecimen begins slashing at observation access doors. Exhibiting an unexpected amount of strength, it is able to d</p>"

/obj/item/weapon/paper/prison_station/warden_note
	name = "paper- 'Final note'"
	info = "It is clear now that this far worse than a simple pirate attack. Half the guard is confirmed dead, and most of the other half are unaccounted for. Prisoner control would be an issue if there were any prisoners left.<BR>\n<BR>\nI have activated the central lockdown, which will hopefully keep the bulk of them at bay. Those trapped in the yard are now beyond our help.<BR>\n<BR>\nIf they reach here regardless, whoever finds this should know that the blast doors to the central ring corridor will open automatically at 12:45. The blast doors to the low-security cellblocks and yard must be toggled manually from the low-security monitoring r<BR>\n<BR>\nsomething comes"

/obj/item/weapon/paper/prison_station/chapel_program
