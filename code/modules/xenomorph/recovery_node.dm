/obj/structure/xeno/recovery_node
	name = "recovery node"
...
/obj/structure/xeno/recovery_node/Initialize(mapload, _hivenumber)
	. = ..()
	SSminimaps.add_marker(src, MINIMAP_FLAG_XENO, image('icons/UI_icons/map_blips.dmi', null, "recovery", ABOVE_FLOAT_LAYER))
	GLOB.hive_datums[hivenumber].recovery_nodes += src
	create_effects()

/obj/structure/xeno/recovery_node/Destroy()
	GLOB.hive_datums[hivenumber].recovery_nodes -= src
	delete_effects()
	return ..()

/obj/structure/xeno/recovery_node/on_changed_z_level(turf/old_turf, turf/new_turf, notify_contents = TRUE)
	. = ..()
	delete_effects()
	create_effects()
...
/obj/structure/xeno/recovery_node/proc/apply_buff(datum/source, atom/movable/entering_movable, atom/old_loc)
	SIGNAL_HANDLER
...
/obj/structure/xeno/recovery_node/proc/remove_buff(datum/source, atom/movable/leaving_movable, direction)
	SIGNAL_HANDLER
	if(direction) // No need to change anything if they are staying within range.
		...
		if(HAS_TRAIT(leaving_to_turf, TRAIT_RECOVERY_NODE_TURF))
			return
...
/obj/structure/xeno/recovery_node/proc/create_effects()
