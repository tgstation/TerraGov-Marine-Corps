...
/proc/curse_of_madness(mob/user, message)
	if(user) //in this case either someone holding a spellbook or a badmin
		to_chat(user, "<span class='warning'>I sent a curse of madness with the message \"[message]\"!</span>")
	...
	for(var/mob/living/carbon/human/H in GLOB.player_list)
		...
		if(H.anti_magic_check(TRUE, FALSE, TRUE))
			to_chat(H, "<span class='notice'>I have a strange feeling for a moment, but then it passes.</span>")
			continue
...
/proc/give_madness(mob/living/carbon/human/H, message)
	...
	to_chat(H, "<span class='warning'>My mind shatters!</span>")
	switch(rand(1,10))
		if(1 to 3)
