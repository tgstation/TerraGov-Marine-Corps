...
#define PROC_HOLDER_ACTIVE      (1<<0)
#define PROC_HOLDER_HAS_ACTION  (1<<1)

/obj/effect/proc_holder
	var/panel = "Debug"//What panel the proc holder needs to go on.
	var/proc_holder_flags = PROC_HOLDER_HAS_ACTION
...
/obj/effect/proc_holder/Initialize()
	. = ..()
	if(CHECK_BITFIELD(proc_holder_flags, PROC_HOLDER_HAS_ACTION))
...
/obj/effect/proc_holder/proc/remove_ranged_ability(msg)
	if(!ranged_ability_user?.client || (ranged_ability_user.ranged_ability && ranged_ability_user.ranged_ability != src)) //To avoid removing the wrong ability
		return
...
#define SPELL_RECHARGING        (1<<0)
#define SPELL_CLOTHES_REQ       (1<<1) //See if it requires clothes
#define SPELL_CULT_REQ          (1<<2) //SPECIAL SNOWFLAKE clothes required for cult only spells
#define SPELL_HUMAN_REQ         (1<<3) //Spell can only be cast by humans
#define SPELL_CORPOREAL_REQ     (1<<4) //Spell can only be cast by mobs that are physical entities
#define SPELL_STAT_ALLOWED      (1<<5) //See if it requires being conscious/alive, needs to be enabled for ghostspells
#define SPELL_PHASE_ALLOWED     (1<<6) //If enabled, the spell can be cast while phased, eg. blood crawling, ethereal jaunting
#define SPELL_ANTIMAGIC_ALLOWED (1<<7) //If disabled, the spell cannot be cast while under the effect of antimagic
#define SPELL_PLAYER_LOCK       (1<<8) //If it can be used by simple mobs
...
/obj/effect/proc_holder/spell
	...
	var/spell_flags = (SPELL_RECHARGING | SPELL_CLOTHES_REQ | SPELL_PLAYER_LOCK)
...
/obj/effect/proc_holder/spell/proc/cast_check(skipcharge = FALSE, mob/user = usr) //checks if the spell can be cast based on its settings; skipcharge is used when an additional cast_check is called inside the spell
	if(CHECK_BITFIELD(spell_flags, SPELL_PLAYER_LOCK))
		if(!user.mind || !(src in user.mind.spell_list) && !(src in user.mob_spell_list))
			to_chat(user, "<span class='warning'>You shouldn't have this spell! Something's wrong.</span>")
			stack_trace("[user] triggered cast_check() on [src] without having the spell.")
			return FALSE
	else
	...
	if(!CHECK_BITFIELD(spell_flags, SPELL_ANTIMAGIC_ALLOWED))
	...
	if(ishuman(user))
		...
		if(CHECK_BITFIELD(spell_flags, SPELL_CLOTHES_REQ)) //clothes check
			...
			if(!is_type_in_typecache(H.head, casting_clothes))
				...
				return FALSE
		if(CHECK_BITFIELD(spell_flags, SPELL_CULT_REQ)) //CULT_REQ CLOTHES CHECK
		...
		if(CHECK_BITFIELD(spell_flags, SPELL_CLOTHES_REQ) || CHECK_BITFIELD(spell_flags, SPELL_HUMAN_REQ))
			to_chat(user, "<span class='notice'>This spell can only be cast by humans!</span>")
			return FALSE
		if(CHECK_BITFIELD(spell_flags, SPELL_CORPOREAL_REQ) && (isbrain(user) || ispAI(user)))
...
/obj/effect/proc_holder/spell/proc/start_recharge()
	ENABLE_BITFIELD(spell_flags, SPELL_RECHARGING)
...
#define SPELL_TARGETED_INCLUDE_USER     (1<<0)
#define SPELL_TARGETED_RANDOM_TARGET    (1<<1)
...
/obj/effect/proc_holder/spell/targeted/choose_targets(mob/user = usr)
		...
		if(1) //single target can be picked
			if(range < 0)
				...
				for(var/mob/living/M in view_or_range(range, user, selection_type))
					if(user == M && !CHECK_BITFLAG(spell_targeted_flags, SPELL_TARGETED_INCLUDE_USER))
						continue
					if(!can_target(M))
						continue
					possible_targets += M
				...
				var/mob/M
				if(!CHECK_BITFLAG(spell_targeted_flags, SPELL_TARGETED_RANDOM_TARGET))
					...
					switch(random_target_priority)
						...
						if(TARGET_CLOSEST)
							for(var/mob/living/L in possible_targets)
								if(M)
									if(get_dist(user, L) >= get_dist(user, M) || !los_check(user,L))
										continue
									M = L
								else
									if(!los_check(user, L))
	...
	if(!CHECK_BITFLAG(spell_targeted_flags, SPELL_TARGETED_INCLUDE_USER) && (user in targets))
		targets -= user
...
/obj/effect/proc_holder/spell/targeted/proc/los_check(mob/A, mob/B)
	...
	var/los_check_result = TRUE
	for(var/turf/turf in getline(A,B))
		for(var/atom/movable/AM in turf)
			if(AM.CanPass(dummy, turf))
				continue
			los_result = FALSE
	qdel(dummy)
	return los_check_result
...
/obj/effect/proc_holder/spell/proc/can_cast(mob/user = usr)
	...
	if(user.stat && !CHECK_BITFIELD(spell_flags, SPELL_STAT_ALLOWED))
		return FALSE

	if(!CHECK_BITFIELD(spell_flags, SPELL_ANTIMAGIC_ALLOWED) && user.anti_magic_check(TRUE, FALSE, major = FALSE, self = TRUE))
		return FALSE
...
/obj/effect/proc_holder/spell/self/basic_heal //This spell exists mainly for debugging purposes, and also to show how casting works
	...
	spell_flags = (SPELL_RECHARGING | SPELL_PLAYER_LOCK | SPELL_HUMAN_REQ)
	charge_max = 100
	cooldown_min = 50
