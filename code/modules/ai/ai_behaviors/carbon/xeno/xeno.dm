...
/datum/ai_mind/carbon/xeno/New(loc, parent_to_assign)
	..()
...
/datum/ai_mind/carbon/xeno/get_new_state(reason_for)
	...
	switch(reason_for)
		if(REASON_TARGET_KILLED, REASON_FINISHED_NODE_MOVE, REASON_TARGET_SPOTTED, REASON_REFRESH_TARGET)
			...
			if(!favorable_target)
				reason_for = REASON_TARGET_KILLED //Probably no more targets nearby
				return ..()
