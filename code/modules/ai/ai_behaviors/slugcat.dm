...
// /datum/ai_behavior/slugcat/register_action_signals(action_type)
// 	. = ..()
// 	switch(action_type)
// 		if(MOVING_TO_ATOM)
// 			target
...
/datum/ai_behavior/slugcat/proc/handle_mob_master_speech(datum/source, message, atom/movable/speaker, message_language, raw_message, radio_freq, list/spans, message_mode)
	...
	addtimer(CALLBACK(src, action), 1 SECONDS, TIMER_UNIQUE|TIMER_STOPPABLE)
...
/datum/ai_behavior/slugcat/proc/handle_repeat_speech(datum/source, message, atom/movable/speaker, message_language, raw_message, radio_freq, list/spans, message_mode)
	...
	if(speaker != mob_master)
		return
	addtimer(CALLBACK(src, .proc/say, raw_message, SPEECH_CONTROLLER_QUEUE_SAY_VERB), 1 SECONDS, TIMER_UNIQUE|TIMER_STOPPABLE)
...
/datum/ai_behavior/slugcat/proc/handle_fetch(datum/source, atom/target)
	SIGNAL_HANDLER
	INVOKE_ASYNC(src, .proc/async_fetch)

/datum/ai_behavior/slugcat/proc/async_fetch(atom/target)
	if(!can_interact_item(target))
...
/datum/ai_behavior/slugcat/proc/pickup_item(datum/source, atom/target)
...
/datum/ai_behavior/slugcat/proc/can_interact_item(atom/movable/target)
	if(!can_interact_obj(target))
		return FALSE
	return isitem(target)

/datum/ai_behavior/slugcat/proc/can_interact_obj(atom/movable/target)
	if(!target || !isturf(target?.loc) || !isobj(target) || target.anchored)
		return FALSE
