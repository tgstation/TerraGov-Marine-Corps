...
/obj/item/weapon/gun/proc/reload(var/mob/user = null, var/obj/item/ammo_magazine/magazine)
	...
	for(var/I in magazine.stored_ammo)
		if(magazine.stored_ammo[I] > 0) //Does the ammo type actually have ammo in it?
			load_type = I
			found = 1
			break

	if(magazine.stored_ammo.len == 0 || found == 0)
		if(user) user << "That [src.name] is empty!"
		return 0

	if(!ammo) //No ammo datum stored
		if(ispath(load_type)) //Make sure it's correct in the mag!
			ammo = new load_type() //Create the new datum!

	if(ispath(load_type) && !istype(ammo,load_type)) //Our old datum might have been different. Let's change it. The old one will be GC'd. ugh
		ammo = new load_type()

	magazine.stored_ammo[load_type]-- //Reduce the ammo in the mag.

	if(rail)
		if(rail.accuracy_mod) ammo.accuracy = initial(ammo.accuracy) + rail.accuracy_mod
		if(rail.ranged_dmg_mod != 100) ammo.damage = (initial(ammo.damage) * rail.ranged_dmg_mod / 100)
	if(muzzle)
		if(muzzle.accuracy_mod) ammo.accuracy = initial(ammo.accuracy) + muzzle.accuracy_mod
		if(muzzle.ranged_dmg_mod != 100) ammo.damage = (initial(ammo.damage) * muzzle.ranged_dmg_mod / 100)
	if(under)
		if(under.accuracy_mod) ammo.accuracy = initial(ammo.accuracy) + under.accuracy_mod
		if(under.ranged_dmg_mod != 100) ammo.damage = (initial(ammo.damage) * under.ranged_dmg_mod / 100)
...
/obj/item/weapon/gun/proc/Fire(atom/target as mob|obj|turf|area, mob/living/user as mob|obj, params, reflex = 0)
	...
	if(istype(user, /mob/living/carbon))
		...
		if(burst_toggled)
			var/scatter_chance = 30 // Base chance of scatter on burst fire.
