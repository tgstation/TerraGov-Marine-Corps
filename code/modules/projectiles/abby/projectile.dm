...
/mob/proc/bullet_message(obj/item/projectile/P)
	...
	if(firingMob && ishuman(firingMob) && ishuman(src) && !firingMob.mind.special_role && !src.mind.special_role) //One human shot another, be worried about it but do everything basically the same //special_role should be null or an empty string if done correctly
