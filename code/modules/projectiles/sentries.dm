...
/obj/machinery/deployable/mounted/sentry/proc/scan()
	...
	for (var/atom/movable/target AS in GLOB.targetable_moving_atoms_list)
		if(QDELETED(target) || z != target.z || get_dist(src, target) > range || HAS_TRAIT(target, TRAIT_TURRET_HIDDEN))
			continue
