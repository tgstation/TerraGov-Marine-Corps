/obj/item/weapon/gun
	...
	var/flags_gun_innate_features = GUN_AUTO_EJECTOR|GUN_CAN_POINTBLANK
...
/obj/item/weapon/gun/Initialize(mapload, spawn_empty) //You can pass on spawn_empty to make the sure the gun has no bullets or mag or anything when created.
	...
	if(current_mag)
		if(spawn_empty && !(flags_gun_innate_features & GUN_INTERNAL_MAG)) //Internal mags will still spawn, but they won't be filled.
			current_mag = null
			update_icon()
		else
		...
		if(flags_gun_innate_features & GUN_LOAD_INTO_CHAMBER && current_mag?.current_rounds > 0)
			load_into_chamber()
	else
	...
	if(CHECK_BITFIELD(flags_gun_innate_features, GUN_IS_ATTACHMENT))
	...
	if(flags_item & IS_DEPLOYABLE)
		if(flags_gun_innate_features & GUN_IS_SENTRY)
...
/obj/item/weapon/gun/proc/examine_ammo_count(mob/user)
	var/list/dat = list()
	if(!(flags_gun_innate_features & (GUN_INTERNAL_MAG|GUN_UNUSUAL_DESIGN))) //Internal mags and unusual guns have their own stuff set.
		if(current_mag?.current_rounds > 0)
			if(flags_gun_innate_features & GUN_AMMO_COUNTER)
...
/obj/item/weapon/gun/wield(mob/user)
	if(CHECK_BITFIELD(flags_gun_innate_features, GUN_DEPLOYED_FIRE_ONLY))
...
/obj/item/weapon/gun/proc/reload(mob/user, obj/item/ammo_magazine/magazine)
	if((flags_gun_innate_features & (GUN_UNUSUAL_DESIGN|GUN_INTERNAL_MAG)) || HAS_TRAIT(src, TRAIT_GUN_BURST_FIRING))
		return
...
/obj/item/weapon/gun/proc/replace_magazine(mob/user, obj/item/ammo_magazine/magazine)
	...
	if(!in_chamber)
		load_into_chamber()
		if(!(flags_gun_innate_features & GUN_ENERGY))
			cock_gun(user)
...
/obj/item/weapon/gun/proc/unload(mob/user, reload_override = 0, drop_override = 0) //Override for reloading mags after shooting, so it doesn't interrupt burst. Drop is for dropping the magazine on the ground.
	if(!reload_override && ((flags_gun_innate_features & (GUN_UNUSUAL_DESIGN|GUN_INTERNAL_MAG)) || HAS_TRAIT(src, TRAIT_GUN_BURST_FIRING)))
		return FALSE

	if((!current_mag || isnull(current_mag) || current_mag.loc != src) && !(flags_gun_innate_features & GUN_ENERGY))
		return cock(user)
...
/obj/item/weapon/gun/proc/cock(mob/user)
	if((flags_gun_innate_features & (GUN_UNUSUAL_DESIGN|GUN_INTERNAL_MAG|GUN_ENERGY)) || HAS_TRAIT(src, TRAIT_GUN_BURST_FIRING))
		return FALSE
...
/obj/item/weapon/gun/proc/reload_into_chamber(mob/user)
	...
	*/
	...
	if(current_mag) //If there is no mag, we can't reload.
		...
		if(current_mag.current_rounds <= 0 && flags_gun_innate_features & GUN_AUTO_EJECTOR) // This is where the magazine is auto-ejected.
...
/obj/item/weapon/gun/proc/Fire()
	...
	if(CHECK_BITFIELD(flags_gun_innate_features, GUN_IS_SENTRY) && CHECK_BITFIELD(flags_item, IS_DEPLOYED) && CHECK_BITFIELD(turret_flags, TURRET_RADIAL) && !gun_user)
...
/obj/item/weapon/gun/attack(mob/living/M, mob/living/user, def_zone)
	if(!CHECK_BITFIELD(flags_gun_innate_features, GUN_CAN_POINTBLANK)) // If it can't point blank, you can't suicide and such.
		return ..()
	...
	DISABLE_BITFIELD(flags_gun_innate_features, GUN_CAN_POINTBLANK) //If they try to click again, they're going to hit themselves.
	...
	if(!do_after(user, 40, TRUE, src, BUSY_ICON_DANGER))
		...
		ENABLE_BITFIELD(flags_gun_innate_features, GUN_CAN_POINTBLANK)
		return
...
/obj/item/weapon/gun/proc/able_to_fire(mob/user)
	...
	if(!user.dextrous)
		...
		return FALSE
	if(!(flags_gun_innate_features & GUN_ALLOW_SYNTHETIC) && !CONFIG_GET(flag/allow_synthetic_gun_use) && issynth(user))
		to_chat(user, span_warning("Your program does not allow you to use this firearm."))
		return FALSE
	if(HAS_TRAIT(src, TRAIT_GUN_SAFETY))
		to_chat(user, span_warning("The safety is on!"))
		return FALSE
	if(CHECK_BITFIELD(flags_gun_innate_features, GUN_WIELDED_FIRING_ONLY)) //If we're not holding the weapon with both hands when we should.
	...
	if(LAZYACCESS(user.do_actions, src))
		...
		return FALSE
	if(CHECK_BITFIELD(flags_gun_innate_features, GUN_WIELDED_STABLE_FIRING_ONLY))//If we must wait to finish wielding before shooting.
...
/obj/item/weapon/gun/proc/apply_gun_modifiers(obj/projectile/projectile_to_fire, atom/target, firer)
	...
	if(flags_gun_innate_features & GUN_IFF || HAS_TRAIT(src, TRAIT_GUN_IS_AIMING) || projectile_to_fire.ammo.flags_ammo_behavior & AMMO_IFF)
		var/iff_signal
		if(ishuman(firer))
