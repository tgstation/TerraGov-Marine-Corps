...
/obj/fire/melting_fire/melting_acid/handle_human(mob/living/carbon/human/affected_human)
	// In sum: a quarter of the damage compared to normal melting fire, but unavoidable / cannot be extinguished.
	var/datum/status_effect/stacking/melting_acid/debuff = affected_human.has_status_effect(STATUS_EFFECT_MELTING_ACID)
	if(debuff)
		debuff.add_stacks(max(1, PYROGEN_MELTING_FIRE_EFFECT_STACK / 2))
	else
		affected_human.apply_status_effect(STATUS_EFFECT_MELTING_FIRE, 1)
	affected_human.take_overall_damage(PYROGEN_MELTING_FIRE_DAMAGE / 2, BURN, FIRE, updating_health = TRUE, max_limbs = 2)
