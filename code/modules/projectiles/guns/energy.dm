...
/obj/item/weapon/gun/energy/rad_cleanser
	name = "rad cleanser"
	desc = "Rads go brr."
	icon_state = "taser"
	item_state = "taser"
	...
	ammo_datum_type  = /datum/ammo/energy/taser
	default_ammo_type = /obj/item/cell/lasgun/volkite/highcap
	allowed_ammo_types = list(/obj/item/cell/lasgun/volkite, /obj/item/cell/lasgun/volkite/highcap)
	rounds_per_shot = 120
	max_shots = 12
	flags_gun_features = GUN_AMMO_COUNTER|GUN_AMMO_COUNT_BY_SHOTS_REMAINING|GUN_ENERGY|GUN_WIELDED_FIRING_ONLY|GUN_WIELDED_STABLE_FIRING_ONLY|GUN_NO_PITCH_SHIFT_NEAR_EMPTY
	gun_skill_category = GUN_SKILL_RIFLES
	attachable_allowed = list()
	attachable_offset = list("muzzle_x" = 34, "muzzle_y" = 14,"rail_x" = 18, "rail_y" = 18, "under_x" = 23, "under_y" = 10, "stock_x" = 22, "stock_y" = 12)
	aim_slowdown = 0.5
	wield_delay = 0.5 SECONDS
	wield_penalty = 0.4 SECONDS
	...
	var/max_range = 8
	///how wide of a cone the cleanser hits.
	var/cone_angle = 55

/obj/item/weapon/gun/energy/rad_cleanser/do_fire(obj/projectile/projectile_to_fire)
	playsound(loc, fire_sound, 50, 1)
	var/dir_to_target = Get_Angle(src, target)
	var/list/turf/turfs_to_ignite = generate_true_cone(get_turf(src), max_range, 1, cone_angle, dir_to_target, bypass_window = TRUE, projectile = TRUE, bypass_xeno = TRUE)
...
/obj/item/weapon/gun/energy/rad_cleanser/proc/burn_list(list/turf/turfs_to_burn)
	if(!length(turfs_to_burn) || !length(chamber_items))
		return FALSE

	for(var/turf/turf_to_ignite AS in turfs_to_burn)
		for(var/mob/living/victim in turf_to_ignite)
			var/strength
			//geiger_counter = new(null, FALSE)
			if(get_dist(victim, get_turf(src)) <= inner_range)
				strength = rad_strength
				//geiger_counter.severity = 3
			else if(get_dist(victim, get_turf(src)) <= mid_range)
				strength = rad_strength * 0.7
				//geiger_counter.severity = 3
			else
				strength = rad_strength * 0.5
				//geiger_counter.severity = 2
			irradiate(victim, strength)
			//geiger_counter.start(victim)


///Applies the actual effects of the rad cleanser
/obj/item/weapon/gun/energy/rad_cleanser/proc/irradiate(mob/living/victim, strength)
