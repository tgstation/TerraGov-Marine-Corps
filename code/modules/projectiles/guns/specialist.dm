...
/obj/item/weapon/gun/launcher/m92/attackby(obj/item/attackedby, mob/user, params)
	if(istype(attackedby, /obj/item/explosive/grenade))
		...
		to_chat(user, "<span class='notice'>You put [attackedby] in the grenade launcher.</span>")
		to_chat(user, "<span class='info'>Now storing: [grenades.len] / [max_grenades] grenades.</span>")

	else if(istype(attackedby, /obj/item/attachable) && check_inactive_hand(user))
...
/obj/item/weapon/gun/launcher/m81/attackby(obj/item/attackedby, mob/user, params)
	if(istype(attackedby, /obj/item/explosive/grenade))
		if(!istype(attackedby, grenade_type_allowed))
