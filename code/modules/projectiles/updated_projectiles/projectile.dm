...
/obj/item/projectile/proc/generate_bullet(ammo_datum, bonus_damage = 0, reagent_multiplier = 0)
	...
	accuracy   *= rand(CONFIG_GET(number/proj_variance_low)-ammo.accuracy_var_low, CONFIG_GET(number/proj_variance_high)+ammo.accuracy_var_high) * CONFIG_GET(number/proj_base_accuracy_mult)//Rand only works with integers.
	damage     *= rand(CONFIG_GET(number/proj_variance_low)-ammo.damage_var_low, CONFIG_GET(number/proj_variance_high)+ammo.damage_var_high) * CONFIG_GET(number/proj_base_damage_mult)
	...
	for(var/atom/movable/A in T)
		...
		if(hit_chance)
			if(isliving(A))
				...
				var/hit_roll
				var/critical_miss = rand(CONFIG_GET(number/critical_chance_low), CONFIG_GET(number/critical_chance_high))
				var/i = 0
...
/mob/living/carbon/human/bullet_act(obj/item/projectile/P)
	...
	if( damage > 0 && !(P.ammo.flags_ammo_behavior & AMMO_IGNORE_ARMOR) )
		...
		if(armor > 0) //Armor check. We should have some to continue.
			...
			var/critical_hit = rand(CONFIG_GET(number/critical_chance_low),CONFIG_GET(number/critical_chance_high))
			damage 			-= prob(critical_hit) ? 0 : armor_soak //Chance that you won't soak the initial amount.
			armor			-= round(armor_soak * CONFIG_GET(number/base_armor_resist_low)) //If you still have armor left over, you generally should, we subtract the soak.
			...
			if(damage)
				while(armor > 0 && i < 2) //Going twice. Armor has to exist to continue. Post increment.
					if(prob(armor))
						...
						armor 		-= armor_soak * CONFIG_GET(number/base_armor_resist_high)
						damage 		-= armor_soak
						#if DEBUG_HUMAN_DEFENSE
	...
	if(damage > 0 && !(P.ammo.flags_ammo_behavior & AMMO_IGNORE_ARMOR))
		...
		if(isXenoQueen(src) || isXenoCrusher(src)) //Charging and crest resistances. Charging Xenos get a lot of extra armor, currently Crushers and Queens
			...
			if(P.dir == charger.dir)
				if(isXenoQueen(src))
					armor = max(0, armor - (xeno_caste.armor_deflection * CONFIG_GET(number/xeno_armor_resist_low))) //Both facing same way -- ie. shooting from behind; armour reduced by 50% of base.
				else
					armor = max(0, armor - (xeno_caste.armor_deflection * CONFIG_GET(number/xeno_armor_resist_lmed))) //Both facing same way -- ie. shooting from behind; armour reduced by 75% of base.
			else if(P.dir == reverse_direction(charger.dir))
				armor += round(xeno_caste.armor_deflection * CONFIG_GET(number/xeno_armor_resist_low)) //We are facing the bullet.
			else if(isXenoCrusher(src))
				armor = max(0, armor - (xeno_caste.armor_deflection * CONFIG_GET(number/xeno_armor_resist_vlow))) //side armour eats a bit of shit if we're a Crusher
