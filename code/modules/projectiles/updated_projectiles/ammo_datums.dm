...
/datum/ammo
	...
	New()
		accuracy 			= CONFIG_GET(number/min_hit_accuracy) 	// This is added to the bullet's base accuracy.
		accuracy_var_low	= CONFIG_GET(number/min_proj_variance) 	// How much the accuracy varies when fired.
		accuracy_var_high	= CONFIG_GET(number/min_proj_variance)
		accurate_range 		= CONFIG_GET(number/close_shell_range) 	// For most guns, this is where the bullet dramatically looses accuracy. Not for snipers though.
		max_range 			= CONFIG_GET(number/norm_shell_range) 	// This will de-increment a counter on the bullet.
		damage_var_low		= CONFIG_GET(number/min_proj_variance) 	// Same as with accuracy variance.
		damage_var_high		= CONFIG_GET(number/min_proj_variance)
		damage_falloff 		= CONFIG_GET(number/reg_damage_falloff) 	// How much damage the bullet loses per turf traveled.
		shell_speed 		= CONFIG_GET(number/slow_shell_speed) 	// How fast the projectile moves.
...
/datum/ammo/bullet/New()
	..()
	damage = CONFIG_GET(number/base_hit_damage)
	shrapnel_chance = CONFIG_GET(number/low_shrapnel_chance)
	shell_speed = CONFIG_GET(number/super_shell_speed)
...
/datum/ammo/bullet/pistol/New()
	..()
	damage = CONFIG_GET(number/low_hit_damage)
	accuracy = -CONFIG_GET(number/low_hit_accuracy)
...
/datum/ammo/bullet/pistol/hollow/New()
	..()
	accuracy = -CONFIG_GET(number/med_hit_accuracy)
	shrapnel_chance = CONFIG_GET(number/high_shrapnel_chance)

/datum/ammo/bullet/pistol/hollow/on_hit_mob(mob/M,obj/item/projectile/P)
	staggerstun(M, P, CONFIG_GET(number/close_shell_range), 0, 0, 1, 0.5, 0)
...
/datum/ammo/bullet/pistol/ap/New()
	..()
	damage = CONFIG_GET(number/mlow_hit_damage)
	accuracy = CONFIG_GET(number/low_hit_accuracy)
	penetration= CONFIG_GET(number/med_armor_penetration)
	shrapnel_chance = CONFIG_GET(number/med_shrapnel_chance)
...
/datum/ammo/bullet/pistol/heavy/New()
	..()
	accuracy = -CONFIG_GET(number/low_hit_accuracy)
	accuracy_var_low = CONFIG_GET(number/med_proj_variance)
	damage = CONFIG_GET(number/lmed_hit_damage)
	penetration= CONFIG_GET(number/min_armor_penetration)
	shrapnel_chance = CONFIG_GET(number/med_shrapnel_chance)
...
/datum/ammo/bullet/pistol/incendiary/New()
	..()
	accuracy = CONFIG_GET(number/med_hit_accuracy)
	damage = CONFIG_GET(number/llow_hit_damage)
...
/datum/ammo/bullet/pistol/squash/New()
	..()
	accuracy = CONFIG_GET(number/med_hit_accuracy)
	damage = CONFIG_GET(number/hlow_hit_damage)
	penetration= CONFIG_GET(number/mlow_armor_penetration)
	shrapnel_chance = CONFIG_GET(number/med_shrapnel_chance)
...
/datum/ammo/bullet/pistol/mankey/New()
	..()
	damage = CONFIG_GET(number/min_hit_damage)
	damage_var_high = CONFIG_GET(number/high_proj_variance)
	shell_speed = CONFIG_GET(number/reg_shell_speed)
...
/datum/ammo/bullet/revolver/marksman/New()
	..()
	accuracy = CONFIG_GET(number/med_hit_accuracy)
	accurate_range = CONFIG_GET(number/short_shell_range)
...
/datum/ammo/bullet/revolver/heavy/New()
	..()
	damage = CONFIG_GET(number/med_hit_damage)
	penetration= CONFIG_GET(number/min_armor_penetration)
	accuracy = -CONFIG_GET(number/med_hit_accuracy)
...
/datum/ammo/bullet/revolver/highimpact/New()
	..()
	accuracy_var_high = CONFIG_GET(number/max_proj_variance)
	damage = CONFIG_GET(number/hmed_hit_damage)
	damage_var_low = CONFIG_GET(number/low_proj_variance)
	damage_var_high = CONFIG_GET(number/med_proj_variance)
	penetration= CONFIG_GET(number/mlow_armor_penetration)

/datum/ammo/bullet/revolver/highimpact/on_hit_mob(mob/M,obj/item/projectile/P)
	staggerstun(M, P, CONFIG_GET(number/close_shell_range), 0, 1, 2, 2)
...
/datum/ammo/bullet/smg/New()
	..()
	accuracy_var_low = CONFIG_GET(number/med_proj_variance)
	accuracy_var_high = CONFIG_GET(number/med_proj_variance)
	damage = CONFIG_GET(number/low_hit_damage)
	damage_var_low = CONFIG_GET(number/med_proj_variance)
	damage_var_high = CONFIG_GET(number/high_proj_variance)
	accurate_range = CONFIG_GET(number/close_shell_range)
	damage_falloff = CONFIG_GET(number/reg_damage_falloff)
...
/datum/ammo/bullet/smg/ap/New()
	..()
	damage = CONFIG_GET(number/llow_hit_damage)
	penetration= CONFIG_GET(number/hmed_armor_penetration) //40 AP
...
/datum/ammo/bullet/rifle/ap/New()
	..()
	damage = CONFIG_GET(number/low_hit_damage)
	penetration = CONFIG_GET(number/high_armor_penetration)
...
/datum/ammo/bullet/rifle/m4ra/New()
	...
	scatter = -CONFIG_GET(number/low_scatter_value)
	penetration= CONFIG_GET(number/med_armor_penetration)
	shell_speed = CONFIG_GET(number/fast_shell_speed)
...
/datum/ammo/bullet/rifle/m4ra/incendiary/New()
	..()
	damage = CONFIG_GET(number/hmed_hit_damage)
	accuracy = CONFIG_GET(number/hmed_hit_accuracy)
	scatter = -CONFIG_GET(number/low_scatter_value)
	penetration= CONFIG_GET(number/low_armor_penetration)
	shell_speed = CONFIG_GET(number/fast_shell_speed)
...
/datum/ammo/bullet/rifle/m4ra/impact/on_hit_mob(mob/M, obj/item/projectile/P)
	staggerstun(M, P, CONFIG_GET(number/max_shell_range), 0, 1, 1)
...
/datum/ammo/bullet/shotgun/slug/New()
	..()
	max_range = CONFIG_GET(number/short_shell_range)
	damage = CONFIG_GET(number/high_hit_damage)
	penetration= CONFIG_GET(number/low_armor_penetration)

/datum/ammo/bullet/shotgun/slug/on_hit_mob(mob/M,obj/item/projectile/P)
	staggerstun(M, P, CONFIG_GET(number/close_shell_range), 0, 1, 2, 4)
...
/datum/ammo/bullet/shotgun/flechette/New()
	...
	damage = CONFIG_GET(number/med_hit_damage)
	damage_var_low = -CONFIG_GET(number/low_proj_variance)
	damage_var_high = CONFIG_GET(number/low_proj_variance)
	damage_falloff *= 0.5
	penetration	= CONFIG_GET(number/high_armor_penetration)
	bonus_projectiles_amount = CONFIG_GET(number/low_proj_extra)
...
/datum/ammo/bullet/shotgun/flechette_spread/New()
	...
	damage_falloff *= 0.5
	penetration	= CONFIG_GET(number/high_armor_penetration)
	scatter = CONFIG_GET(number/thirty_scatter_value) //bonus projectiles run their own scatter chance
...
/datum/ammo/bullet/shotgun/buckshot/New()
	..()
	accuracy_var_low = CONFIG_GET(number/high_proj_variance)
	accuracy_var_high = CONFIG_GET(number/high_proj_variance)
	accurate_range = CONFIG_GET(number/min_shell_range)
	max_range = CONFIG_GET(number/near_shell_range)
	damage = CONFIG_GET(number/max_hit_damage)
	damage_var_low = -CONFIG_GET(number/med_proj_variance)
	damage_var_high = CONFIG_GET(number/med_proj_variance)
	damage_falloff = CONFIG_GET(number/buckshot_damage_falloff)
	penetration	= -CONFIG_GET(number/mlow_armor_penetration)
	bonus_projectiles_amount = CONFIG_GET(number/low_proj_extra)
	shell_speed = CONFIG_GET(number/reg_shell_speed)
...
/datum/ammo/bullet/shotgun/spread/New()
	...
	penetration	= -CONFIG_GET(number/mlow_armor_penetration)
	shell_speed = CONFIG_GET(number/reg_shell_speed)
	scatter = CONFIG_GET(number/max_scatter_value)*1.5 //bonus projectiles run their own scatter chance
...
/datum/ammo/bullet/sniper/New()
	..()
	accurate_range = CONFIG_GET(number/long_shell_range)
	max_range = CONFIG_GET(number/max_shell_range)
	scatter = -CONFIG_GET(number/med_scatter_value)
	damage = CONFIG_GET(number/mhigh_hit_damage)
	penetration= CONFIG_GET(number/mhigh_armor_penetration)
	shell_speed = CONFIG_GET(number/ultra_shell_speed)
...
/datum/ammo/bullet/sniper/incendiary/New()
	..()
	accuracy_var_high = CONFIG_GET(number/med_proj_variance)
	max_range = CONFIG_GET(number/norm_shell_range)
	scatter = CONFIG_GET(number/low_scatter_value)
	damage = CONFIG_GET(number/hmed_hit_damage)
	penetration= CONFIG_GET(number/low_armor_penetration)
...
/datum/ammo/bullet/sniper/elite/New()
	..()
	accuracy = CONFIG_GET(number/max_hit_accuracy)
	damage = CONFIG_GET(number/super_hit_damage)
	shell_speed = CONFIG_GET(number/ultra_shell_speed) + 1
...
/datum/ammo/bullet/smartgun/dirty/New()
	..()
	shrapnel_chance = CONFIG_GET(number/max_shrapnel_chance)
...
/datum/ammo/bullet/turret/New()
	..()
	accurate_range = CONFIG_GET(number/short_shell_range)
	accuracy_var_low = CONFIG_GET(number/low_proj_variance)
	accuracy_var_high = CONFIG_GET(number/low_proj_variance)
	damage = CONFIG_GET(number/lmed_hit_damage)
	penetration= CONFIG_GET(number/low_armor_penetration)
...
/datum/ammo/bullet/turret/mini/New()
	. = ..()
	damage = CONFIG_GET(number/mlow_hit_damage) //25
	penetration= CONFIG_GET(number/hmed_armor_penetration) //40
...
/datum/ammo/bullet/machinegun/New()
	...
	accurate_range = CONFIG_GET(number/short_shell_range)
	damage = CONFIG_GET(number/med_hit_damage)
	penetration= CONFIG_GET(number/mhigh_armor_penetration) //Bumped the penetration to serve a different role from sentries, MGs are a bit more offensive
	accuracy = CONFIG_GET(number/med_hit_accuracy)
...
/datum/ammo/rocket/New()
	...
	smoke = new()
	accuracy = CONFIG_GET(number/max_hit_accuracy)
	accurate_range = CONFIG_GET(number/norm_shell_range)
	max_range = CONFIG_GET(number/long_shell_range)
	damage = CONFIG_GET(number/med_hit_damage)
	penetration = CONFIG_GET(number/max_armor_penetration)
	shell_speed = CONFIG_GET(number/slow_shell_speed)
...
/datum/ammo/rocket/ap/New()
	. = ..()
	accurate_range = CONFIG_GET(number/short_shell_range)
	penetration = CONFIG_GET(number/aprocket_armor_penetration)
	damage = CONFIG_GET(number/aprocket_hit_damage)
...
/datum/ammo/rocket/ltb/New()
	...
	accurate_range = CONFIG_GET(number/short_shell_range)
	max_range = CONFIG_GET(number/max_shell_range)
	penetration = CONFIG_GET(number/ltb_armor_penetration)
	damage = CONFIG_GET(number/ltb_hit_damage)
	shell_speed = CONFIG_GET(number/fast_shell_speed)
...
/datum/ammo/rocket/wp/quad/New()
	..()
	damage = CONFIG_GET(number/ultra_hit_damage)
	max_range = CONFIG_GET(number/long_shell_range)
...
/datum/ammo/energy/emitter/New()
	..()
	accurate_range 	= CONFIG_GET(number/near_shell_range)
	max_range 		= CONFIG_GET(number/near_shell_range)
...
/datum/ammo/xeno/boiler_gas/corrosive/New()
	..()
	damage = CONFIG_GET(number/med_hit_damage)
	damage_var_high = CONFIG_GET(number/max_proj_variance)
	damage_type = BURN
...
/datum/ammo/flamethrower/New()
	..()
	max_range = CONFIG_GET(number/close_shell_range)
	damage = CONFIG_GET(number/med_hit_damage)
