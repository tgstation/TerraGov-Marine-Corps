...
/obj/item/weapon/gun/flamer/proc/triangular_flame(var/atom/target, var/mob/living/user, var/burntime, var/burnlevel)
	...
	for (var/turf/T in turfs)
		...
		if(DirBlocked(T,user.dir))
			break

		else if (DirBlocked(T,turn(user.dir,180)))
			break

		if(locate(/obj/structure/mineral_door/resin, T))
			break
		...
		for (var/turf/R in right)
			if (R.density || locate(/obj/structure/mineral_door/resin, R))
				break
		...
		for (var/turf/L in left)
			if (L.density || locate(/obj/structure/mineral_door/resin, L))
				break
		...
		do_triangular_flame_width(T, user, distance, burntime, burnlevel)
		distance++


/obj/item/weapon/gun/flamer/proc/do_triangular_flame_width(var/turf/T, var/mob/living/user, var/distance, var/burntime, var/burnlevel)
	var/list/turf/right = list()
	var/list/turf/left = list()
	var/turf/right_turf = T
	var/turf/left_turf = T
	...
	var/obj/structure/window/W = null
