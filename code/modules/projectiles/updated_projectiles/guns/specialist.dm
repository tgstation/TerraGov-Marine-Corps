...
/obj/item/weapon/gun/rifle/sniper/M42A/proc/laser_on(mob/user, silent = FALSE)
	...
	if(!zoom)
		...
		return
	targetlaser_on = TRUE
	accuracy_mult += CONFIG_GET(number/max_hit_accuracy_mult) //We get a big accuracy bonus vs the lasered target
	if(!silent && user)
...
/obj/item/weapon/gun/rifle/sniper/M42A/proc/laser_off(mob/user, toggle_off = TRUE, silent = FALSE)
	if(laser_target)
		laser_target.remove_laser()
	laser_target = null
	accuracy_mult -= CONFIG_GET(number/max_hit_accuracy_mult) //We lose a big accuracy bonus vs the now unlasered target
...
/obj/item/weapon/gun/rifle/sniper/M42A/set_gun_config_values()
	fire_delay = CONFIG_GET(number/high_fire_delay) * 5
	burst_amount = CONFIG_GET(number/min_burst_value)
...
/obj/item/weapon/gun/rifle/sniper/elite/set_gun_config_values()
	...
	damage_mult = CONFIG_GET(number/base_hit_damage_mult)
	recoil = CONFIG_GET(number/max_recoil_value)
...
/obj/item/weapon/gun/smartgun/dirty/set_gun_config_values()
	...
	burst_amount = CONFIG_GET(number/med_burst_value)
	burst_delay = CONFIG_GET(number/min_fire_delay)
	accuracy_mult = CONFIG_GET(number/base_hit_accuracy_mult) + CONFIG_GET(number/min_hit_accuracy_mult) + CONFIG_GET(number/min_hit_accuracy_mult)
	scatter = CONFIG_GET(number/med_scatter_value)
	damage_mult = CONFIG_GET(number/base_hit_damage_mult)
...
/obj/item/weapon/gun/launcher/m92/set_gun_config_values()
	fire_delay = CONFIG_GET(number/max_fire_delay) * 3
	accuracy_mult = CONFIG_GET(number/base_hit_accuracy_mult)
...
/obj/item/weapon/gun/launcher/m81/set_gun_config_values()
	fire_delay = CONFIG_GET(number/max_fire_delay) * 1.5
	accuracy_mult = CONFIG_GET(number/base_hit_accuracy_mult)
	scatter = CONFIG_GET(number/med_scatter_value)
	damage_mult = CONFIG_GET(number/base_hit_damage_mult)
